...
def getLogsFromFile(request):
    try:
        ...
        status = {"status": 0, "logstatus":0,"error":"Could not fetch data from log file, please see FoxitiCP main log file through command line."}
        logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[getLogsFromFile]")
        final_json = json.dumps(status)
        return HttpResponse(final_json)
        ...
        try:
            if request.method == 'POST':
                ...
                allowedLogFiles = {
                    ...
                    "/var/log/messages",
                    "/var/log/syslog",
                    "/home/foxitipanel/error-logs.txt",
                }

                if fileName not in allowedLogFiles:
                    return ACLManager.loadErrorJson('cleanStatus', 0)

                execPath = "/usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/plogical/serverLogs.py"
