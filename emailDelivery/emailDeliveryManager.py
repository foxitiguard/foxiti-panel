...
class EmailDeliveryManager:
    ...
    PLATFORM_URL = 'https://cp.foxitiguard.com/email/cp/'
    ...
    def enableRelay(self, request, userID):
        try:
            ...
            import shlex
            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/mailUtilities.py" \
    ...
    def disableRelay(self, request, userID):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/mailUtilities.py removeRelayHost"
            output = ProcessUtilities.outputExecutioner(execPath)
