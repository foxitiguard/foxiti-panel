...
class ContainerManager(multi.Thread):
    ...
    def executeContainerCommand(self, userID=None, data=None):
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f'executeContainerCommand error: {error_msg}')
            data_ret = {'commandStatus': 0, 'error_message': error_msg}
            return HttpResponse(json.dumps(data_ret))
    ...
    def _check_rate_limit(self, userID, containerName):
        ...
        rate_limit_dir = '/tmp/foxitipanel_docker_rate_limit'
        if not os.path.exists(rate_limit_dir):
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(f'Warning: Could not create rate limit directory: {str(e)}')
                return True
        ...
        try:
            ...
            if len(recent_timestamps) >= 10:
                logging.FoxitiCPLogFileWriter.writeToFile(f'Rate limit exceeded for user {userID}, container {containerName}')
                return False
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f'Rate limiting error: {str(e)}')
            return True
    ...
    def _log_command_execution(self, userID, containerName, command):
        ...
        logging.FoxitiCPLogFileWriter.writeToFile(log_message)
