...
# Test script for Ubuntu 26.04 support in foxitiPanel
# Verifies that foxitiPanel detects Ubuntu 26.04 (Resolute Raccoon) and that the
# dependencies which differ from earlier Ubuntu releases resolve correctly.
#
# Ubuntu 26.04 differs from 24.04 in several ways that matter to foxitiPanel:
...
echo "foxitiPanel Ubuntu 26.04 Support Test"
echo "===================================="
echo ""
...
echo "----------------------"
if [ -x /usr/local/FoxitiCP/bin/python ]; then
    VENV_PY=$(/usr/local/FoxitiCP/bin/python --version 2>&1 | awk '{print $2}')
    VENV_MINOR=$(echo "$VENV_PY" | cut -d. -f1,2)
    if [[ "$VENV_MINOR" == "3.12" ]]; then
        pass "FoxitiCP venv runs Python $VENV_PY"
    else
        fail "FoxitiCP venv runs Python $VENV_PY, expected 3.12.x (Django 4.2 supports <=3.12)"
    fi
    ...
    if [[ "$VERSION_ID" == "26.04" ]]; then
        if [[ "$(stat -c %a /usr/local/FoxitiCP/pyvenv.cfg 2>/dev/null)" == "644" ]]; then
            pass "FoxitiCP venv metadata is readable by lswsgi"
        else
            fail "FoxitiCP venv metadata permissions prevent lswsgi startup"
        fi

        if runuser -u foxitipanel -- test -r /usr/local/FoxitiCP/lib/python3.12/encodings/__init__.py; then
    ...
    info "FoxitiCP venv not present; skipping (run after install)"
fi

if grep -q 'cp -a /usr/lib/python3.12/. /usr/local/FoxitiCP/lib/python3.12/' "$REPO_ROOT/foxitipanel.sh" \
    && grep -q 'chmod 0644 /usr/local/FoxitiCP/pyvenv.cfg' "$REPO_ROOT/foxitipanel.sh"; then
...
echo "-----------------------------"
if grep -q '/root/foxitipanel/cert_conf' "$REPO_ROOT/foxitipanel.sh"; then
    fail "certificate generation assumes a /root/foxitipanel checkout"
else
...
' "$REPO_ROOT/install/installfoxitiPanel.py"; then
...
echo "---------------------------"
if grep -q 'DNSStubListener=no' "$REPO_ROOT/foxitipanel.sh" \
    && grep -q 'chmod 0644 /etc/systemd/resolved.conf' "$REPO_ROOT/foxitipanel.sh" \
    && grep -q 'keep_resolved' "$REPO_ROOT/install/installfoxitiPanel.py"; then
...
if grep -q 'Acquire::Retries' "$REPO_ROOT/foxitipanel.sh" \
    && grep -q 'APT::Update::Error-Mode=any' "$REPO_ROOT/foxitipanel.sh" \
...
if [[ "$VERSION_ID" == "26.04" ]] && [ -d /usr/local/FoxitiCP ]; then
    ...
    if [[ "$(stat -c %a /etc/systemd/resolved.conf 2>/dev/null)" == "644" ]]; then
        pass "systemd-resolved can read the foxitiPanel configuration"
    else
        fail "systemd-resolved configuration permissions are incorrect"
    fi

    if runuser -u foxitipanel -- test -r /etc/foxitipanel/machineIP; then
        pass "foxitiPanel can read the installed server address"
    else
        fail "foxitiPanel cannot read the installed server address"
    fi

    if runuser -u foxitipanel -- /usr/local/FoxitiCP/bin/python -c \
        'import socket; s=socket.socket(socket.AF_UNIX); s.connect("/usr/local/lscpd/admin/comm.sock"); s.close()' \
        2>/dev/null; then
        pass "foxitiPanel can reach the LSCPD command socket"
    else
        fail "foxitiPanel cannot reach the LSCPD command socket"
    fi
...
if [[ "$VERSION_ID" == "26.04" ]] && [ -d /usr/local/FoxitiCP ]; then
    ...
    if [[ "$PANEL_STATUS" == "200" ]]; then
        pass "foxitiPanel returns HTTP 200"
    else
        fail "foxitiPanel returned HTTP $PANEL_STATUS"
    fi
fi
echo ""
