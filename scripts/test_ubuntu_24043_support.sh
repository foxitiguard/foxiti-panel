...
# Test script for Ubuntu 24.04.3 support in foxitiPanel
# This script verifies that foxitiPanel properly detects and handles Ubuntu 24.04.3
...
if command -v python3 -m venv --help &> /dev/null; then
    ...
    TEST_VENV="/tmp/foxitipanel_test_venv"
...
# Test 6: foxitiPanel version detection
echo ""
echo "Test 6: foxitiPanel Version Detection"
echo "------------------------------------"
if [ -f /usr/local/FoxitiCP/plogical/upgrade.py ]; then
...
" 2>/dev/null; then
        echo "✅ foxitiPanel OS detection test completed"
    else
        echo "❌ foxitiPanel OS detection test failed"
    fi
else
    echo "⚠️  foxitiPanel not installed - skipping detection test"
fi
