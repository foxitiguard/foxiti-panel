...
WEBMAIL_CONF = '/etc/foxitipanel/webmail.conf'
...
class WebmailManager:
    ...
    def _get_master_config(self):
        """Read master user config from /etc/foxitipanel/webmail.conf"""
        try:
    ...
    def _get_managed_accounts(self):
        """Get email accounts the current foxitiPanel user can access."""
        try:
    ...
    def apiSSO(self):
        """Auto-login for foxitiPanel users."""
        if self.request.session.get('webmail_standalone'):
    ...
    def apiDeleteFolder(self):
        ...
        if not name:
            return self._error('Folder name is required.')
        # foxitiPanel/Dovecot folder names (INBOX. prefix, separator '.')
    ...
    def apiSaveDraft(self):
        try:
            ...
            with self._get_imap() as imap:
                # foxitiPanel's Dovecot uses INBOX.Drafts
    ...
    def apiDeleteMessages(self):
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile('Webmail deletion failed: %s' % error)
    ...
    def apiMoveMessages(self):
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile('Webmail move failed: %s' % error)
            return self._error(str(error))
        except Exception as error:
            logging.FoxitiCPLogFileWriter.writeToFile('Webmail move failed: %s' % error)
    ...
    def _sync_sieve_rules(self, email):
        ...
        try:
            with self._get_sieve(email) as sieve:
                sieve.put_script('foxitipanel', script)
                sieve.activate_script('foxitipanel')
        except ConnectionRefusedError:
            logging.FoxitiCPLogFileWriter.writeToFile(
                'Sieve sync skipped for %s: ManageSieve not running on port 4190. '
                'Install dovecot-sieve and enable ManageSieve.' % email)
        except Exception as e:
            logging.FoxitiCPLogFileWriter.writeToFile('Sieve sync failed for %s: %s' % (email, str(e)))
    ...
    def apiProxyImage(self):
        ...
        try:
            ...
            req = urllib.request.Request(url, headers={
                'User-Agent': 'foxitiPanel-Webmail-Proxy/1.0',
            })
