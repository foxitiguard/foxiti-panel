...
class TestManageSSL(TestCase):
    ...
    def test_issueSSL(self):
        ...
        command = 'rm -rf /home/%s/public_html/' % ('foxitipanel.xyz')
        ProcessUtilities.normalExecutioner(command)

        command = 'mkdir /home/%s/public_html/' % ('foxitipanel.xyz')
        ProcessUtilities.normalExecutioner(command)

        command = 'chown cyberpa:cyberpa /home/%s/public_html/' % ('foxitipanel.xyz')
        ProcessUtilities.normalExecutioner(command)

        path = '/home/%s/public_html/index.html' % ('foxitipanel.xyz')

        writeToFile = open(path, 'w')
        writeToFile.write('foxitiPanel')
        writeToFile.close()

        exists = 0
        if self.MakeRequestRaw('https://foxitipanel.xyz').find('foxitiPanel') > -1:
            exists = 1
