...
class MailServerManager(multi.Thread):
    ...
    def run(self):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + ' [MailServerManager.run]')
    ...
    def createEmailAccount(self):
        ...
        if not os.path.exists('/home/foxitipanel/postfix'):
        ...
        try:
            ...
            if result[0] == 1:
                ...
                if EmailLimits != -1:
                    ...
                    RandomFile = "/home/foxitipanel/" + str(randint(100000, 999999))
                    writeToFile = open(RandomFile, 'w')
                    writeToFile.write(limitString)
                    writeToFile.close()

                    execPath = "/usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/plogical/mailUtilities.py"
    ...
    def fixMailSSL(self, data = None):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/plogical/virtualHostUtilities.py"
    ...
    def generateDKIMKeys(self):
        try:
            ...
            if output.find("1,None") > -1:
                ...
                command = 'chown foxitipanel:foxitipanel -R /usr/local/FoxitiCP/lib/python3.6/site-packages/tldextract/.suffix_cache'
                ProcessUtilities.executioner(command)

                command = 'chown foxitipanel:foxitipanel -R /usr/local/FoxitiCP/lib/python3.8/site-packages/tldextract/.suffix_cache'
                ProcessUtilities.executioner(command)

                command = 'chown foxitipanel:foxitipanel -R /usr/local/FoxitiCP/lib/python*/site-packages/tldextract/.suffix_cache'
    ...
    def RunServerLevelEmailChecks(self):
        try:
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Checking if MailServer SSL issued..,10')
    ...
    def install_postfix_dovecot(self):
        try:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Postfix/dovecot reinstalled.,40')

        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], '%s [install_postfix_dovecot][404]' % (str(msg)), 10)
            return 0
    ...
    def setup_postfix_dovecot_config(self):
        try:
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu:
                self.centos_lib_dir_to_ubuntu("/usr/local/FoxitiCP/install/email-configs-one/master.cf", "/usr/libexec/", "/usr/lib/")
                self.centos_lib_dir_to_ubuntu("/usr/local/FoxitiCP/install/email-configs-one/main.cf", "/usr/libexec/postfix",
            ...
            shutil.copy("/usr/local/FoxitiCP/install/email-configs-one/mysql-virtual_domains.cf", "/etc/postfix/mysql-virtual_domains.cf")
            shutil.copy("/usr/local/FoxitiCP/install/email-configs-one/mysql-virtual_forwardings.cf",
                        "/etc/postfix/mysql-virtual_forwardings.cf")
            shutil.copy("/usr/local/FoxitiCP/install/email-configs-one/mysql-virtual_mailboxes.cf", "/etc/postfix/mysql-virtual_mailboxes.cf")
...
"""
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f'Error in configureOpenDKIM {str(msg)}')
            return 0
    ...
    def installSieveAfterReset(self):
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile("Sieve reinstalled and configured after email reset")
            return 1
            
        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.writeToFile("Failed to reinstall Sieve after email reset: " + str(msg))
            return 0
...
"""
            ...
            logging.FoxitiCPLogFileWriter.writeToFile("Sieve configured in Dovecot successfully")
            return 1
            
        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.writeToFile("Failed to configure Sieve in Dovecot: " + str(msg))
            return 0
    ...
    def ResetEmailConfigurations(self):
        try:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Removing and re-installing postfix/dovecot..,5')

            if self.install_postfix_dovecot() == 0:
                return 0

            # Ensure Sieve remains functional after email debugger reset
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Reinstalling Sieve after email reset..,45')
            ...
            if self.setup_postfix_dovecot_config() == 0:
                logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'setup_postfix_dovecot_config failed. [404].')
                return 0

            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Restoreing OpenDKIM configurations..,70')
            ...
            if self.configureOpenDKIM() == 0:
                logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'configureOpenDKIM failed. [404].')
                return 0


            if self.MailSSL:
                logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Setting up Mail Server SSL if any..,75')
                from plogical.virtualHostUtilities import virtualHostUtilities
    ...
    @premium_entitlement_required('all', label='Email Limits',
                                  page_redirect='https://foxitipanel.net/foxitipanel-addons')
    ...
    def _regenerateVirtualRegexp(self):
        ...
        try:
            ...
            content = "# Auto-generated by foxitiPanel - DO NOT EDIT MANUALLY\n"
            for rule in rules:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + ' [_regenerateVirtualRegexp]')
            return False


def main():

    parser = argparse.ArgumentParser(description='foxitiPanel')
