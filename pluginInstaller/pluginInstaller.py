...
class pluginInstaller:
    ...
    def migrationsEnabled(pluginName: str) -> bool:
        pluginHome = '/usr/local/FoxitiCP/' + pluginName
    ...
    def extractPlugin(pluginName):
        ...
        command = 'unzip ' + pathToPlugin + ' -d /usr/local/FoxitiCP'
        subprocess.call(shlex.split(command))

    @staticmethod
    def upgradingSettingsFile(pluginName):
        data = open("/usr/local/FoxitiCP/FoxitiCP/settings.py", 'r').readlines()
        writeToFile = open("/usr/local/FoxitiCP/FoxitiCP/settings.py", 'w')
    ...
    @staticmethod
    def upgradingURLs(pluginName, urlsPath="/usr/local/FoxitiCP/FoxitiCP/urls.py"):
    ...
    @staticmethod
    def informfoxitiPanel(pluginName):
        pluginPath = '/home/foxitipanel/plugins'
    ...
    def addInterfaceLink(pluginName):
        data = open("/usr/local/FoxitiCP/baseTemplate/templates/baseTemplate/index.html", 'r').readlines()
        writeToFile = open("/usr/local/FoxitiCP/baseTemplate/templates/baseTemplate/index.html", 'w')
    ...
    def staticContent():
        ...
        command = "rm -rf /usr/local/lscp/foxitipanel/static"
        subprocess.call(shlex.split(command))

        os.chdir('/usr/local/FoxitiCP')

        command = "/usr/local/FoxitiCP/bin/python manage.py collectstatic --noinput"
        subprocess.call(shlex.split(command))

        command = "mv /usr/local/FoxitiCP/static /usr/local/lscp/foxitipanel"
        subprocess.call(shlex.split(command))
    ...
    def installMigrations(pluginName):
        currentDir = os.getcwd()
        os.chdir('/usr/local/FoxitiCP')
        command = "/usr/local/FoxitiCP/bin/python manage.py makemigrations %s" % pluginName
        subprocess.call(shlex.split(command))
        command = "/usr/local/FoxitiCP/bin/python manage.py migrate %s" % pluginName
        subprocess.call(shlex.split(command))
        os.chdir(currentDir)
    ...
    def installPlugin(pluginName):
        try:
            ...
            pluginInstaller.stdOut('Informing foxitiPanel about plugin.')
            pluginInstaller.informfoxitiPanel(pluginName)
            pluginInstaller.stdOut('foxitiPanel core informed about the plugin.')
    ...
    def removeFiles(pluginName):
        pluginPath = '/usr/local/FoxitiCP/' + pluginName
    ...
    @staticmethod
    def removeFromURLs(pluginName, urlsPath="/usr/local/FoxitiCP/FoxitiCP/urls.py"):
    ...
    @staticmethod
    def informfoxitiPanelRemoval(pluginName):
        pluginPath = '/home/foxitipanel/plugins'
        pluginFile = pluginPath + '/' + pluginName
    ...
    def removeMigrations(pluginName):
        currentDir = os.getcwd()
        os.chdir('/usr/local/FoxitiCP')
        command = "/usr/local/FoxitiCP/bin/python manage.py migrate %s zero" % pluginName
        subprocess.call(shlex.split(command))
        os.chdir(currentDir)
    ...
    def removePlugin(pluginName):
        try:
            ...
            pluginInstaller.stdOut('Informing foxitiPanel about plugin removal.')
            pluginInstaller.informfoxitiPanelRemoval(pluginName)
            pluginInstaller.stdOut('foxitiPanel core informed about the plugin removal.')
