...
**foxitiPanel Version**: 2.4.5+
**Platform**: https://cp.foxitiguard.com
...
1. foxitiPanel 2.4.5 or later installed
...
The CyberMail module is included in foxitiPanel 2.4.5+. No separate installation needed.

### Verify the Module

```bash
# Check the app exists
ls /usr/local/FoxitiCP/emailDelivery/

# Check it's in INSTALLED_APPS
grep -n "emailDelivery" /usr/local/FoxitiCP/FoxitiCP/settings.py
...
2. Enter your email address (defaults to your foxitiPanel admin email)
...
Upgrade at: https://cp.foxitiguard.com
...
- Check if the domain was created via foxitiPanel's DNS management
...
- Ensure `/usr/local/FoxitiCP/plogical/mailUtilities.py` has the `configureRelayHost` method
