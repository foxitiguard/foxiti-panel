...
**Platform API Base**: `https://cp.foxitiguard.com/email/cp/`
...
- **Per-User API Keys**: Each foxitiPanel admin gets their own platform API key (no server-level key)
...
## API Endpoints (foxitiPanel Internal)

All endpoints are POST, require authenticated foxitiPanel session, and return JSON.
...
Each foxitiPanel endpoint maps to a platform API call:

| foxitiPanel Method | Platform Endpoint | Auth | Notes |
|-------------------|-------------------|------|-------|
...
| Platform Field | foxitiPanel Field | Method |
|---------------|-----------------|--------|
...
2. If no credential:
   → POST api/smtp/create/ {email, description: "foxitiPanel Relay"}
...
### Logging
All errors logged via `FoxitiCPLogFileWriter.writeToFile()` with format:
```
[EmailDeliveryManager.<method>] Error: <message>
```

Log file: `/home/foxitipanel/error-logs.txt`
...
| File | Modifications |
|------|--------------|
| `FoxitiCP/settings.py` | Added `'emailDelivery'` to `INSTALLED_APPS` |
| `FoxitiCP/urls.py` | Added `path('emailDelivery/', include('emailDelivery.urls'))` |
