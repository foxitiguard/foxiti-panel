# Resource Limits Guide for foxitiPanel

## Overview

foxitiPanel now supports comprehensive resource limits for shared hosting environments using OpenLiteSpeed's native cgroups v2 integration. This feature allows you to enforce CPU, memory, I/O, inode, process, and connection limits on a per-package basis.
...
2. **foxitiPanel Version**: v2.4.5 or later
...
3. **Create Website**
   ...
   - foxitiPanel will automatically:
...
foxitiPanel automatically handles OpenLiteSpeed cgroups setup:
...
1. **Check if enforcement is enabled**:
   ```bash
   mysql -u root -e "SELECT packageName, enforceDiskLimits FROM foxitipanel.packages_package;"
   ```
...
4. **Check foxitiPanel logs**:
   ```bash
   tail -100 /usr/local/FoxitiCP/logs/access.log
   ```
...
1. foxitiPanel should auto-run lssetup
...
- GitHub: https://github.com/usmannasir/foxitipanel/issues
- Community Forum: https://community.foxitipanel.net
- Documentation: https://docs.foxitipanel.net
