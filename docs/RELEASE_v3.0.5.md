# foxitiPanel v3.0.5 (build 5) — Security Release

_Release candidate prepared 2026-08-26._

foxitiPanel 3.0.5 closes an API authentication gap for accounts protected by
...
```bash
/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/adminPass.py \
...
1. Push the tested candidate to `v3.0.5` and merge that exact candidate into
   `stable`.
2. Set `https://foxitipanel.net/version.txt` to exactly
   `{"version":"3.0","build":5}`.
