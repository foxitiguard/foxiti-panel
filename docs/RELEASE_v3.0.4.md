# foxitiPanel v3.0.4 (build 4) — Release Notes

_Released 2026-08-23._

_Updated 2026-08-24 with native AlmaLinux 10 support._

foxitiPanel v3.0.4 improves installation and upgrade reliability, completes
...
- Uses foxitiPanel's integrated webmail exclusively on fresh installations while
...
- Restores phpMyAdmin auto-login with a short-lived, one-time handoff from the
  authenticated foxitiPanel session (#1904).
...
- Adds native AlmaLinux 10 installation and upgrade support using the matched
  OpenLiteSpeed 2.5.2, `foxitipanel_ols.so` 2.7.6, and ModSecurity 2.5.2 EL10
  release set.
...
2. Confirm `https://foxitipanel.net/version.txt` is exactly
   `{"version":"3.0","build":4}` with no trailing newline.
3. Publish the `v3.0.4` changelog entry on the foxitiPanel changelog page.
