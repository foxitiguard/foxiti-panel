...
def versionManagement(request):
    ...
    u = "https://api.github.com/repos/usmannasir/foxitipanel/commits?sha=v%s.%s" % (latestVersion, latestBuild)
    logging.writeToFile(u)
    r = requests.get(u)
    latestcomit = r.json()[0]['sha']

    command = "git -C /usr/local/FoxitiCP/ rev-parse HEAD"
...
@ensure_csrf_cookie
def upgrade_foxitipanel(request):
    if request.method == 'POST':
        try:
            upgrade_command = 'sh <(curl https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/preUpgrade.sh || wget -O - https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/preUpgrade.sh)'
            result = subprocess.run(upgrade_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                    universal_newlines=True)

            if result.returncode == 0:
                response_data = {'success': True, 'message': 'foxitiPanel upgrade completed successfully.'}
            else:
                response_data = {'success': False,
                                 'message': 'foxitiPanel upgrade failed. Error output: ' + result.stderr}
...
def versionManagment(request):
    ...
    u = "https://api.github.com/repos/usmannasir/foxitipanel/commits?sha=v%s.%s" % (latestVersion, latestBuild)
    logging.FoxitiCPLogFileWriter.writeToFile(u)
    r = requests.get(u)
    latestcomit = r.json()[0]['sha']
...
def design(request):
    ### Load Custom CSS
    try:
        from baseTemplate.models import foxitiPanelCosmetic
    ...
    sha_url = "https://api.github.com/repos/usmannasir/foxitiPanel-Themes/commits"

    sha_res = requests.get(sha_url)

    sha = sha_res.json()[0]['sha']

    l = "https://api.github.com/repos/usmannasir/foxitiPanel-Themes/git/trees/%s" % sha
    fres = requests.get(l)
...
def getthemedata(request):
    try:
        ...
        # logging.FoxitiCPLogFileWriter.writeToFile(str(data) + "  [themedata]")

        url = "https://raw.githubusercontent.com/usmannasir/foxitiPanel-Themes/main/%s/design.css" % data['Themename']
...
def buildServices(request):
    # In-panel landing for foxitiPanel development services (Android, iOS,
    # web and custom software). The full marketing page lives on
    # foxitipanel.net; this page introduces the offering and deep-links out.
...
def runonboarding(request):
    try:
        ...
        command = shlex.join([
            ...
            '/usr/local/FoxitiCP/plogical/virtualHostUtilities.py',
            'OnBoardingHostName',
...
def RestartfoxitiPanel(request):
    try:
        userID = request.session['userID']
        currentACL = ACLManager.loadedACL(userID)
...
def analyzeSSHSecurity(request):
    try:
        ...
        # Check if user has foxitiPanel addons
        if not ACLManager.CheckForPremFeature('all'):
            return HttpResponse(json.dumps({
                ...
                'addon_url': 'https://foxitipanel.net/foxitipanel-addons'
...
def blockIPAddress(request):
    ...
    try:
        ...
        if success:
            # Log the action
            import plogical.FoxitiCPLogFileWriter as logging
            logging.FoxitiCPLogFileWriter.writeToFile(f'IP address {ip_address} blocked via foxitiPanel dashboard by user {user_id}')
