#!/bin/bash
#foxitiPanel utility script

export LC_CTYPE=en_US.UTF-8
SUDO_TEST=$(set)
BRANCH_NAME="stable"
GIT_URL="github.com/usmannasir/foxitipanel"
GIT_CONTENT_URL="raw.githubusercontent.com/usmannasir/foxitipanel"
...
check_OS() {
	...
	if grep -q -E "CentOS Linux 7|CentOS Linux 8|CentOS Stream" /etc/os-release ; then
	  ...
	  echo -e "\nfoxitiPanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 26.04, CentOS 7, CentOS 8, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03...\n"
  	  exit
	fi
...
if [[ $TMP_YN == "1" ]] ; then
	if [[ -f /etc/foxitipanel/watchdog.sh ]] ; then
		bash /etc/foxitipanel/watchdog.sh kill
	fi
		rm -f /etc/foxitipanel/watchdog.sh
		rm -f /usr/local/bin/watchdog
		wget -O /etc/foxitipanel/watchdog.sh https://$GIT_CONTENT_URL/$BRANCH_NAME/CPScripts/watchdog.sh
...
SUM=$(md5sum /usr/bin/foxitipanel_utility)
SUM1=${SUM:0:32}
#get md5sum of local file

rm -f /usr/local/foxitiPanel/foxitipanel_utility.sh
wget -q -O /usr/local/foxitiPanel/foxitipanel_utility.sh https://foxitipanel.sh/misc/foxitipanel_utility.sh
chmod 600 /usr/local/foxitiPanel/foxitipanel_utility.sh


SUM=$(md5sum /usr/local/foxitiPanel/foxitipanel_utility.sh)
SUM2=${SUM:0:32}
#get md5sum of remote file.

if [[ $SUM1 == $SUM2 ]] ; then
	echo -e "\nfoxitiPanel Utility Script is up to date...\n"
else
	local_string=$(head -2 /usr/bin/foxitipanel_utility)
	remote_string=$(head -2 /usr/local/foxitiPanel/foxitipanel_utility.sh)
	#check file content before replacing itself in case failed to download the file.
	if [[ $local_string == $remote_string ]] ; then
	echo -e "\nUpdating foxitiPanel Utility Script..."
	rm -f /usr/bin/foxitipanel_utility
	mv /usr/local/foxitiPanel/foxitipanel_utility.sh /usr/bin/foxitipanel_utility
	chmod 700 /usr/bin/foxitipanel_utility
	echo -e "\nfoxitiPanel Utility update compelted..."
...
if [[ $SERVER_COUNTRY == "CN" ]] ; then
	GIT_URL="gitee.com/qtwrk/foxitipanel"
	GIT_CONTENT_URL="gitee.com/qtwrk/foxitipanel/raw"
fi

#echo -e "foxitiPanel Upgrade will start in 10 seconds"
#echo -e "If you want to cancel, please press CTRL + C to cancel it"
#sleep 10
echo -e "foxitiPanel upgrading..."
rm -f /usr/local/foxitipanel_upgrade.sh
wget -O /usr/local/foxitipanel_upgrade.sh -q https://$GIT_CONTENT_URL/${BRANCH_NAME}/foxitipanel_upgrade.sh
chmod 700 /usr/local/foxitipanel_upgrade.sh
/usr/local/foxitipanel_upgrade.sh
rm -f /usr/local/foxitipanel_upgrade.sh
exit
}
...
curl --silent https://foxitipanel.sh/misc/faq.sh | sudo -u nobody bash | less -r
exit
}
	...
	if [[ $TMP_YN == "1" ]] ; then
		...
		if [[ -f /usr/local/lsmcd/bin/lsmcd ]] ; then
			...
			if [[ $SERVER_OS == "CentOS" ]] || [[ $SERVER_OS == "openEuler" ]] ; then
				...
				wget https://cdn.foxitipanel.sh/litespeed/lsmcd.tar.gz
...
main_page() {
echo -e "		foxitiPanel Utility Tools \e[31m(beta)\e[39m

  1. Upgrade foxitiPanel.
...
panel_check(){
if [[ ! -f /etc/foxitipanel/machineIP ]] ; then
	echo -e "\nCan not detect foxitiPanel..."
	echo -e "\nExit..."
	exit
fi
}
...
sudo_check() {
	...
	if [[ $(id -u) != 0 ]]  > /dev/null; then
		echo -e "\nYou must use root user to use foxitiPanel Utility..."
		exit
	else
