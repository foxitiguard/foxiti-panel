# Firewall Blocking Feature for foxitiPanel

## Overview

This feature adds a convenient "Block IP" button directly in the foxitiPanel dashboard's SSH Security Analysis section, allowing administrators to quickly block malicious IP addresses without needing to access SSH or manually run firewall commands.
...
- **Admin-Only Access**: Restricted to administrators with foxitiPanel addons
...
- **File**: `foxitipanel/baseTemplate/views.py`
- **Method**: POST
- **Authentication**: Admin-only with foxitiPanel addons
- **Functionality**:
...
#### 2. URL Configuration
- **File**: `foxitipanel/baseTemplate/urls.py`
- **Route**: `re_path(r'^blockIPAddress$', views.blockIPAddress, name='blockIPAddress')`

### Frontend Changes

#### 1. Template Updates
- **File**: `foxitipanel/baseTemplate/templates/baseTemplate/homePage.html`
- **Changes**:
...
- **File**: `foxitipanel/baseTemplate/static/baseTemplate/custom-js/system-status.js`
- **Features**:
...
### Prerequisites
1. foxitiPanel with admin privileges
2. foxitiPanel addons enabled
3. Active firewalld service

### How to Use
1. Navigate to **Dashboard** in foxitiPanel
...
2. **Premium Feature**: Requires foxitiPanel addons
...
1. **"Premium feature required" error**
   - Ensure foxitiPanel addons are enabled
...
Check foxitiPanel logs for detailed error information:
- `/usr/local/FoxitiCP/logs/foxitipanel.log`
- Firewalld logs: `journalctl -u firewalld`

## Support

For issues or questions regarding this feature:
1. Check foxitiPanel documentation
2. Review firewall configuration
3. Check system logs for detailed error messages
4. Contact foxitiPanel support if needed

---

**Note**: This feature enhances foxitiPanel's security capabilities by providing a streamlined way to block malicious IP addresses directly from the web interface, improving the overall user experience for server administrators.
