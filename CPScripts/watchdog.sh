...
show_help() {
	echo -e "\nrun command: \e[31mnohup bash /etc/foxitipanel/watchdog.sh SERVICE_NAME >/dev/null 2>&1 &\e[39m"
	...
	echo -e "\nto exit watchdog , run command \e[31mbash /etc/foxitipanel/watchdog.sh kill\e[39m"
	echo -e "\n\nplease also create \e[31m/etc/foxitipanel/watchdog.flag\e[39m file with following format:"
...
check_service() {
	systemctl status $NAME 2>&1>/dev/null
		if [[ $? == "0" ]] ; then
			...
			if [ -f /etc/foxitipanel/watchdog.flag ] ; then
			flag="/etc/foxitipanel/watchdog.flag"
			LINE3=$(awk 'NR==3' $flag)
			LINE2=$(awk 'NR==2' $flag)
			LINE1=$(awk 'NR==1' $flag)
...
while [ true = true ]
	do
		if [[ $NAME == "pdns" ]] ; then
			if [ -f /home/foxitipanel/powerdns ] ; then
				check_service
			fi
		elif [[ $NAME == "postfix" ]] ; then
			if  [ -f /home/foxitipanel/postfix ] ; then
				check_service
			fi
		elif [[ $name == "pure-ftpd" ]] || [[ $name == "pure-ftpd-mysql" ]] ; then
			if [ -f /home/foxitipanel/pureftpd ] ; then
