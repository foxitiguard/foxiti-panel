...
rm -f /root/.ssh/foxitipanel_migration_key.pub
ssh-keygen -t rsa -N "" -f /root/.ssh/foxitipanel_migration_key
if [[ -f /root/.ssh/authorized_keys ]] ; then
  ...
  if [[ $string != "ssh" ]] ; then
    ...
    cat /root/.ssh/foxitipanel_migration_key.pub > /root/.ssh/authorized_keys
  else
    cat /root/.ssh/foxitipanel_migration_key.pub >> /root/.ssh/authorized_keys
  fi
else
