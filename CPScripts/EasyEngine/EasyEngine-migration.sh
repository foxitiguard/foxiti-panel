...
#EasyEngine to foxitiPanel migration script
...
ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key "$sudoer wget -q -O /root/header.sh https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/CPScripts/EasyEngine/header.sh ; $sudoer bash /root/header.sh ${domains[$i]}"
fi
}

fix_permission() {
ssh_v="ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key"
echo -e "\nget the user and group on remote foxitiPanel server...."
...
rsync --stats -av -e "ssh -o StrictHostKeyChecking=no -p $server_port -i /root/.ssh/foxitipanel_migration_key" $cert_file root@$server_ip:/etc/letsencrypt/live/${domains[$i]}/fullchain.pem
  if [[ $? == "0" ]] ; then
...
rsync --stats -av -e "ssh -o StrictHostKeyChecking=no -p $server_port -i /root/.ssh/foxitipanel_migration_key" $key_file root@$server_ip:/etc/letsencrypt/live/${domains[$i]}/privkey.pem
    if [[ $? == "0" ]] ; then
      echo -e "\nkey file has been succesfully transferred to foxitiPanel server...\n"
    else
...
show_foxitipanel_site() {
  echo -e "\nchecking current websites on remote foxitiPanel server..."
  ssh_v="ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key"
  $ssh_v "foxitipanel listWebsitesPretty"
}

create_database() {
  echo -e "\nstarting database creation on remote foxitiPanel server..."
  ssh_v="ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key"

  check_string=$(${ssh_v} "foxitipanel createDatabase --databaseWebsite  ${domains[$i]} --dbName $WPDBNAME --dbUsername $WPDBUSER --dbPassword $WPDBPASS")
  ...
  if echo $check_string | grep -q "OK" ; then
    echo -e "\nstarting  database import on remote foxitiPanel..."
...
ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key "$sudoer wget -q -O /root/key.sh https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/CPScripts/EasyEngine/key.sh ; $sudoer bash /root/key.sh disable"
rm -f /root/.ssh/foxitipanel_migration_key
rm -rf /opt/easyengine/tmp
echo -e "\nclean up successful..."
}

create_site_foxitipanel() {
ssh_v="ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key"
echo -e "\nstarting to create ${domains[$i]} on remote foxitiPanel server..."
echo -e "\nyou may see error message on acme.sh but this is normal as actual DNS is not pointed to remote server.\n\n\n"
check_string=$(${ssh_v} "foxitipanel createWebsite --package Default --owner admin --domainName ${domains[$i]} --email admin@${domains[$i]} --php 7.4 --ssl 1")
  if echo $check_string | grep -q "None" ; then
...
if [[ -f /opt/easyengine/sites/${domains[$i]}/app/wp-config.php ]] ; then
  ...
  rsync --stats -av --chown=${owner_user}:${owner_group} -e "ssh -o StrictHostKeyChecking=no -p $server_port -i /root/.ssh/foxitipanel_migration_key" /opt/easyengine/sites/${domains[$i]}/app/wp-config.php root@$server_ip:/home/${domains[$i]}/public_html/wp-config.php
  if [[ $? == "0" ]] ; then
...
rsync --stats -av --chown=${owner_user}:${owner_group} -e "ssh -o StrictHostKeyChecking=no -p $server_port -i /root/.ssh/foxitipanel_migration_key" /opt/easyengine/sites/${domains[$i]}/app/htdocs/ root@$server_ip:/home/${domains[$i]}/public_html/
  if [[ $? == "0" ]] ; then
...
rsync --stats -av -e "ssh -o StrictHostKeyChecking=no -p $server_port -i /root/.ssh/foxitipanel_migration_key" $OUTPUT/$database_name root@$server_ip:/home/${domains[$i]}/$database_name
  if [[ $? == "0" ]] ; then
...
fetch_foxitipanel_key() {
if [[ ! -d /root/.ssh ]] ; then
  mkdir /root/.ssh
  chmod 700 /root/.ssh
fi
echo -e "\nPlease input your foxitiPanel server address"
...
echo -e "\nPlease input your foxitiPanel server SSH port"
...
if grep -q "PRIVATE KEY" /root/.ssh/foxitipanel_migration_key 2>/dev/null ; then
  status=$(ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key echo ok 2>&1)
  if [[ $status == ok ]] ; then
    echo -e "\nvalid key detected..."
    return
  else
...
#check the input , if it's a file , consider it as key.
  ssh -o StrictHostKeyChecking=no $user_name@$server_ip -p$server_port -i $password "$sudoer wget -q -O /root/key.sh https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/CPScripts/EasyEngine/key.sh ; $sudoer bash /root/key.sh enable"
  if [[ $? == "0" ]] ; then
    ssh -o StrictHostKeyChecking=no $user_name@$server_ip -p$server_port -i $password "$sudoer cat /root/.ssh/foxitipanel_migration_key" > /root/.ssh/foxitipanel_migration_key
      if [[ $? == "0" ]] ; then
        chmod 400 /root/.ssh/foxitipanel_migration_key
        status=$(ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key echo ok 2>&1)
        if [[ $status == ok ]] ; then
          echo -e "\nvalid key detected..."
        else
  ...
  sshpass -p "${password}" ssh -o StrictHostKeyChecking=no $user_name@$server_ip -p$server_port "$sudoer wget -q -O /root/key.sh https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/CPScripts/EasyEngine/key.sh ; $sudoer bash /root/key.sh enable"
  if [[ $? == "0" ]] ; then
    sshpass -p "${password}" ssh -o StrictHostKeyChecking=no $user_name@$server_ip -p$server_port "$sudoer cat /root/.ssh/foxitipanel_migration_key" > /root/.ssh/foxitipanel_migration_key
    chmod 400 /root/.ssh/foxitipanel_migration_key
    status=$(ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key echo ok 2>&1)
    if [[ $status == ok ]] ; then
      echo -e "\nvalid key detected..."
...
echo -e "\nEasyEngine to foxitiPanel Migration Script"
echo -e "\nThis script will do:"
echo -e "\n1. Generate public key and private key for root user on remote foxitiPanel server."
echo -e "2. Find the Wordpress sites hosting on this EasyEngine server"
echo -e "3. Export the site's database and its SSL cert/key if available and trasnfer to remote foxitiPanel server."
echo -e "4. Create website with same domain on remote foxitiPanel server and its related database."
...
echo -e "8. Once the migration process is completed, previously generated key will be removed on remote foxitiPanel server."
echo -e "9. All the temporary generated files on this server will also be cleaned up."
echo -e "\nOnce migration is completed, you can use local host file to override the DNS record to test site on remote foxitiPanel server"
...
db_length_check() {
  ssh_v="ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key"
  output=$($ssh_v "$sudoer cat /usr/local/FoxitiCP/plogical/mysqlUtilities.py")
  if echo $output | grep -q "should be 16 at max" ; then
    echo -e "\nPlease upgrade your foxitiPanel to latest first..."
    clean_up
    exit
  fi
}
...
fetch_foxitipanel_key
#function to get foxitipanel server key so future SSH command won't require password input.
...
for (( i=0; i<${tLen}; i++ ));
  do
    ...
    #create a file to save variable to source in foxitipanel server to read it.
...
ssh -o StrictHostKeyChecking=no root@$server_ip -p$server_port -i /root/.ssh/foxitipanel_migration_key "$sudoer wget -q -O /root/ext.sh https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/CPScripts/EasyEngine/ext.sh ; $sudoer bash /root/ext.sh"
