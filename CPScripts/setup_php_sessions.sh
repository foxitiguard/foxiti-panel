...
## Objective Fix session issues on foxitiPanel and standardized session paths.
# Fixes #430
# https://github.com/usmannasir/foxitipanel/issues/430
...
if [[ ! -e /usr/local/FoxitiCP/bin/cleansessions ]]; then
	touch /usr/local/FoxitiCP/bin/cleansessions
	chmod +x /usr/local/FoxitiCP/bin/cleansessions
	cat >> /usr/local/FoxitiCP/bin/cleansessions <<"EOL"
#!/bin/bash
...
command="/usr/local/FoxitiCP/bin/cleansessions >/dev/null 2>&1"
job="09,39 * * * * $command"
cat <(grep -i -v "$command" <(crontab -l)) <(echo "$job") | crontab -

echo "Checking cleansessions file"
cat /usr/local/FoxitiCP/bin/cleansessions
