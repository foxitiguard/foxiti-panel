...
if [ -d "$DIR" ]; then
  ...
  echo "Please install SpamAssasin through the foxitiPanel interface before proceeding"
  exit
fi
...
if grep -q -E "CentOS Linux 7|CentOS Linux 8" /etc/os-release ; then
  ...
  echo -e "\nfoxitiPanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 26.04, CentOS 7, CentOS 8, AlmaLinux 8, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03...\n"
  exit
fi
...
mkdir /usr/local/FoxitiCP/public/mailwatch

cd /usr/local/FoxitiCP/public/mailwatch

git clone --depth=1 https://github.com/mailwatch/MailWatch.git --branch 1.2 --single-branch

mv /usr/local/FoxitiCP/public/mailwatch/MailWatch/* /usr/local/FoxitiCP/public/mailwatch/

PASSWORD=$(cat /etc/foxitipanel/mysqlPassword)
USER=root
DATABASE=mailscanner
ADMINPASS=$(cat /etc/foxitipanel/adminPass)

### Fix a bug in MailWatch SQL File

sed -i 's/char(512)/char(255)/g' /usr/local/FoxitiCP/public/mailwatch/create.sql

##

mysql -u${USER} -p${PASSWORD} <"/usr/local/FoxitiCP/public/mailwatch/create.sql"
...
cp /usr/local/FoxitiCP/public/mailwatch/mailscanner/conf.php.example /usr/local/FoxitiCP/public/mailwatch/mailscanner/conf.php

sed -i "s/^define('DB_USER',.*/define('DB_USER','root');/" /usr/local/FoxitiCP/public/mailwatch/mailscanner/conf.php
sed -i "s/^define('DB_PASS',.*/define('DB_PASS','${PASSWORD}');/" /usr/local/FoxitiCP/public/mailwatch/mailscanner/conf.php
sed -i "s/^define('MAILWATCH_HOME',.*/define(\'MAILWATCH_HOME\', \'\/usr\/local\/FoxitiCP\/public\/mailwatch\/mailscanner');/" /usr/local/FoxitiCP/public/mailwatch/mailscanner/conf.php
...
cp /usr/local/FoxitiCP/public/mailwatch/MailScanner_perl_scripts/MailWatchConf.pm /usr/share/MailScanner/perl/custom/
...
ln -s /usr/local/FoxitiCP/public/mailwatch/MailScanner_perl_scripts/MailWatch.pm /usr/share/MailScanner/perl/custom
ln -s /usr/local/FoxitiCP/public/mailwatch/MailScanner_perl_scripts/SQLBlackWhiteList.pm /usr/share/MailScanner/perl/custom
ln -s /usr/local/FoxitiCP/public/mailwatch/MailScanner_perl_scripts/SQLSpamSettings.pm /usr/share/MailScanner/perl/custom
sed -i "s/^\$pathToFunctions =.*/\$pathToFunctions = '\/usr\/local\/FoxitiCP\/public\/mailwatch\/mailscanner\/functions.php';/" /usr/local/FoxitiCP/public/mailwatch/upgrade.php

/usr/local/lsws/lsphp72/bin/php /usr/local/FoxitiCP/public/mailwatch/upgrade.php
systemctl enable mailscanner
systemctl restart mailscanner

IPADDRESS=$(cat /etc/foxitipanel/machineIP)
