    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'foxitipanelCleaner purgeLimitDomain ' + domainName + ' ' + str(operationVal)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'foxitipanelCleaner updateDomainLimit ' + domainName + ' ' + str(newLimit)
                cacheClient.handleCachePurgeRequest(command)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'foxitipanelCleaner purgeLimit ' + emailAddress + ' ' + str(operationVal)
                cacheClient.handleCachePurgeRequest(command)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'foxitipanelCleaner purgeLog ' + emailAddress + ' ' + str(operationVal)
                cacheClient.handleCachePurgeRequest(command)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'foxitipanelCleaner purgeLimitEmail ' + emailAddress + ' ' + str(monthlyLimit) + ' ' + str(
                    hourlyLimit)
                cacheClient.handleCachePurgeRequest(command)
        ...
        try:
            if request.method == 'POST':
                ...
                if installStatus.find("[200]") > -1:
                    ...
                    execPath = "export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/root/bin  && /usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/plogical/mailUtilities.py"
        ...
        try:
            if request.method == 'POST':
                ...
                if mailUtilities.checkIfSpamAssassinInstalled() == 1:
                    ...
                    # logging.FoxitiCPLogFileWriter.writeToFile(str(data))
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                tempConfigPath = "/home/foxitipanel/" + str(randint(1000, 9999))
                ...
                if mailUtilities.checkIfRspamdInstalled() == 1:
                    ...
                    for items in data:
                        ...
                        if items.find('action =') > -1:
                            tempData = items.split(' ')
                            # logging.FoxitiCPLogFileWriter.writeToFile(str(tempData) + "action")
                            try:
                                if len(tempData) > 4:
                                    a = tempData[4]
                                elif len(tempData) > 2:
                    ...
                    for i in postdata:
                        if (i.find('smtpd_milters=') > -1 or i.find('smtpd_milters =') > -1) and i.find('non_smtpd_milters') < 0:
                            ...
                            if len(split_data) > 1:
                                ...
                                if os.path.exists(ProcessUtilities.debugPath):
                                    logging.FoxitiCPLogFileWriter.writeToFile(f'smtpd_milters: {tempData}')
                        ...
                        if i.find('non_smtpd_milters=') > -1 or i.find('non_smtpd_milters =') > -1:
                            ...
                            if len(eq_split) > 1:
                                ...
                                if os.path.exists(ProcessUtilities.debugPath):
                                    logging.FoxitiCPLogFileWriter.writeToFile(f'non_smtpd_milters: {tempData}')
                    ...
                    for i in postdata:
                        if i.find('write_servers =') > -1:
                            tempData = i.split(' ')
                            # logging.FoxitiCPLogFileWriter.writeToFile(str(tempData) + "redis")
                            if len(tempData) > 2:
                                write = tempData[2]
                                i = write.rstrip(";")
                                j = i.split('"')
                                if len(j) > 1:
                                    write_servers = j[1]
                                # logging.FoxitiCPLogFileWriter.writeToFile(str(write_servers) + "write_servers")

                        if i.find('read_servers =') > -1:
                            tempData = i.split(' ')
                            # logging.FoxitiCPLogFileWriter.writeToFile(str(tempData) + "redis2")
                            if len(tempData) > 2:
                                read = tempData[2]
                                i = read.rstrip(";")
                                j = i.split('"')
                                if len(j) > 1:
                                    read_servers = j[1]
                                # logging.FoxitiCPLogFileWriter.writeToFile(str(read_servers) + "read_servers")
...
def unistallRspamd(request):
    try:
        logging.FoxitiCPLogFileWriter.writeToFile("unistallRspamd...1")
        userID = request.session['userID']
        currentACL = ACLManager.loadedACL(userID)
...
def ReadReport(request):
    try:
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile("Result....3:" + str(msg))
    except KeyError:
        return redirect(loadLoginPage)
...
def fixMailSSL(request):
    try:
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            logging.FoxitiCPLogFileWriter.writeToFile("Result....3:" + str(res.content))
        return res
    except KeyError:
        return redirect(loadLoginPage)
