...
class CloudManager:
    ...
    def fetchSSHKey(self, request):
        try:
            pubKey = os.path.join("/root", ".ssh", 'foxitipanel.pub')
    ...
    def forceRunAWSBackup(self, request):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/IncScheduler.py forceRunAWSBackup --planName %s" % (
                self.data['planName'])
    ...
    def ResetFTPConfigurations(self):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/ftp/ftpManager.py"
    ...
    def ResetDNSConfigurations(self):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/dns/dnsManager.py"
    ...
    def SubmitCloudBackup(self):
        try:
            ...
            import time
            BackupPath = '/home/foxitipanel/backups/%s/backup-' % (self.data['domain']) + self.data[
    ...
    def getCurrentCloudBackups(self):
        try:
            ...
            backupsPath = '/home/foxitipanel/backups/%s/' % (backupDomain)
            try:
    ...
    def deleteCloudBackup(self):
        try:
            ...
            backupsPathComplete = '/home/foxitipanel/backups/%s/%s' % (backupDomain, backupFile)
    ...
    def DeployWordPress(self):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/plogical/applicationInstaller.py"
    ...
    def AutoLogin(self):
        try:
            ...
            command = 'wp user create foxitipanel support@foxitipanel.cloud --role=administrator --user_pass="%s" --path=%s --skip-plugins --skip-themes' % (
                password, path)
            ProcessUtilities.executioner(command, website.externalApp)

            command = 'wp user update foxitipanel --user_pass="%s" --path=%s --skip-plugins --skip-themes' % (password,
                                                                                                             path)
            ProcessUtilities.executioner(command, website.externalApp)
    ...
    def SubmitPublicKey(self):
        try:
            ...
            try:
                BackupPath = '/home/foxitipanel/backups/%s' % (self.data['domain'])
    ...
    def SubmitfoxitiPanelUpgrade(self):
        try:
            if os.path.lexists('/etc/csf'):
                from foxitipanel_firewall_migration import CSF_UPGRADE_MESSAGE
            ...
            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/foxitiPanelUpgrade.py --branch %s --mail %s --dns %s --ftp %s" % (
                self.data['foxitiPanelBranch'], mail, dns, ftp)
    ...
    def DetachCluster(self):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function %s --type %s" % (
                'DetachCluster', type)
            ProcessUtilities.executioner(execPath)
    ...
    def SetupCluster(self):
        try:
            ...
            ClusterConfigPath = '/home/foxitipanel/cluster'
            writeToFile = open(ClusterConfigPath, 'w')
            writeToFile.write(json.dumps(self.data))
            writeToFile.close()

            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function SetupCluster --type %s" % (
            self.data['type'])
            ProcessUtilities.executioner(execPath)
    ...
    def BootMaster(self):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function BootMaster --type Master"
            ProcessUtilities.executioner(execPath)
    ...
    def BootChild(self):
        try:
            ...
            ChildData = '/home/foxitipanel/childaata'
            writeToFile = open(ChildData, 'w')
            writeToFile.write(json.dumps(self.data))
            writeToFile.close()

            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function BootChild --type Child"
            ProcessUtilities.executioner(execPath)
    ...
    def CreatePendingVirtualHosts(self):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function CreatePendingVirtualHosts --type Child"
            ProcessUtilities.popenExecutioner(execPath)
    ...
    def SwitchDNS(self):
        try:
            ...
            command = 'chown -R foxitipanel:foxitipanel /usr/local/FoxitiCP/lib/python3.8/site-packages/tldextract/.suffix_cache/'
            ProcessUtilities.executioner(command)
    ...
    def UptimeMonitor(self):
        try:
            ...
            CloudConfigPath = '/home/foxitipanel/cloud'
            writeToFile = open(CloudConfigPath, 'w')
            writeToFile.write(json.dumps(self.data))
            writeToFile.close()

            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function UptimeMonitor --type All"
            ProcessUtilities.executioner(execPath)
    ...
    def SyncToMaster(self):
        try:
            ...
            command = '/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function SyncToMaster --type Failover'
            ProcessUtilities.executioner(command)
    ...
    def _install_n8n_with_website(self, admin_pk, website_data, domain_name, n8n_username, n8n_password, n8n_email, n8n_port,
        ...
        try:
            ...
            # Step 2: Create database using native foxitiPanel process
            statusWriter.writeToFile('Creating database using foxitiPanel...,25')
    ...
    def _setupPostgreSQLForN8N(self):
        ...
        try:
            ...
            postgres_pass_file = "/etc/foxitipanel/postgresqlPassword"
            ...
            if not postgres_password:
                ...
                ProcessUtilities.executioner("mkdir -p /etc/foxitipanel", 'root', True)
...
'''
    ...
    def _perform_n8n_installation_OLD_REMOVE(self, domain_name, status_file_path):
        ...
        try:
            ...
            with open(status_file_path, 'a') as f:
                f.write('Inside try block, attempting to import FoxitiCPLogFileWriter...\n')

            from plogical.FoxitiCPLogFileWriter import FoxitiCPLogFileWriter as logging

            with open(status_file_path, 'a') as f:
                f.write('FoxitiCPLogFileWriter import successful\n')
            ...
            try:
                with open(status_file_path, 'a') as f:
                    f.write('Creating FoxitiCPLogFileWriter instance...\n')
                status_writer = logging(status_file_path, 'a')

                with open(status_file_path, 'a') as f:
                    f.write('FoxitiCPLogFileWriter instance created, testing writeToFile...\n')

                status_writer.writeToFile('FoxitiCPLogFileWriter initialized successfully')
                ...
                with open(status_file_path, 'a') as f:
                    f.write('All FoxitiCPLogFileWriter operations successful\n')

            except Exception as writer_error:
                with open(status_file_path, 'a') as f:
                    f.write(f'FoxitiCPLogFileWriter error: {str(writer_error)}\n')
                    f.write(f'Continuing without FoxitiCPLogFileWriter...\n')
        ...
        try:
            ...
            if not statusFile:
                ...
                if domain_identifier:
                    ...
                    import os
                    import glob
                    possible_files = glob.glob(f'/home/foxitipanel/*')
    ...
    def removeN8NInstallation(self):
        try:
            ...
            status_file_path = f'/home/foxitipanel/n8n_install_{domain_name.replace(".", "_")}_status'
            try:
