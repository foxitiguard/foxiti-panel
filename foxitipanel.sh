...
# Logging setup
LOG_DIR="/var/log/foxitipanel"
LOG_FILE="$LOG_DIR/foxitipanel_install_$(date +%Y%m%d_%H%M%S).log"
DEBUG_LOG_FILE="$LOG_DIR/foxitipanel_install_debug_$(date +%Y%m%d_%H%M%S).log"

# Create log directory if it doesn't exist
mkdir -p "$LOG_DIR" 2>/dev/null || {
    # If /var/log/foxitipanel cannot be created, use /tmp
    LOG_DIR="/tmp/foxitipanel_logs"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/foxitipanel_install_$(date +%Y%m%d_%H%M%S).log"
    DEBUG_LOG_FILE="$LOG_DIR/foxitipanel_install_debug_$(date +%Y%m%d_%H%M%S).log"
}
...
# Initialize logging
log_info "foxitiPanel installation started"
...
echo -e "Fetching latest data from foxitiPanel server...\n"
echo -e "This may take few seconds..."
log_info "Fetching latest data from foxitiPanel server"
...
# Interpreter the foxitiPanel/FoxitiCP virtualenvs are built on. Setup_Python_Runtime
...
setup_epel_repo() {
    case "$Server_OS_Version" in
        "7")
            rpm --import https://foxitipanel.sh/dl.fedoraproject.org/pub/epel/RPM-GPG-KEY-EPEL-7
            yum install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm
            Check_Return "yum repo" "no_exit"
            ;;
        "8")
            rpm --import https://foxitipanel.sh/dl.fedoraproject.org/pub/epel/RPM-GPG-KEY-EPEL-8
            yum install -y https://foxitipanel.sh/dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm
            Check_Return "yum repo" "no_exit"
            ;;
        "9")
            yum install -y https://foxitipanel.sh/dl.fedoraproject.org/pub/epel/epel-release-latest-9.noarch.rpm
            Check_Return "yum repo" "no_exit"
            ;;
        "10")
...
import_litespeed_gpg_key() {
  if [[ "$Server_OS_Version" != "10" ]]; then
    rpm --import https://foxitipanel.sh/rpms.litespeedtech.com/centos/RPM-GPG-KEY-litespeed
    return $?
  fi
...
Debug_Log() {
echo -e "\n${1}=${2}\n" >> "/var/log/foxitipanel_debug_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
}
...
curl --max-time 20 -d '{"ipAddress": "'"$Server_IP"'", "InstallfoxitiPanelStatus": "'"$1"'"}' -H "Content-Type: application/json" -X POST https://cloud.foxitipanel.net/servers/RecvData  >/dev/null 2>&1
}
...
log_info "Checking root privileges"
  ...
  if [[ $(id -u) != 0 ]] >/dev/null; then
    echo -e "\nYou must run on root user to install foxitiPanel...\n"
...
if [ -z "$XDG_CURRENT_DESKTOP" ]; then
    ...
    echo "$XDG_CURRENT_DESKTOP defined appears to be a desktop OS. Bailing as foxitiPanel is incompatible."
    echo -e "\nfoxitiPanel is supported on server OS types only. Such as Ubuntu 18.04 x86_64, Ubuntu 20.04 x86_64, Ubuntu 20.10 x86_64, Ubuntu 22.04 x86_64, Ubuntu 24.04 x86_64, Ubuntu 24.04.3 x86_64, Ubuntu 26.04 x86_64, CentOS 8.x, AlmaLinux 8.x, AlmaLinux 9.x, AlmaLinux 10.x and CloudLinux 7.x...\n"
    exit
fi
...
if grep -q -E "CentOS Linux 7|CentOS Linux 8|CentOS Stream" /etc/os-release ; then
  ...
  echo -e "\nfoxitiPanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 24.04.3, Ubuntu 26.04, CentOS 7, CentOS 8, CentOS 9, RHEL 8, RHEL 9, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03...\n"
  Debug_Log2 "foxitiPanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 24.04.3, Ubuntu 26.04, CentOS 7, CentOS 8, CentOS 9, RHEL 8, RHEL 9, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03... [404]"
  exit
fi
...
#  echo -e "\nLXC detected..."
#  echo -e "foxitiPanel does not support LXC"
#  echo -e "Exiting..."
#  Debug_Log2 "foxitiPanel does not support LXC.. [404]"
#  exit
#fi
#remove per https://github.com/usmannasir/foxitipanel/issues/589
...
Show_Help() {
echo -e "\nfoxitiPanel Installer Script Help\n"
echo -e "\nUsage: sh <(curl foxitipanel.sh) --argument"
echo -e "\n\e[31m-v\e[39m or \e[31m--version\e[39m : choose to install foxitiPanel OpenLiteSpeed or foxitiPanel Enterprise, available options are \e[31mols\e[39m , \e[31mTRIAL\e[39m and \e[31mSERIAL_NUMBER\e[39m, default ols"
...
echo -e "\nExample:"
echo -e "\nsh <(curl foxitipanel.sh) -v ols -p r or ./foxitipanel.sh --version ols --password random"
echo -e "\nThis will install foxitiPanel OpenLiteSpeed and randomly generate the password."
echo -e "\nsh <(curl foxitipanel.sh) -v LICENSE_KEY -a -p my_pass_word"
...
Interactive_Mode() {
echo -e "		foxitiPanel Installer v$Panel_Version.$Panel_Build

1. Install foxitiPanel.
...
1. Install foxitiPanel with \e[31mOpenLiteSpeed\e[39m.
...
echo -e "\nInstall Full service for foxitiPanel? This will include PowerDNS, Postfix and Pure-FTPd."
echo -e ""
...
if [[ $(expr "x$Tmp_Input" : 'x[Yy]') -gt 1 ]] || [[ $Tmp_Input = "" ]]; then
  ...
  ### Ask if you want to set up this foxitiPanel with remote MySQL
...
if [[ $Server_OS = "CentOS" ]] ; then
  ...
  if [[ "$Server_OS_Version" = "8" ]]; then
    rpm --import https://foxitipanel.sh/www.centos.org/keys/RPM-GPG-KEY-CentOS-Official
...
EOF
    ...
    yum install --nogpg -y https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/gf-release-latest.gf.el7.noarch.rpm
      Check_Return "yum repo" "no_exit"

    rpm -ivh https://foxitipanel.sh/repo.iotti.biz/CentOS/7/noarch/lux-release-7-1.noarch.rpm
      Check_Return "yum repo" "no_exit"
...
[global]
index-url=https://foxitipanel.sh/pip-repo/pypi/simple/
EOF
#default to self-host pip for CN

if [[ "$Server_Provider" = "Alibaba Cloud" ]] ; then
sed -i 's|https://foxitipanel.sh/pip-repo/pypi/simple/|http://mirrors.cloud.aliyuncs.com/pypi/simple/|g' /root/.pip/pip.conf
echo "trusted-host = mirrors.cloud.aliyuncs.com" >> /root/.pip/pip.conf
fi

if [[ "$Server_Provider" = "Tencent Cloud" ]] ; then
sed -i 's|https://foxitipanel.sh/pip-repo/pypi/simple/|https://mirrors.cloud.tencent.com/pypi/simple/|g' /root/.pip/pip.conf
fi
...
#  sed -i 's|http://mirrors.tencentyun.com/ubuntu/|https://foxitipanel.sh/us.archive.ubuntu.com/ubuntu/|g' /etc/apt/sources.list
...
# Resolve the interpreter the foxitiPanel/FoxitiCP virtualenvs are built on.
#
...
# Ubuntu's own tooling, and only foxitiPanel's venvs point at 3.12.
Setup_Python_Runtime() {
  ...
  if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "26" ]] ; then
    ...
    if [[ -x /usr/bin/python3.12 ]] ; then
      ...
      echo -e "Using $foxitiPanel_Python for the foxitiPanel virtualenv"
    else
...
echo -e "Creating foxitiPanel virtual environment..."

# First ensure the directory exists
mkdir -p /usr/local/foxitiPanel

if [[ "$Server_OS" = "Ubuntu" ]] && ([[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]] || [[ "$Server_OS_Version" = "26" ]]) ; then
  echo -e "Ubuntu 22.04/24.04/26.04 detected, using $foxitiPanel_Python -m venv..."
  if "$foxitiPanel_Python" -m venv /usr/local/foxitiPanel 2>&1; then
    echo -e "Virtual environment created successfully"
  else
    echo -e "$foxitiPanel_Python -m venv failed, trying virtualenv..."
    ...
    virtualenv -p "$foxitiPanel_Python" /usr/local/foxitiPanel
  fi
else
  virtualenv -p /usr/bin/python3 /usr/local/foxitiPanel
fi

# Verify virtual environment was created
if [[ ! -f /usr/local/foxitiPanel/bin/activate ]]; then
...
if [ "$Server_OS" = "Ubuntu" ]; then
  ...
  # shellcheck disable=SC1091
  source /usr/local/foxitiPanel/bin/activate
fi
...
Debug_Log2 "Getting foxitiPanel code..,4"

Retry_Command "git clone ${Git_Clone_URL}"
  Check_Return "git clone ${Git_Clone_URL}"

echo -e "\nfoxitiPanel source code downloaded...\n"

cd foxitipanel || exit
git checkout "$Branch_Name"
  Check_Return "git checkout"
cd - || exit
cp -r foxitipanel /usr/local/foxitipanel
cd foxitipanel/install || exit
...
if [[ "$Server_OS" = "CentOS" ]] ; then
  ...
  #disable SELinux
    ...
    if [[ "$Server_OS_Version" = "8" ]] ; then
        ...
        if [[ "$Server_Country" = "CN" ]] ; then
          sed -i 's|rpm -Uvh http://rpms.litespeedtech.com/centos/litespeed-repo-1.1-1.el8.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://foxitipanel.sh/litespeed/litespeed_cn.repo|g' install.py
        else
          sed -i 's|rpm -Uvh http://rpms.litespeedtech.com/centos/litespeed-repo-1.1-1.el8.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://foxitipanel.sh/litespeed/litespeed.repo|g' install.py
        fi
      fi
    fi
...
if ! grep -q "pid_max" /etc/rc.local 2>/dev/null ; then
  ...
  *                hard    nproc           65535
  ...
  if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "26" ]] ; then
    ...
    rm -f /etc/systemd/resolved.conf.d/foxitipanel.conf
...
EOF
  ...
  for j in {1..20}; do
    sleep 0.5
    if getent ahostsv4 foxitipanel.sh >/dev/null 2>&1 || nslookup foxitipanel.sh >/dev/null 2>&1; then
      DNS_Ready="Yes"
      break
    fi
  done
  ...
  if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "26" ]] ; then
    ...
    cat > /etc/apt/apt.conf.d/80-foxitipanel-retries <<EOF
Acquire::Retries "5";
EOF
...
Line1="$(grep -n "f.write('nameserver 8.8.8.8')" installfoxitiPanel.py | head -n 1 | cut -d: -f1)"

# Only modify the file if the pattern was found
if [[ -n "$Line1" ]] && [[ "$Line1" =~ ^[0-9]+$ ]]; then
    sed -i "${Line1}i\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ subprocess.call\(command, shell=True)" installfoxitiPanel.py
    sed -i "${Line1}i\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ command = 'cat /etc/resolv.conf-tmp > /etc/resolv.conf'" installfoxitiPanel.py
else
    echo "Warning: Could not find 'nameserver 8.8.8.8' pattern in installfoxitiPanel.py - skipping resolv.conf modification"
fi
...
if [ -f /root/foxitipanel-tmp ]; then
  rm -rf /root/foxitipanel-tmp
fi

mkdir /root/foxitipanel-tmp
cd /root/foxitipanel-tmp || exit

Retry_Command "wget https://foxitipanel.sh/www.litespeedtech.com/packages/${LSWS_Stable_Version:0:1}.0/lsws-$LSWS_Stable_Version-ent-x86_64-linux.tar.gz"
tar xzvf "lsws-$LSWS_Stable_Version-ent-x86_64-linux.tar.gz" >/dev/null
cd "/root/foxitipanel-tmp/lsws-$LSWS_Stable_Version/conf"  || exit
if [[ "$License_Key" = "Trial" ]]; then
  Retry_Command "wget -q https://foxitipanel.sh/license.litespeedtech.com/reseller/trial.key"
  # Update the serial number handling to use trial key
  sed -i "s|writeSerial = open('lsws-[0-9.]\+/serial.no', 'w')|command = 'wget -q --output-document=./lsws-$LSWS_Stable_Version/trial.key https://foxitipanel.sh/license.litespeedtech.com/reseller/trial.key'|g" "$Current_Dir/installfoxitiPanel.py"
  sed -i 's|writeSerial.writelines(self.serial)|subprocess.call(command, shell=True)|g' "$Current_Dir/installfoxitiPanel.py"
  sed -i 's|writeSerial.close()||g' "$Current_Dir/installfoxitiPanel.py"
else
  echo "$License_Key" > serial.no
fi

cd "/root/foxitipanel-tmp/lsws-$LSWS_Stable_Version/bin"  || exit
...
if [[ "$Server_OS" = "Ubuntu" ]] ; then
  sed -i 's|wget http://rpms.litespeedtech.com/debian/|wget https://foxitipanel.sh/litespeed/|g' install.py
  sed -i 's|https://repo.dovecot.org/|https://foxitipanel.sh/repo.dovecot.org/|g' install.py
fi
  #replace litespeed repo on ubuntu 18/20

if [[ "$Server_OS" = "CentOS" ]] ; then
  sed -i 's|rpm -ivh http://rpms.litespeedtech.com/centos/litespeed-repo-1.2-1.el7.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://foxitipanel.sh/litespeed/litespeed_cn.repo|g' install.py
  sed -i 's|rpm -Uvh http://rpms.litespeedtech.com/centos/litespeed-repo-1.1-1.el8.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://foxitipanel.sh/litespeed/litespeed_cn.repo|g' install.py
  sed -i 's|https://mirror.ghettoforge.org/distributions|https://foxitipanel.sh/mirror.ghettoforge.net/distributions|g' install.py
  ...
  sed -i 's|dnf --nogpg install -y https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/gf-release-latest.gf.el8.noarch.rpm|echo gf8|g' install.py

  Retry_Command "dnf --nogpg install -y https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/gf-release-latest.gf.el8.noarch.rpm"
  sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/gf/\$basearch/mirrorlist|baseurl=https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/el/8/gf/x86_64/|g" /etc/yum.repos.d/gf.repo
  sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/plus/\$basearch/mirrorlist|baseurl=https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/el/8/plus/x86_64/|g" /etc/yum.repos.d/gf.repo
  ...
  if [[ "$Server_OS_Version" = "9" ]] || [[ "$Server_OS_Version" = "10" ]] ; then
    ...
    sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/gf/\$basearch/mirrorlist|baseurl=https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/el/9/gf/x86_64/|g" /etc/yum.repos.d/gf.repo
    sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/plus/\$basearch/mirrorlist|baseurl=https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/el/9/plus/x86_64/|g" /etc/yum.repos.d/gf.repo
  fi
fi

sed -i "s|https://www.litespeedtech.com/|https://foxitipanel.sh/www.litespeedtech.com/|g" installfoxitiPanel.py
...
sed -i 's|http://www.litespeedtech.com|https://foxitipanel.sh/www.litespeedtech.com|g' install.py
sed -i "s|rep.foxitipanel.net|foxitipanel.sh/rep.foxitipanel.net|g" installfoxitiPanel.py
sed -i "s|rep.foxitipanel.net|foxitipanel.sh/rep.foxitipanel.net|g" install.py
...
sed -i 's|$PROJECT/archive/$BRANCH.tar.gz|https://foxitipanel.sh/codeload.github.com/acmesh-official/acme.sh/tar.gz/master|g' /root/.acme.sh/acme.sh
...
log_info "Starting main foxitiPanel installation"
if [[ -d /usr/local/FoxitiCP ]] ; then
  echo -e "\n foxitiPanel already installed, exiting..."
  Debug_Log2 "foxitiPanel already installed, exiting... [404]"
  exit
fi
...
if [[ $Server_Edition = "Enterprise" ]] ; then
  ...
  sed -i "s|lsws-5.4.2|lsws-$LSWS_Stable_Version|g" installfoxitiPanel.py
  sed -i "s|lsws-5.3.5|lsws-$LSWS_Stable_Version|g" installfoxitiPanel.py
  sed -i "s|lsws-6.0|lsws-$LSWS_Stable_Version|g" installfoxitiPanel.py
  #this sed must be done after license validation

  Enterprise_Flag="--ent ent --serial "
fi

sed -i 's|git clone https://github.com/usmannasir/foxitipanel|echo downloaded|g' install.py
sed -i 's|mirror.foxitipanel.net|foxitipanel.sh|g' install.py
...
if [[ "$Remote_MySQL" = "On" ]] ; then
FP_INSTALL_MYSQL_PASSWORD="$MySQL_Password" \
    /usr/local/foxitiPanel/bin/python install.py "${Final_Flags[@]}"
...
if grep "foxitiPanel installation successfully completed" /var/log/installLogs.txt >/dev/null; then
  echo -e "\nfoxitiPanel installation sucessfully completed...\n"
...
Post_Install_Addon_Mecached_LSMCD() {
  ...
  wget -O lsmcd-master.zip https://foxitipanel.sh/codeload.github.com/litespeedtech/lsmcd/zip/master
...
chmod +x /usr/local/FoxitiCP/CPScripts/setup_php_sessions.sh
bash /usr/local/FoxitiCP/CPScripts/setup_php_sessions.sh
...
Current_Dir="$(pwd)"
rm -f /usr/local/lsws/foxitipanel-tmp
mkdir /usr/local/lsws/foxitipanel-tmp
cd /usr/local/lsws/foxitipanel-tmp || exit
wget -O timezonedb.tgz https://foxitipanel.sh/pecl.php.net/get/timezonedb
...
rm -rf /usr/local/lsws/foxitipanel-tmp
cd "$Current_Dir" || exit
...
echo "${Webadmin_Pass}" >/etc/foxitipanel/webadmin_passwd
...
if [[ "$Watchdog" = "On" ]]; then
  ...
  wget -O /etc/foxitipanel/watchdog.sh "${Git_Content_URL}/stable/CPScripts/watchdog.sh"
...
echo "###################################################################"
echo "                foxitiPanel Successfully Installed                  "
echo "                                                                   "
...
#echo "                                                                   "
echo "                                                                   "
echo -e "             Run \e[31mfoxitipanel help\e[39m to get FAQ info"
echo -e "             Run \e[31mfoxitipanel upgrade\e[39m to upgrade it to latest version."
echo -e "             Run \e[31mfoxitipanel utility\e[39m to access some handy tools ."
echo "                                                                   "
echo "              Website : https://www.foxitipanel.net                 "
echo "              Forums  : https://forums.foxitipanel.net              "
echo "              Wikipage: https://foxitipanel.net/KnowledgeBase/                "
echo "              Docs    : https://foxitipanel.net/docs/               "
echo "                                                                   "
echo -e "            Enjoy your accelerated Internet by                  "
echo -e "                foxitiPanel & $Word 				                     "
echo "###################################################################"
...
echo -e "\033[0;32mTCP: 8090\033[39m for foxitiPanel"
...
local cert_conf
cert_conf=$(mktemp /tmp/foxitipanel-cert-conf.XXXXXX)
cat <<EOF >"$cert_conf"
[req]
prompt=no
...
# When lscpd sets PYTHONHOME=/usr, lswsgi loads Django from system Python—not the FoxitiCP venv.
...
Install_FoxitiCP_Runtime_Python_Requirements() {
  ...
  if [[ -n "$req_hint" && -f "$req_hint" ]] && grep -q "Django==" "$req_hint" 2>/dev/null; then
    req_file="$req_hint"
  elif [[ -f /etc/foxitipanel/foxitipanel-requirments-runtime.txt ]] && grep -q "Django==" /etc/foxitipanel/foxitipanel-requirments-runtime.txt 2>/dev/null; then
    req_file="/etc/foxitipanel/foxitipanel-requirments-runtime.txt"
    ...
    local td="/tmp/foxitipanel-req-inst.$$"
    ...
    [[ -z "$req_file" ]] && wget -q -O "$td/s.txt" "https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/requirments.txt" 2>/dev/null \
...
echo -e "Creating FoxitiCP virtual environment..."
...
if [[ "$Server_OS" = "Ubuntu" ]] && ([[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]] || [[ "$Server_OS_Version" = "26" ]]) ; then
  echo -e "Ubuntu 22.04/24.04/26.04 detected, using $foxitiPanel_Python -m venv..."
  if "$foxitiPanel_Python" -m venv /usr/local/FoxitiCP 2>&1; then
    echo -e "Virtual environment created successfully"
  else
    echo -e "$foxitiPanel_Python -m venv failed, trying virtualenv..."
    # Ensure virtualenv is properly installed
    pip3 install --upgrade virtualenv
    virtualenv -p "$foxitiPanel_Python" /usr/local/FoxitiCP
  fi
  ...
  if python3 -m venv /usr/local/FoxitiCP 2>&1; then
    echo -e "Virtual environment created successfully"
  else
    ...
    virtualenv -p "$PYTHON_PATH" /usr/local/FoxitiCP
  fi
else
  virtualenv -p /usr/bin/python3 /usr/local/FoxitiCP
fi
...
if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "20" ]] ; then
  # shellcheck disable=SC1091
  . /usr/local/FoxitiCP/bin/activate
   Check_Return
else
  # shellcheck disable=SC1091
  source /usr/local/FoxitiCP/bin/activate
   Check_Return
...
if ! /usr/local/FoxitiCP/bin/python -c "import django" 2>/dev/null; then
...
if [[ -f /usr/local/requirments.txt ]]; then
  cp -f /usr/local/requirments.txt /etc/foxitipanel/foxitipanel-requirments-runtime.txt
fi
...
if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "26" ]] ; then
  ...
  cp "$foxitiPanel_Python" /usr/local/FoxitiCP/bin/python3
  if [[ -d /usr/lib/python3.12 ]] ; then
    cp -a /usr/lib/python3.12/. /usr/local/FoxitiCP/lib/python3.12/
  else
    echo -e "\nERROR: Python 3.12 standard library is missing on Ubuntu 26.04.\n"
    exit 1
  fi

  # lswsgi starts as the foxitipanel account. Keep the venv readable even when
  # the installer itself was launched with a restrictive umask.
  chmod 0644 /usr/local/FoxitiCP/pyvenv.cfg
  chmod -R a+rX /usr/local/FoxitiCP/lib /usr/local/FoxitiCP/lib64 2>/dev/null || true
elif [[ "$Server_OS" = "Ubuntu" ]] && ([[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]]) ; then
  # Ubuntu 24.04 ships with Python 3.12, but using 3.10 for compatibility with foxitiPanel
  cp /usr/bin/python3.10 /usr/local/FoxitiCP/bin/python3
else
  if [[ "$Server_OS_Version" = "9" ]] || [[ "$Server_OS_Version" = "10" ]] || [[ "$Server_OS_Version" = "8" ]] || [[ "$Server_OS_Version" = "20" ]] || [[ "$Server_OS_Version" = "24" ]]; then
    ...
    Install_FoxitiCP_Runtime_Python_Requirements "/etc/foxitipanel/foxitipanel-requirments-runtime.txt"
  else
...
chown -R foxitipanel:foxitipanel /usr/local/FoxitiCP/lib64 || true
}
...
# longer package it, so fall back to the PHP versions installed by foxitiPanel.
...
rm -rf /etc/profile.d/foxitipanel*
curl --silent -o /etc/profile.d/foxitipanel.sh https://foxitipanel.sh/?banner 2>/dev/null
chmod 700 /etc/profile.d/foxitipanel.sh
echo "$Admin_Pass" > /etc/foxitipanel/adminPass
chmod 600 /etc/foxitipanel/adminPass
/usr/local/foxitiPanel/bin/python /usr/local/FoxitiCP/plogical/adminPass.py --password "$Admin_Pass"
mkdir -p /etc/opendkim

echo '/usr/local/foxitiPanel/bin/python /usr/local/FoxitiCP/plogical/adminPass.py --password "$@"' > /usr/bin/adminPass
echo "systemctl restart lscpd" >> /usr/bin/adminPass
echo "echo \$@ > /etc/foxitipanel/adminPass" >> /usr/bin/adminPass
...
HostName=$(hostname --fqdn); [ -n "$(dig @1.1.1.1 +short "$HostName")" ]  &&  echo "$HostName resolves to valid IP. Setting up hostname SSL" && foxitipanel createWebsite --package Default --owner admin --domainName $(hostname --fqdn) --email root@localhost --php 7.4 && foxitipanel hostNameSSL --domainName $(hostname --fqdn)


}

Post_Install_CN_Replacement() {
sed -i 's|wp core download|wp core download https://foxitipanel.sh/wordpress.org/latest.tar.gz|g' /usr/local/FoxitiCP/plogical/applicationInstaller.py
sed -i 's|https://raw.githubusercontent.com/|https://foxitipanel.sh/raw.githubusercontent.com/|g' /usr/local/FoxitiCP/plogical/applicationInstaller.py
sed -i 's|wp plugin install litespeed-cache|wp plugin install  https://foxitipanel.sh/downloads.wordpress.org/plugin/litespeed-cache.zip|g' /usr/local/FoxitiCP/plogical/applicationInstaller.py
...
echo -e "\nInitializing...\n"
log_info "============================================="
log_info "foxitiPanel installation script started"
...
if [[ "$*" = *"--debug"* ]] ; then
  Debug="On"
  find /var/log -name 'foxitipanel_debug_*' -exec rm {} +
  Random_Log_Name=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 5)
  echo -e "$(date)" > "/var/log/foxitipanel_debug_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
  chmod 600 "/var/log/foxitipanel_debug_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
fi
