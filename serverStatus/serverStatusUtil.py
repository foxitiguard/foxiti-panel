...
class ServerStatusUtil(multi.Thread):
    ...
    lswsInstallStatusPath = '/home/foxitipanel/switchLSWSStatus'
    ...
    def getLatestLSWSVersion():
        ...
        try:
            ...
            if versions:
                ...
                logging.FoxitiCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,
    ...
    def installLiteSpeed(licenseKey, statusFile):
        try:
            ...
            if os.path.exists('/usr/local/FoxitiCP/lsws-6.0/'):
                shutil.rmtree('/usr/local/FoxitiCP/lsws-6.0')

            if os.path.exists(f'/usr/local/FoxitiCP/lsws-{lsws_version}/'):
                shutil.rmtree(f'/usr/local/FoxitiCP/lsws-{lsws_version}/')

            if ACLManager.ISARM():
                command = f'tar zxf lsws-{lsws_version}-ent-aarch64-linux.tar.gz -C /usr/local/FoxitiCP'
            else:
                command = f'tar zxf lsws-{lsws_version}-ent-x86_64-linux.tar.gz -C /usr/local/FoxitiCP'

            if ServerStatusUtil.executioner(command, statusFile) == 0:
                return 0

            if licenseKey == 'trial':
                command = f'wget -q --output-document=/usr/local/FoxitiCP/lsws-{lsws_version}/trial.key http://license.litespeedtech.com/reseller/trial.key'
                if ServerStatusUtil.executioner(command, statusFile) == 0:
                    return 0
            else:
                writeSerial = open(f'/usr/local/FoxitiCP/lsws-{lsws_version}/serial.no', 'w')
                writeSerial.writelines(licenseKey)
                writeSerial.close()

            shutil.copy('/usr/local/FoxitiCP/serverStatus/litespeed/install.sh', f'/usr/local/FoxitiCP/lsws-{lsws_version}/')
            shutil.copy('/usr/local/FoxitiCP/serverStatus/litespeed/functions.sh', f'/usr/local/FoxitiCP/lsws-{lsws_version}/')

            os.chdir(f'/usr/local/FoxitiCP/lsws-{lsws_version}/')
            ...
            shutil.copy('/usr/local/FoxitiCP/serverStatus/litespeed/httpd_config.xml', confPath)
            shutil.copy('/usr/local/FoxitiCP/serverStatus/litespeed/modsec.conf', confPath)
            shutil.copy('/usr/local/FoxitiCP/serverStatus/litespeed/httpd.conf', confPath)
            ...
            try:
                os.rmdir(f"/usr/local/FoxitiCP/lsws-{lsws_version}")
            except:
                pass
    ...
    def setupFileManager(statusFile):
        try:
            logging.FoxitiCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath, "Setting up Filemanager files..\n")
            ...
            shutil.copytree("/usr/local/FoxitiCP/serverStatus/litespeed/FileManager",fileManagerPath)
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,"Filemanager files are set!\n")
    ...
    def switchTOLSWS(licenseKey):
        try:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,"Starting conversion process..\n")
            logging.FoxitiCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,
            ...
            if ServerStatusUtil.installLiteSpeed(licenseKey, statusFile) == 0:
                logging.FoxitiCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath, "Failed to install LiteSpeed. [404]", 1)
            ...
            #     logging.FoxitiCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath, "Failed to set up File Manager. [404]", 1)
            #     ServerStatusUtil.recover()
            #     return 0
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,"Successfully switched to LITESPEED ENTERPRISE WEB SERVER. [200]\n", 1)
