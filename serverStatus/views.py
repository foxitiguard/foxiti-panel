...
def litespeedStatus(request):
    try:
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[litespeedStatus]")
            proc = httpProc(request, 'serverStatus/litespeedStatus.html',
                            {"processList": processList,
                             "liteSpeedVersionStatus": "For some reaons not able to load version details, see FoxitiCP main log file.",
    ...
    try:
        ...
        if reboot == 1:
            if installUtilities.reStartLiteSpeedSocket() == 1:
                ...
                status = {"reboot": 0, "shutdown": 0, "error_message": "Please see FoxitiCP main log file."}
        else:
        ...
        logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[stopOrRestartLitespeed]")
        return HttpResponse("Not Logged in as admin")


def cyberCPMainLogFile(request):
    proc = httpProc(request, 'serverStatus/foxiticpmainlogfile.html', None, 'admin')
    return proc.render()
...
def getFurtherDataFromLogFile(request):
    try:
        ...
        fewLinesOfLogFile = logging.FoxitiCPLogFileWriter.readLastNFiles(50, logging.FoxitiCPLogFileWriter.fileName)
        ...
        status = {"logstatus": 0,
                  "error": "Could not fetch data from log file, please see FoxitiCP main log file through command line."}
        logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[getFurtherDataFromLogFile]")
        return HttpResponse("Not Logged in as admin")
            ...
            if request.method == 'POST':
                ...
                if service not in ["lsws", "mysql", "pdns", "pure-ftpd", "docker"]:
                    ...
                    if not result or result[0] != 1:
                        final_dic = {
                            'serviceAction': 0,
                            'error_message': 'Service command failed. Please check the service status and foxitiPanel main log.'
                        }
...
def switchTOLSWS(request):
    try:
        ...
        execPath = "sudo /usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/serverStatus/serverStatusUtil.py"
...
def changeLicense(request):
    try:
        ...
        try:
            ...
            command = 'sudo chown -R foxitipanel:foxitipanel /usr/local/lsws/conf'
...
def topProcessesStatus(request):
    try:
        ...
        with open("/home/foxitipanel/top", "w") as outfile:
            subprocess.call("top -n1 -b", shell=True, stdout=outfile)

        data = open('/home/foxitipanel/top', 'r').readlines()
        ...
        data['foxitiPanelVersion'] = 'v%s.%s' % (VERSION, str(BUILD))
    ...
    try:
        ...
        if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
            ...
            if type == 'foxitiPanel':
                ...
                command = 'cat /usr/local/FoxitiCP/AllCPUbuntu.json'
            ...
            if type == 'installed':
                ...
                for items in packages:
                    if items == '':
                        ...
                        startForUpdate = startForUpdate + 1
            elif type == 'foxitiPanel':
                command = 'cat /usr/local/FoxitiCP/CPCent7repo.json'
                packages = json.loads(ProcessUtilities.outputExecutioner(command))

        ## make list of packages that need update

        # if os.path.exists(ProcessUtilities.debugPath):
        #    logging.FoxitiCPLogFileWriter.writeToFile('All packages: %s' % (str(packages)))
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            logging.FoxitiCPLogFileWriter.writeToFile('Final packages: %s' % (str(finalPackages)))
        ...
        for items in finalPackages:
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
                try:
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile('[ERROR] %s. [fetchPackages:773]' % (str(msg)))
            elif ProcessUtilities.decideDistro() == ProcessUtilities.centos or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                try:
                    ...
                    print(str(msg))
                    logging.FoxitiCPLogFileWriter.writeToFile('[ERROR] %s. [fetchPackages:839]' % (str(msg)))
...
def lockStatus(request):
    try:
        ...
        if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
            ...
            yumConfTmp = '/home/foxitipanel/yumTemp'
...
def foxitiPanelPort(request):
    port = ProcessUtilities.fetchCurrentPort()
    proc = httpProc(request, "serverStatus/changefoxitiPanelPort.html", {'port': port}, 'admin')
    return proc.render()
