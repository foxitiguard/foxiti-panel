...
# Test Plugin Installation Script for foxitiPanel
...
PLUGIN_DIR="/home/foxitipanel/plugins"
FOXITIPANEL_DIR="/usr/local/FoxitiCP"
GITHUB_REPO="https://github.com/foxitipanel/testPlugin.git"
TEMP_DIR="/tmp/foxitipanel_plugin_install"
...
# Function to check if foxitiPanel is installed
check_foxitipanel() {
    if [ ! -d "$FOXITIPANEL_DIR" ]; then
        print_error "foxitiPanel is not installed at $FOXITIPANEL_DIR"
        print_error "Please install foxitiPanel first: https://foxitipanel.net/docs/"
        exit 1
    fi
    
    # Check if foxitiPanel is running
    if ! $SERVICE_CMD is-active --quiet lscpd; then
        print_warning "foxitiPanel service (lscpd) is not running. Starting it..."
        $SERVICE_CMD start lscpd
    fi
    
    print_success "foxitiPanel installation verified"
}
...
create_plugin_directory() {
    ...
    # Create foxitiPanel plugin directory
    mkdir -p "$FOXITIPANEL_DIR/$PLUGIN_NAME"
    
    # Set proper permissions
    chown -R foxitipanel:foxitipanel "$PLUGIN_DIR" 2>/dev/null || chown -R root:root "$PLUGIN_DIR"
    chmod -R 755 "$PLUGIN_DIR"
    
    chown -R foxitipanel:foxitipanel "$FOXITIPANEL_DIR/$PLUGIN_NAME" 2>/dev/null || chown -R root:root "$FOXITIPANEL_DIR/$PLUGIN_NAME"
    chmod -R 755 "$FOXITIPANEL_DIR/$PLUGIN_NAME"
...
install_plugin_files() {
    ...
    # Copy plugin files
    cp -r "$TEMP_DIR"/* "$FOXITIPANEL_DIR/$PLUGIN_NAME/"
    
    # Create symlink
    ln -sf "$FOXITIPANEL_DIR/$PLUGIN_NAME" "$PLUGIN_DIR/$PLUGIN_NAME"
    ...
    chmod +x "$FOXITIPANEL_DIR/$PLUGIN_NAME/install.sh" 2>/dev/null || true
...
update_django_settings() {
    ...
    SETTINGS_FILE="$FOXITIPANEL_DIR/foxitipanel/settings.py"
...
update_urls() {
    ...
    URLS_FILE="$FOXITIPANEL_DIR/foxitipanel/urls.py"
...
restart_services() {
    print_status "Restarting foxitiPanel services..."
    ...
    case "$OS_NAME" in
        "ubuntu"|"debian")
            if $SERVICE_CMD is-active --quiet foxitipanel; then
                $SERVICE_CMD restart foxitipanel
                print_success "foxitipanel service restarted"
            fi
            ;;
        "almalinux"|"rocky"|"rhel"|"centos"|"cloudlinux")
            if $SERVICE_CMD is-active --quiet foxitipanel; then
...
verify_installation() {
    ...
    if [ ! -d "$FOXITIPANEL_DIR/$PLUGIN_NAME" ]; then
    ...
    if [ ! -f "$FOXITIPANEL_DIR/$PLUGIN_NAME/meta.xml" ]; then
...
display_summary() {
    ...
    echo "Installation Directory: $FOXITIPANEL_DIR/$PLUGIN_NAME"
    ...
    echo "✓ Official foxitiPanel Guide"
...
uninstall_plugin() {
    ...
    # Remove plugin files
    rm -rf "$FOXITIPANEL_DIR/$PLUGIN_NAME"
