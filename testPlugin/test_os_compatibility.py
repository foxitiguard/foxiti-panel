...
class OSCompatibilityTester:
    ...
    def run_all_tests(self):
        ...
        print("🔍 Testing OS Compatibility for foxitiPanel Test Plugin")
        print("=" * 60)
        ...
        # Test 8: foxitiPanel Integration
        self.test_foxitipanel_integration()
    ...
    def test_file_permissions(self):
        ...
        try:
            ...
            can_create_foxitipanel_dir = True
            ...
            try:
                os.makedirs(f"{foxitipanel_dir}/test", exist_ok=True)
                os.rmdir(f"{foxitipanel_dir}/test")
            except PermissionError:
                can_create_foxitipanel_dir = False
            
            self.test_results['file_permissions'] = {
                'status': 'PASS' if can_create_plugin_dir and can_create_foxitipanel_dir else 'WARN',
                'can_create_plugin_dir': can_create_plugin_dir,
                'can_create_foxitipanel_dir': can_create_foxitipanel_dir,
                'plugin_dir': plugin_dir,
                'foxitipanel_dir': foxitipanel_dir
            }
            
            print(f"   {'✅' if can_create_plugin_dir else '⚠️'} Plugin Directory: {plugin_dir}")
            print(f"   {'✅' if can_create_foxitipanel_dir else '⚠️'} foxitiPanel Directory: {foxitipanel_dir}")
    ...
    def test_foxitipanel_integration(self):
        """Test foxitiPanel integration"""
        ...
        try:
            ...
            foxitipanel_installed = os.path.exists(foxitipanel_dir)
            
            # Check if Django settings exist
            settings_file = f"{foxitipanel_dir}/foxitipanel/settings.py"
            settings_exist = os.path.exists(settings_file)
            
            # Check if URLs file exists
            urls_file = f"{foxitipanel_dir}/foxitipanel/urls.py"
            urls_exist = os.path.exists(urls_file)
            
            # Check if lscpd service exists
            lscpd_exists = os.path.exists("/usr/local/lscp/bin/lscpd")
            
            self.test_results['foxitipanel_integration'] = {
                'status': 'PASS' if foxitipanel_installed and settings_exist and urls_exist else 'WARN',
                'foxitipanel_installed': foxitipanel_installed,
                'settings_exist': settings_exist,
                'urls_exist': urls_exist,
                'lscpd_exists': lscpd_exists
            }
            
            print(f"   {'✅' if foxitipanel_installed else '⚠️'} foxitiPanel Installed: {foxitipanel_installed}")
