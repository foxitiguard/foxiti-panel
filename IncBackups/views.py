...
def add_destination(request):
    try:
        ...
        if data['type'].lower() == IncBackupProvider.SFTP.name.lower():
            ...
            python_path = Path('/usr/local/FoxitiCP/bin/python')
            ...
            if output.find('1,') > -1:
                ...
                tmp_file = '/home/foxitipanel/sshconfig'
                with open(tmp_file, 'w') as outfile:
                    if current_config.find('cat') == -1:
                        outfile.write(current_config)

                    content = "Host %s\n" \
                              "    IdentityFile ~/.ssh/foxitipanel\n" \
                              "    Port %s\n" % (ip_address, port)
...
def submit_backup_creation(request):
    try:
        ...
        temp_path = Path("/home/foxitipanel/") / str(randint(1000, 9999))
...
def get_backup_status(request):
    try:
        ...
        if os.path.exists(status):
            command = "cat " + status
            result = pu.outputExecutioner(command, 'foxitipanel')
...
def selectwebsiteRetorev2(request):
    try:
        ...
        #/home/foxitipanel.net/.config/rclone/rclone.conf
