...
class IncJobs(multi.Thread):
    ...
    def findRestorePath(self):
        ...
        if ProcessUtilities.decideDistro() == ProcessUtilities.centos or ProcessUtilities.decideDistro() == ProcessUtilities.cent8 \
            ...
            if self.jobid.type[:8] == 'database':
                self.restoreTarget = '/usr/local/FoxitiCP/tmp/'
    ...
    def getAWSData(self):
        ...
        path = '/home/foxitipanel/aws/%s' % (key)
    ...
    def restoreDatabase(self):
        try:
            ...
            if self.reconstruct == 'remote':
                ...
                command = 'chown foxitipanel:foxitipanel /home/foxitipanel'
                ...
                if mysqlUtilities.mysqlUtilities.restoreDatabaseBackup(self.path.split('/')[-1].rstrip('.sql'),
                                                                       '/usr/local/FoxitiCP/tmp', 'dummy', 'dummy') == 0:
                    raise BaseException
            else:
            ...
            try:
                if self.reconstruct == 'remote':
                    os.remove('/usr/local/FoxitiCP/tmp/%s' % (self.path.split('/')[-1]))
                else:
                    os.remove('/usr/local/FoxitiCP/tmp/%s.sql' % (self.jobid.type.split(':')[1]))
    ...
    def reconstructWithMeta(self):
        try:
            ...
            execPath = "nice -n 10 /usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/plogical/restoreMeta.py"
    ...
    def backupDatabases(self):
        try:
            ...
            for items in databases:
                ...
                try:
                    dbPath = '/usr/local/FoxitiCP/tmp/%s.sql' % (items.dbName)
...
""" % (password)
        ...
        sender = 'foxitipanel@%s' % (self.website.domain)
            ...
            if ProcessUtilities.outputExecutioner(command).find('restic is a backup program which') == -1:
                try:
                    ...
                    logging.statusWriter(self.statusPath,
                                         ...
                                         'guide -> https://foxitipanel.net/KnowledgeBase/. [5009]', 1)
                    pass
