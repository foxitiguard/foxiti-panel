# foxitiPanel Secure Installation Guide

## Overview

This document describes the secure installation process for foxitiPanel that eliminates hardcoded passwords and implements environment-based configuration.
...
python install/env_generator.py /usr/local/FoxitiCP
```
...
1. Check if `.env.backup` file exists:
   ```bash
   sudo cat /usr/local/FoxitiCP/.env.backup
   ```
...
```bash
cd /usr/local/FoxitiCP
sudo python install/env_generator.py /usr/local/FoxitiCP
```
...
| `DB_PASSWORD` | foxitiPanel DB password | Generated (24 chars) |
...
```bash
python install/env_generator.py /usr/local/FoxitiCP "your_root_password" "your_db_password"
```
...
For existing foxitiPanel installations with hardcoded passwords:

1. **Backup current configuration**:
   ```bash
   cp /usr/local/FoxitiCP/FoxitiCP/settings.py /usr/local/FoxitiCP/FoxitiCP/settings.py.backup
   ```
