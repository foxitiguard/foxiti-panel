...
#foxitiPanel installer script for CentOS 7, CentOS 8, CloudLinux 7, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, RockyLinux 8, Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 24.04.3, Ubuntu 26.04, openEuler 20.03 and openEuler 22.03
...
rm -f /var/log/foxitipanel_upgrade_debug.log
rm -f /var/log/installLogs.txt
rm -f /var/log/upgradeLogs.txt

# Initialize new debug log
echo -e "\n\n========================================" > /var/log/foxitipanel_upgrade_debug.log
echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Starting foxitiPanel Upgrade Script" >> /var/log/foxitipanel_upgrade_debug.log
echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Old log files have been cleared" >> /var/log/foxitipanel_upgrade_debug.log
echo -e "========================================\n" >> /var/log/foxitipanel_upgrade_debug.log

export LC_CTYPE=en_US.UTF-8
echo -e "\nFetching latest data from foxitiPanel server...\n"
...
Temp_Value=$(curl --silent --max-time 30 -4 https://foxitipanel.net/version.txt)
if ! parse_panel_version "$Temp_Value"; then
  echo -e "\nUnable to fetch a valid foxitiPanel version."
...
MySQL_Password=$(cat /etc/foxitipanel/mysqlPassword)


LSWS_Latest_URL="https://foxitipanel.sh/update.litespeedtech.com/ws/latest.php"
...
rm -rf /root/foxitipanel_upgrade_tmp
mkdir -p /root/foxitipanel_upgrade_tmp
cd /root/foxitipanel_upgrade_tmp || exit
}

Debug_Log() {
echo -e "\n${1}=${2}\n" >>  "/var/log/foxitipanel_debug_upgrade_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
}
...
echo -e "\nChecking root privileges..."
  ...
  if [[ $(id -u) != 0 ]] >/dev/null; then
    echo -e "\nYou must run as root user to install foxitiPanel...\n"
    echo -e "or run the following command: (do NOT miss the quotes)"
    echo -e "\e[31msudo su -c \"sh <(curl https://foxitipanel.sh || wget -O - https://foxitipanel.sh)\"\e[39m"
    exit 1
  else
...
Server_Country=$(curl --silent --max-time 10 -4 https://foxitipanel.sh/?country)
...
if grep -q -E "CentOS Linux 7|CentOS Linux 8|CentOS Stream" /etc/os-release ; then
  ...
  echo -e "\nfoxitiPanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 24.04.3, Ubuntu 26.04, CentOS 7, CentOS 8, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, RockyLinux 8, CloudLinux 7, CloudLinux 8, CloudLinux 9, openEuler 20.03, openEuler 22.03...\n"
  Debug_Log2 "foxitiPanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 24.04.3, Ubuntu 26.04, CentOS 7, CentOS 8, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, RockyLinux 8, CloudLinux 7, CloudLinux 8, CloudLinux 9, openEuler 20.03, openEuler 22.03... [404]"
  exit
fi
...
# Interpreter the foxitiPanel/FoxitiCP virtualenvs are built on.
#
...
foxitiPanel_Python="/usr/bin/python3"

if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "26" ]] ; then
  if [[ ! -x /usr/bin/python3.12 ]] ; then
    echo -e "Ubuntu 26.04 detected - installing Python 3.12 for the foxitiPanel virtualenv..."
  ...
  if [[ -x /usr/bin/python3.12 ]] ; then
    foxitiPanel_Python="/usr/bin/python3.12"
  else
    echo -e "\nERROR: Python 3.12 is required on Ubuntu 26.04 but could not be installed."
    echo -e "foxitiPanel cannot run on the system Python 3.14 with Django 4.2.\n"
...
if [[ $LAST_EXIT_CODE != "0" ]]; then
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ERROR: Command failed with exit code: $LAST_EXIT_CODE" | tee -a /var/log/foxitipanel_upgrade_debug.log
  if [[ -n "$1" ]] ; then
    echo -e "\n\n\n$1"
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Error message: $1" | tee -a /var/log/foxitipanel_upgrade_debug.log
  fi
  ...
  if [[ "$2" = "no_exit" ]] || [[ "$3" = "continue" ]]; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Continuing despite error..." | tee -a /var/log/foxitipanel_upgrade_debug.log
  else
    # Only exit for critical errors
    if [[ "$1" == *"Virtualenv creation failed"* ]] || [[ "$1" == *"Python upgrade.py"* ]]; then
      echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] FATAL: Critical error, exiting" | tee -a /var/log/foxitipanel_upgrade_debug.log
      exit $LAST_EXIT_CODE
    else
      echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Non-critical error, continuing..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    fi
  fi
else
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Command succeeded" | tee -a /var/log/foxitipanel_upgrade_debug.log
fi
}
# check command success or not

Regenerate_Cert() {
  cat <<EOF >/usr/local/FoxitiCP/cert_conf
[req]
prompt=no
distinguished_name=foxitipanel
[foxitipanel]
commonName = www.example.com
countryName = CP
localityName = foxitiPanel
organizationName = foxitiPanel
organizationalUnitName = foxitiPanel
stateOrProvinceName = CP
emailAddress = mail@example.com
name = foxitiPanel
surname = foxitiPanel
givenName = foxitiPanel
initials = CP
dnQualifier = foxitiPanel
[server_exts]
extendedKeyUsage = 1.3.6.1.5.5.7.3.1
EOF
  if [[ $1 == "8090" ]]; then
    openssl req -x509 -config /usr/local/FoxitiCP/cert_conf -extensions 'server_exts' -nodes -days 820 -newkey rsa:2048 -keyout /usr/local/lscp/conf/key.pem -out /usr/local/lscp/conf/cert.pem
  fi
  ...
  if [[ $1 == "7080" ]]; then
    ...
    openssl req -x509 -config /usr/local/FoxitiCP/cert_conf -extensions 'server_exts' -nodes -days 820 -newkey rsa:2048 -keyout $key_path -out $cert_path
  fi
  rm -f /usr/local/FoxitiCP/cert_conf
}
...
Pre_Upgrade_Setup_Git_URL() {
  if [[ $Server_Country != "CN" ]] ; then
    Git_User="usmannasir"
    Git_Content_URL="https://raw.githubusercontent.com/${Git_User}/foxitipanel"
    Git_Clone_URL="https://github.com/${Git_User}/foxitipanel.git"
  else
    Git_User="qtwrk"
    Git_Content_URL="https://gitee.com/${Git_User}/foxitipanel/raw"
    Git_Clone_URL="https://gitee.com/${Git_User}/foxitipanel.git"
  fi
...
echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Pre_Upgrade_Setup_Repository started for OS: $Server_OS" | tee -a /var/log/foxitipanel_upgrade_debug.log

if [[ "$Server_OS" = "CentOS" ]] ; then
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Setting up CentOS repositories..." | tee -a /var/log/foxitipanel_upgrade_debug.log
  rm -f /etc/yum.repos.d/foxitiPanel.repo
  rm -f /etc/yum.repos.d/litespeed.repo
  if [[ "$Server_Country" = "CN" ]] ; then
    curl -o /etc/yum.repos.d/litespeed.repo https://foxitipanel.sh/litespeed/litespeed_cn.repo
  else
    curl -o /etc/yum.repos.d/litespeed.repo https://foxitipanel.sh/litespeed/litespeed.repo
  fi
  yum clean all
  yum update -y
...
#all pre-upgrade operation for CentOS both 7/8
  ...
  if [[ "$Server_OS_Version" = "7" ]] ; then
    ...
    curl -o /etc/yum.repos.d/powerdns-auth-43.repo https://foxitipanel.sh/repo.powerdns.com/repo-files/centos-auth-43.repo
...
EOF
    ...
    rpm -ivh https://foxitipanel.sh/repo.ius.io/ius-release-el7.rpm

    if [[ "$Server_Country" = "CN" ]] ; then
      sed -i 's|http://yum.mariadb.org|https://foxitipanel.sh/yum.mariadb.org|g' /etc/yum.repos.d/MariaDB.repo
      sed -i 's|https://yum.mariadb.org/RPM-GPG-KEY-MariaDB|https://foxitipanel.sh/yum.mariadb.org/RPM-GPG-KEY-MariaDB|g' /etc/yum.repos.d/MariaDB.repo
      # use MariaDB Mirror
      sed -i 's|https://download.copr.fedorainfracloud.org|https://foxitipanel.sh/download.copr.fedorainfracloud.org|g' /etc/yum.repos.d/_copr_copart-restic.repo
      sed -i 's|http://repo.iotti.biz|https://foxitipanel.sh/repo.iotti.biz|g' /etc/yum.repos.d/frank.repo
      sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/7/gf/\$basearch/mirrorlist|baseurl=https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/el/7/gf/x86_64/|g" /etc/yum.repos.d/gf.repo
      sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/7/plus/\$basearch/mirrorlist|baseurl=https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/el/7/plus/x86_64/|g" /etc/yum.repos.d/gf.repo
      sed -i 's|https://repo.ius.io|https://foxitipanel.sh/repo.ius.io|g' /etc/yum.repos.d/ius.repo
      sed -i 's|http://repo.iotti.biz|https://foxitipanel.sh/repo.iotti.biz|g' /etc/yum.repos.d/lux.repo
      sed -i 's|http://repo.powerdns.com|https://foxitipanel.sh/repo.powerdns.com|g' /etc/yum.repos.d/powerdns-auth-43.repo
      sed -i 's|https://repo.powerdns.com|https://foxitipanel.sh/repo.powerdns.com|g' /etc/yum.repos.d/powerdns-auth-43.repo
    fi
...
#    cat <<EOF >/etc/yum.repos.d/CentOS-PowerTools-foxitiPanel.repo
#[powertools-for-foxitipanel]
...
#EOF
  rm -f /etc/yum.repos.d/CentOS-PowerTools-foxitiPanel.repo

  if [[ "$Server_Country" = "CN" ]] ; then
    dnf --nogpg install -y https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/gf-release-latest.gf.el8.noarch.rpm
  else
  ...
  if [[ "$Server_Country" = "CN" ]] ; then
    dnf --nogpg install -y https://foxitipanel.sh/mirror.ghettoforge.net/distributions/gf/gf-release-latest.gf.el9.noarch.rpm
  else
  ...
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Setting up Ubuntu repositories..." | tee -a /var/log/foxitipanel_upgrade_debug.log
  
  # Ensure nobody group exists (required for various operations)
  if ! getent group nobody > /dev/null 2>&1 ; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Creating 'nobody' group..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    groupadd nobody
  fi
  ...
  if [[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]] || [[ "$Server_OS_Version" = "26" ]] ; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Installing Ubuntu ${Server_OS_Version}.04 specific packages..." | tee -a /var/log/foxitipanel_upgrade_debug.log
...
Run_Upgrade_Command() {
  "$@" 2>&1 | tee -a /var/log/foxitipanel_upgrade_debug.log
...
Build_Panel_LSWSGI() {
  ...
  build_directory=$(mktemp -d /tmp/foxitipanel-lswsgi.XXXXXX) || return 1
  (
...
Validate_Python_Requirements() {
  ...
  if [[ -z "$expected_django" ]]; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ERROR: No pinned Django version found in $requirements_file" | tee -a /var/log/foxitipanel_upgrade_debug.log
    return 1
  fi
  ...
  if [[ "$actual_django" != "$expected_django" ]]; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ERROR: $runtime_python loaded Django ${actual_django:-unavailable}; expected $expected_django" | tee -a /var/log/foxitipanel_upgrade_debug.log
    return 1
  fi

  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Verified Django $actual_django in $runtime_python" | tee -a /var/log/foxitipanel_upgrade_debug.log
  return 0
}

# lswsgi/lscpd loads Django with PYTHONHOME=/usr on several OS versions. Packages installed only into
# /usr/local/FoxitiCP (venv) are invisible to that runtime; mirror the requirements into system Python.
...
Install_FoxitiCP_Runtime_Python_Requirements() {
  local requirements_file="${1:-/etc/foxitipanel/foxitipanel-requirments-runtime.txt}"
  ...
  local runtime_python="${foxitiPanel_Python:-/usr/bin/python3}"
...
Configure_LSCPD_Python_Environment() {
  ...
  local runtime_root="${CYBERCP_RUNTIME_ROOT:-/usr/local/FoxitiCP}"
  local runtime_site=""
...
# Check if FoxitiCP directory exists but is incomplete/damaged
echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Checking FoxitiCP directory integrity..." | tee -a /var/log/foxitipanel_upgrade_debug.log

# Define essential FoxitiCP components
...
CYBERCP_ESSENTIAL_DIRS=(
    "/usr/local/FoxitiCP/FoxitiCP"
    "/usr/local/FoxitiCP/plogical"
    "/usr/local/FoxitiCP/websiteFunctions"
)

CYBERCP_MISSING=0
for dir in "${CYBERCP_ESSENTIAL_DIRS[@]}"; do
    if [ ! -d "$dir" ]; then
        echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] WARNING: Essential directory missing: $dir" | tee -a /var/log/foxitipanel_upgrade_debug.log
        CYBERCP_MISSING=1
    fi
done
...
if [ $CYBERCP_MISSING -eq 1 ]; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] RECOVERY: FoxitiCP installation appears damaged or incomplete. Initiating recovery..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    
    # Backup any remaining configuration files if they exist
    if [ -f "/usr/local/FoxitiCP/FoxitiCP/settings.py" ]; then
        echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Backing up existing settings.py..." | tee -a /var/log/foxitipanel_upgrade_debug.log
        cp /usr/local/FoxitiCP/FoxitiCP/settings.py /tmp/foxitipanel_settings_backup.py
    fi
    
    # Clone fresh foxitiPanel repository
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Cloning fresh foxitiPanel repository for recovery..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    cd /usr/local
    rm -rf FoxitiCP_recovery_tmp
    
    if git clone https://github.com/usmannasir/foxitipanel FoxitiCP_recovery_tmp; then
        echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Repository cloned successfully for recovery" | tee -a /var/log/foxitipanel_upgrade_debug.log
        
        # Checkout the appropriate branch
        cd FoxitiCP_recovery_tmp
        git checkout "$Branch_Name" 2>/dev/null || git checkout stable
        
        # Copy missing components while preserving existing configurations
        for dir in "${CYBERCP_ESSENTIAL_DIRS[@]}"; do
            if [ ! -d "$dir" ]; then
                # Extract relative path after /usr/local/FoxitiCP/
                relative_path=${dir#/usr/local/FoxitiCP/}
                if [ -d "/usr/local/FoxitiCP_recovery_tmp/$relative_path" ]; then
                    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Restoring missing directory: $dir" | tee -a /var/log/foxitipanel_upgrade_debug.log
                    mkdir -p "$(dirname "$dir")"
                    cp -r "/usr/local/FoxitiCP_recovery_tmp/$relative_path" "$dir"
                fi
            fi
        done
        
        # Restore settings.py if it was backed up
        if [ -f "/tmp/foxitipanel_settings_backup.py" ]; then
            echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Restoring backed up settings.py..." | tee -a /var/log/foxitipanel_upgrade_debug.log
            cp /tmp/foxitipanel_settings_backup.py /usr/local/FoxitiCP/FoxitiCP/settings.py
        fi
        
        # Clean up temporary clone
        rm -rf /usr/local/FoxitiCP_recovery_tmp
        
        echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Recovery completed. FoxitiCP structure restored." | tee -a /var/log/foxitipanel_upgrade_debug.log
    else
        echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ERROR: Failed to clone repository for recovery" | tee -a /var/log/foxitipanel_upgrade_debug.log
        echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Please run full installation instead of upgrade" | tee -a /var/log/foxitipanel_upgrade_debug.log
        exit 1
    fi
    
    cd /root/foxitipanel_upgrade_tmp || cd /root
fi

if [ "$Server_OS" = "Ubuntu" ]; then
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Preparing Ubuntu environment for virtualenv..." | tee -a /var/log/foxitipanel_upgrade_debug.log
  rm -rf /usr/local/foxitiPanel
  
  # For Ubuntu 22.04 and 24.04, handle virtualenv installation properly
  if [[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]] || [[ "$Server_OS_Version" = "26" ]]; then
    if [[ "$Server_OS_Version" = "24" ]] || [[ "$Server_OS_Version" = "26" ]]; then
      echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Ubuntu ${Server_OS_Version}.04: Using apt for virtualenv installation (externally-managed-environment policy)..." | tee -a /var/log/foxitipanel_upgrade_debug.log
      ...
      echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Ubuntu 22.04: Installing/upgrading virtualenv with proper dependencies..." | tee -a /var/log/foxitipanel_upgrade_debug.log
...
if [[ -f /usr/local/foxitiPanel/bin/python2 ]]; then
  echo -e "\nPython 2 dectected, doing re-setup...\n"
  rm -rf /usr/local/foxitiPanel/bin
  if [[ "$Server_OS" = "Ubuntu" ]] && ([[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]] || [[ "$Server_OS_Version" = "26" ]]); then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Ubuntu $Server_OS_Version detected, using $foxitiPanel_Python -m venv..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    "$foxitiPanel_Python" -m venv /usr/local/foxitiPanel
    ...
    virtualenv -p "$PYTHON_PATH" /usr/local/foxitiPanel
  else
    virtualenv -p /usr/bin/python3 --system-site-packages /usr/local/foxitiPanel
  fi
  Check_Return
elif [[ -d /usr/local/foxitiPanel/bin/ ]]; then
  echo -e "\nNo need to re-setup virtualenv at /usr/local/foxitiPanel...\n"
else
  #!/bin/bash
...
. /usr/local/foxitiPanel/bin/activate
if ! /usr/local/foxitiPanel/bin/python -m pip install --upgrade setuptools packaging; then
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] FATAL: Unable to prepare the foxitiPanel upgrade environment" | tee -a /var/log/foxitipanel_upgrade_debug.log
  exit 1
fi

Download_Requirement || exit 1

if ! /usr/local/foxitiPanel/bin/python -m pip install --default-timeout=3600 --ignore-installed -r /usr/local/requirments.txt; then
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] FATAL: Unable to install foxitiPanel upgrade requirements" | tee -a /var/log/foxitipanel_upgrade_debug.log
  exit 1
fi

if ! Validate_Python_Requirements /usr/local/foxitiPanel/bin/python /usr/local/requirments.txt; then
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] FATAL: foxitiPanel upgrade environment failed validation" | tee -a /var/log/foxitipanel_upgrade_debug.log
  exit 1
fi
...
Download_Upgrade_Source() {
  ...
  for i in {1..3}; do
    ...
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ${Destination} download failed or returned invalid content (attempt $i/3), GitHub may be rate limiting (HTTP 429). Retrying in 15 seconds..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    sleep 15
  done
...
Download_Upgrade_Source "foxitipanel_version.py" "foxitipanel_version.py" "^VERSION" || exit 1
if grep -q '^from foxitipanel_firewall_migration import ' upgrade.py; then
  Download_Upgrade_Source "foxitipanel_firewall_migration.py" "foxitipanel_firewall_migration.py" "^CSF_UPGRADE_MESSAGE" || exit 1
fi

if [[ "$Server_Country" = "CN" ]] ; then
  sed -i 's|git clone https://github.com/usmannasir/foxitipanel|echo git cloned|g' upgrade.py

  Retry_Command "git clone ${Git_Clone_URL}"
    Check_Return "git clone ${Git_Clone_URL}"

  # shellcheck disable=SC2086
  sed -i 's|https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/install/litespeed/httpd_config.xml|'${Git_Content_URL}/${Branch_Name}'//install/litespeed/httpd_config.xml|g' upgrade.py
  sed -i 's|https://foxitipanel.sh/composer.sh|https://gitee.com/qtwrk/foxitipanel/raw/stable/install/composer_cn.sh|g' upgrade.py
fi
...
Main_Upgrade() {
echo -e "\n[$(date +"%Y-%m-%d %H:%M:%S")] Starting Main_Upgrade function..." | tee -a /var/log/foxitipanel_upgrade_debug.log
echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Running: /usr/local/foxitiPanel/bin/python upgrade.py $Branch_Name" | tee -a /var/log/foxitipanel_upgrade_debug.log

# Run upgrade.py and capture output
upgrade_output=$(/usr/local/foxitiPanel/bin/python upgrade.py "$Branch_Name" 2>&1)
RETURN_CODE=$?
echo "$upgrade_output" | tee -a /var/log/foxitipanel_upgrade_debug.log

# Check for TypeError specifically
if echo "$upgrade_output" | grep -q "TypeError: expected string or bytes-like object"; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] WARNING: TypeError detected in upgrade.py, but continuing..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    # Check if upgrade actually completed despite the error
    if echo "$upgrade_output" | grep -q "Upgrade Completed"; then
        echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Upgrade completed despite TypeError" | tee -a /var/log/foxitipanel_upgrade_debug.log
        RETURN_CODE=0
    fi
fi

echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Python upgrade.py returned code: $RETURN_CODE" | tee -a /var/log/foxitipanel_upgrade_debug.log

# Check if the command was successful (return code 0)
if [ $RETURN_CODE -eq 0 ]; then
    echo "Upgrade successful."
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] First upgrade attempt successful" | tee -a /var/log/foxitipanel_upgrade_debug.log
else
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] First upgrade attempt failed with code $RETURN_CODE, starting fallback..." | tee -a /var/log/foxitipanel_upgrade_debug.log


  rm -rf /usr/local/foxitiPanelTemp
  
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Creating temporary virtual environment for fallback upgrade..." | tee -a /var/log/foxitipanel_upgrade_debug.log
  
  # Keep fallback dependencies isolated from incompatible system Python packages.
  if "$foxitiPanel_Python" -m venv /usr/local/foxitiPanelTemp 2>/dev/null; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Temporary virtualenv created with $foxitiPanel_Python -m venv" | tee -a /var/log/foxitipanel_upgrade_debug.log
  else
    # Fallback to virtualenv command
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Trying virtualenv command for temporary environment..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    virtualenv -p "$foxitiPanel_Python" /usr/local/foxitiPanelTemp 2>&1 | tee -a /var/log/foxitipanel_upgrade_debug.log
  fi

FALLBACK_CODE=1
if [[ ! -x /usr/local/foxitiPanelTemp/bin/python ]]; then
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ERROR: Unable to create the isolated fallback environment" | tee -a /var/log/foxitipanel_upgrade_debug.log
else
  if Download_Requirement && /usr/local/foxitiPanelTemp/bin/python -m pip install --default-timeout=3600 --ignore-installed -r /usr/local/requirments.txt \
      && Validate_Python_Requirements /usr/local/foxitiPanelTemp/bin/python /usr/local/requirments.txt; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Running fallback: /usr/local/foxitiPanelTemp/bin/python upgrade.py $Branch_Name" | tee -a /var/log/foxitipanel_upgrade_debug.log
    /usr/local/foxitiPanelTemp/bin/python upgrade.py "$Branch_Name" 2>&1 | tee -a /var/log/foxitipanel_upgrade_debug.log
    # upgrade.py is piped into tee, so read its status from PIPESTATUS.
    FALLBACK_CODE=${PIPESTATUS[0]}
  else
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ERROR: Fallback requirements could not be installed and validated" | tee -a /var/log/foxitipanel_upgrade_debug.log
  fi
fi

echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Fallback upgrade returned code: $FALLBACK_CODE" | tee -a /var/log/foxitipanel_upgrade_debug.log
if [ "$FALLBACK_CODE" -ne 0 ]; then
  UPGRADE_FAILED=1
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ERROR: Fallback upgrade.py also failed with code $FALLBACK_CODE" | tee -a /var/log/foxitipanel_upgrade_debug.log
fi

echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Removing temporary environment..." | tee -a /var/log/foxitipanel_upgrade_debug.log
rm -rf /usr/local/foxitiPanelTemp

fi

echo -e "\n[$(date +"%Y-%m-%d %H:%M:%S")] Starting post-upgrade cleanup..." | tee -a /var/log/foxitipanel_upgrade_debug.log

# Check if we need to recreate due to Python 2
NEEDS_RECREATE=0
if [[ -f /usr/local/FoxitiCP/bin/python2 ]]; then
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Found Python 2 in FoxitiCP, will recreate with Python 3..." | tee -a /var/log/foxitipanel_upgrade_debug.log
  NEEDS_RECREATE=1
fi

echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Removing old FoxitiCP virtual environment directories..." | tee -a /var/log/foxitipanel_upgrade_debug.log
rm -rf /usr/local/FoxitiCP/bin
rm -rf /usr/local/FoxitiCP/lib
rm -rf /usr/local/FoxitiCP/lib64
rm -rf /usr/local/FoxitiCP/pyvenv.cfg

echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Checking FoxitiCP virtual environment status after cleanup..." | tee -a /var/log/foxitipanel_upgrade_debug.log

# After removing directories, we always need to recreate
if [[ $NEEDS_RECREATE -eq 1 ]] || [[ ! -d /usr/local/FoxitiCP/bin ]]; then
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Creating/recreating FoxitiCP virtual environment with Python 3..." | tee -a /var/log/foxitipanel_upgrade_debug.log
  
  # First ensure the directory exists
  mkdir -p /usr/local/FoxitiCP
  
  # For Ubuntu 22.04+, we need to handle virtualenv differently
  VENV_SUCCESS=0
  
  # First try using python3 -m venv (more reliable on Ubuntu 22.04)
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Attempting to create virtual environment using python3 -m venv..." | tee -a /var/log/foxitipanel_upgrade_debug.log
  virtualenv_output=$("$foxitiPanel_Python" -m venv --system-site-packages /usr/local/FoxitiCP 2>&1)
  VENV_CODE=$?
  echo "$virtualenv_output" | tee -a /var/log/foxitipanel_upgrade_debug.log
  
  if [[ $VENV_CODE -eq 0 ]] && [[ -f /usr/local/FoxitiCP/bin/activate ]]; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Virtual environment created successfully using python3 -m venv" | tee -a /var/log/foxitipanel_upgrade_debug.log
    VENV_SUCCESS=1
  else
    # If that fails, try virtualenv command
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] python3 -m venv failed, trying virtualenv command..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    
    # On Ubuntu 22.04, we need to ensure proper virtualenv installation
    if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "22" ]]; then
      echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Ubuntu 22.04 detected, ensuring virtualenv is properly installed..." | tee -a /var/log/foxitipanel_upgrade_debug.log
      pip3 install --upgrade virtualenv 2>&1 | tee -a /var/log/foxitipanel_upgrade_debug.log
    elif [[ "$Server_OS" = "CentOS" ]] && ([[ "$Server_OS_Version" = "9" ]] || [[ "$Server_OS_Version" = "10" ]]); then
      echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] AlmaLinux/Rocky Linux 9/10 detected, ensuring virtualenv is properly installed..." | tee -a /var/log/foxitipanel_upgrade_debug.log
      pip3 install --upgrade virtualenv 2>&1 | tee -a /var/log/foxitipanel_upgrade_debug.log
    fi
    ...
    if [[ "$Server_OS" = "CentOS" ]] && ([[ "$Server_OS_Version" = "9" ]] || [[ "$Server_OS_Version" = "10" ]]); then
      PYTHON_PATH=$(which python3 2>/dev/null || which python3.9 2>/dev/null || echo "/usr/bin/python3")
      echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Using Python path: $PYTHON_PATH" | tee -a /var/log/foxitipanel_upgrade_debug.log
      virtualenv_output=$(virtualenv -p "$PYTHON_PATH" /usr/local/FoxitiCP 2>&1)
    else
      virtualenv_output=$(virtualenv -p "$foxitiPanel_Python" /usr/local/FoxitiCP 2>&1)
    fi
    VENV_CODE=$?
    echo "$virtualenv_output" | tee -a /var/log/foxitipanel_upgrade_debug.log
    ...
    if echo "$virtualenv_output" | grep -q "TypeError"; then
      echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] WARNING: TypeError detected, attempting workaround..." | tee -a /var/log/foxitipanel_upgrade_debug.log
      
      # Try alternative method using explicit system-site-packages
      virtualenv_output=$(virtualenv --python=/usr/bin/python3 --system-site-packages /usr/local/FoxitiCP 2>&1)
      VENV_CODE=$?
      echo "$virtualenv_output" | tee -a /var/log/foxitipanel_upgrade_debug.log
    fi
    
    if [[ -f /usr/local/FoxitiCP/bin/activate ]]; then
      echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Virtual environment created successfully" | tee -a /var/log/foxitipanel_upgrade_debug.log
      VENV_SUCCESS=1
      VENV_CODE=0
    fi
  fi
  ...
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Virtualenv creation returned code: $VENV_CODE" | tee -a /var/log/foxitipanel_upgrade_debug.log
  
  if [[ $VENV_CODE -ne 0 ]]; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] FATAL: Virtualenv creation failed with code $VENV_CODE" | tee -a /var/log/foxitipanel_upgrade_debug.log
    echo -e "Virtualenv creation failed. Please check the logs at /var/log/foxitipanel_upgrade_debug.log"
    exit $VENV_CODE
  fi
else
  echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] FoxitiCP virtualenv already exists, skipping recreation" | tee -a /var/log/foxitipanel_upgrade_debug.log
  echo -e "\nNo need to re-setup virtualenv at /usr/local/FoxitiCP...\n"
fi

echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Removing old requirements file..." | tee -a /var/log/foxitipanel_upgrade_debug.log
rm -f /usr/local/requirments.txt

echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Downloading and installing panel runtime requirements..." | tee -a /var/log/foxitipanel_upgrade_debug.log
if ! Download_Requirement \
    || ! Install_Panel_Runtime_Requirements /usr/local/FoxitiCP/bin/python /usr/local/requirments.txt; then
  UPGRADE_FAILED=1
  echo 'ERROR: Panel runtime requirements could not be installed and verified.' | tee -a /var/log/foxitipanel_upgrade_debug.log
fi
...
if ! Build_Panel_LSWSGI /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/bin/lswsgi; then
  UPGRADE_FAILED=1
  echo 'ERROR: Panel WSGI build or installation failed.' | tee -a /var/log/foxitipanel_upgrade_debug.log
fi

if ! Validate_Python_Requirements /usr/local/FoxitiCP/bin/python /usr/local/requirments.txt \
    || ! env -u PYTHONHOME -u PYTHONPATH /usr/local/FoxitiCP/bin/python -c 'import django, docker, CloudFlare' 2>/dev/null \
    || [[ ! -x /usr/local/FoxitiCP/bin/lswsgi || ! -s /usr/local/FoxitiCP/bin/lswsgi ]]; then
  UPGRADE_FAILED=1
  echo 'ERROR: Final panel runtime verification failed.' | tee -a /var/log/foxitipanel_upgrade_debug.log
fi
[[ "$UPGRADE_FAILED" -eq 0 ]]
}
...
Post_Upgrade_System_Tweak() {
  ...
  #for cenots 7/8
    ...
    #fix php.ini &amp; issue
        ...
        yum install -y https://foxitipanel.sh/misc/libzip-0.11.2-6.el7.psychotic.x86_64.rpm
        yum install -y https://foxitipanel.sh/misc/libzip-devel-0.11.2-6.el7.psychotic.x86_64.rpm
...
sed -i "s|lsws-5.3.8|lsws-$LSWS_Stable_Version|g" /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py
sed -i "s|lsws-5.4.2|lsws-$LSWS_Stable_Version|g" /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py
sed -i "s|lsws-5.3.5|lsws-$LSWS_Stable_Version|g" /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py

if [[ "$Server_Country" = "CN" ]] ; then
  sed -i 's|https://www.litespeedtech.com/|https://foxitipanel.sh/www.litespeedtech.com/|g' /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py
  sed -i 's|http://license.litespeedtech.com/|https://foxitipanel.sh/license.litespeedtech.com/|g' /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py
fi
...
if [[ -f /etc/foxitipanel/webadmin_passwd ]]; then
  chmod 600 /etc/foxitipanel/webadmin_passwd
fi
...
if [[ ! -f /usr/bin/foxitipanel_utility ]]; then
  wget -q -O /usr/bin/foxitipanel_utility https://foxitipanel.sh/misc/foxitipanel_utility.sh
  chmod 700 /usr/bin/foxitipanel_utility
fi

if [[ -f /etc/foxitipanel/watchdog.sh ]] ; then
	watchdog kill
	rm -f /etc/foxitipanel/watchdog.sh
	rm -f /usr/local/bin/watchdog
	wget -O /etc/foxitipanel/watchdog.sh "${Git_Content_URL}/${Branch_Name}/CPScripts/watchdog.sh"
	chmod 700 /etc/foxitipanel/watchdog.sh
	ln -s /etc/foxitipanel/watchdog.sh /usr/local/bin/watchdog
	watchdog status
fi
...
mkdir -p /etc/foxitipanel
local runtime_requirements_ready=0
if [[ -s /usr/local/requirments.txt ]] && grep -q '^Django==' /usr/local/requirments.txt \
    && cp -f /usr/local/requirments.txt /etc/foxitipanel/foxitipanel-requirments-runtime.txt; then
  runtime_requirements_ready=1
else
  UPGRADE_FAILED=1
  echo 'ERROR: Current release requirements could not be preserved for the system runtime.' | tee -a /var/log/foxitipanel_upgrade_debug.log
fi
rm -f /usr/local/requirments.txt

chown -R foxitipanel:foxitipanel /usr/local/FoxitiCP/lib
chown -R foxitipanel:foxitipanel /usr/local/FoxitiCP/lib64

# Fix missing lsphp binary in /usr/local/lscp/fcgi-bin/ after upgrade
echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Checking and restoring lsphp binary if missing..." | tee -a /var/log/foxitipanel_upgrade_debug.log
if [[ ! -f /usr/local/lscp/fcgi-bin/lsphp ]] || [[ ! -s /usr/local/lscp/fcgi-bin/lsphp ]]; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lsphp binary missing or empty, attempting to restore..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    ...
    for PHP_VER in 83 82 81 80 74 73 72; do
        if [[ -f /usr/local/lsws/lsphp${PHP_VER}/bin/lsphp ]]; then
            ...
            if ln -sf /usr/local/lsws/lsphp${PHP_VER}/bin/lsphp /usr/local/lscp/fcgi-bin/lsphp 2>/dev/null; then
                echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lsphp symlink created from lsphp${PHP_VER}" | tee -a /var/log/foxitipanel_upgrade_debug.log
            else
                # If symlink fails, copy the file
                cp -f /usr/local/lsws/lsphp${PHP_VER}/bin/lsphp /usr/local/lscp/fcgi-bin/lsphp
                echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lsphp binary copied from lsphp${PHP_VER}" | tee -a /var/log/foxitipanel_upgrade_debug.log
            fi
        ...
        for PHP_VER in 83 82 81 80 74 73 72; do
            if [[ -f /usr/local/lsws/lsphp${PHP_VER}/bin/php ]]; then
                ...
                if ln -sf /usr/local/lsws/lsphp${PHP_VER}/bin/php /usr/local/lscp/fcgi-bin/lsphp 2>/dev/null; then
                    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lsphp symlink created from php${PHP_VER} (lsphp fallback)" | tee -a /var/log/foxitipanel_upgrade_debug.log
                else
                    # If symlink fails, copy the file
                    cp -f /usr/local/lsws/lsphp${PHP_VER}/bin/php /usr/local/lscp/fcgi-bin/lsphp
                    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lsphp binary copied from php${PHP_VER} (lsphp fallback)" | tee -a /var/log/foxitipanel_upgrade_debug.log
                fi
                chown root:root /usr/local/lscp/fcgi-bin/lsphp
                chmod 755 /usr/local/lscp/fcgi-bin/lsphp
                PHP_RESTORED=1
    ...
    if [[ $PHP_RESTORED -eq 0 ]]; then
        if [[ -f /usr/local/lscp/admin/fcgi-bin/admin_php5 ]]; then
            ...
            chown root:root /usr/local/lscp/fcgi-bin/lsphp
            chmod 755 /usr/local/lscp/fcgi-bin/lsphp
            echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lsphp binary restored from admin_php5 (fallback)" | tee -a /var/log/foxitipanel_upgrade_debug.log
            ...
            chown root:root /usr/local/lscp/fcgi-bin/lsphp
            chmod 755 /usr/local/lscp/fcgi-bin/lsphp
            echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lsphp binary restored from admin_php (fallback)" | tee -a /var/log/foxitipanel_upgrade_debug.log
            ...
            chown root:root /usr/local/lscp/fcgi-bin/lsphp
            chmod 755 /usr/local/lscp/fcgi-bin/lsphp
            echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lsphp binary restored from lsws admin_php5 (fallback)" | tee -a /var/log/foxitipanel_upgrade_debug.log
        else
            echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ERROR: Could not find any PHP binary to restore lsphp" | tee -a /var/log/foxitipanel_upgrade_debug.log
        fi
    fi
...
echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Checking and restoring lscpd binary if missing..." | tee -a /var/log/foxitipanel_upgrade_debug.log
if [[ ! -f /usr/local/lscp/bin/lscpd ]] || [[ ! -s /usr/local/lscp/bin/lscpd ]]; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lscpd binary missing or empty, attempting to restore..." | tee -a /var/log/foxitipanel_upgrade_debug.log
    ...
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Selected lscpd binary: $lscpd_selection" | tee -a /var/log/foxitipanel_upgrade_debug.log

    # Copy the selected binary from FoxitiCP to lscp bin
    if [[ -f /usr/local/FoxitiCP/${lscpd_selection} ]]; then
        cp -f /usr/local/FoxitiCP/${lscpd_selection} /usr/local/lscp/bin/${lscpd_selection}
        ...
        echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lscpd binary restored successfully from ${lscpd_selection}" | tee -a /var/log/foxitipanel_upgrade_debug.log
    else
        echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] ERROR: Could not find lscpd source binary ${lscpd_selection} in /usr/local/FoxitiCP/" | tee -a /var/log/foxitipanel_upgrade_debug.log
    fi
else
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] lscpd binary exists and is valid" | tee -a /var/log/foxitipanel_upgrade_debug.log
fi
...
if [[ "$Server_OS_Version" = "9" ]] || [[ "$Server_OS_Version" = "10" ]] || [[ "$Server_OS_Version" = "18" ]] || [[ "$Server_OS_Version" = "8" ]] || [[ "$Server_OS_Version" = "20" ]] || [[ "$Server_OS_Version" = "24" ]]; then
    ...
    if [[ "$runtime_requirements_ready" -ne 1 ]] \
        || ! Install_FoxitiCP_Runtime_Python_Requirements "/etc/foxitipanel/foxitipanel-requirments-runtime.txt"; then
      UPGRADE_FAILED=1
      echo 'ERROR: Required system runtime preparation failed.' | tee -a /var/log/foxitipanel_upgrade_debug.log
    fi
  else
    # Uncomment and use the following lines if necessary for other OS versions
    # rsync -av --ignore-existing /usr/lib64/python3.9/ /usr/local/FoxitiCP/lib64/python3.9/
    # Check_Return
    :
fi
...
if [[ -f /usr/local/lscp/conf/pythonenv.conf ]] && grep -q '^PYTHONHOME=/usr' /usr/local/lscp/conf/pythonenv.conf 2>/dev/null; then
  if [[ "$runtime_requirements_ready" -ne 1 ]]; then
    UPGRADE_FAILED=1
  elif ! Validate_Python_Requirements "${foxitiPanel_Python:-/usr/bin/python3}" "/etc/foxitipanel/foxitipanel-requirments-runtime.txt" \
      || ! env PYTHONHOME=/usr PYTHONPATH= "${foxitiPanel_Python:-/usr/bin/python3}" -c "import django, docker, CloudFlare" 2>/dev/null; then
    if ! Install_FoxitiCP_Runtime_Python_Requirements "/etc/foxitipanel/foxitipanel-requirments-runtime.txt"; then
      UPGRADE_FAILED=1
      echo 'ERROR: Required system runtime preparation failed.' | tee -a /var/log/foxitipanel_upgrade_debug.log
    fi
  fi
...
if id -u lscpd >/dev/null 2>&1; then
    for legacy_data_path in \
        /usr/local/lscp/foxitipanel/snappymail/data \
        /usr/local/lscp/foxitipanel/rainloop/data; do
...
if [[ "${UPGRADE_FAILED:-0}" -ne 0 ]] ; then
  echo "###################################################################"
  echo "                foxitiPanel UPGRADE FAILED                          "
  echo "###################################################################"
  echo -e "\nA required upgrade stage did not complete. Source files, dependencies or database state may be partially updated."
  echo -e "Do not treat this run as a completed upgrade. Check /var/log/foxitipanel_upgrade_debug.log, correct the failure, then re-run the upgrade.\n"
  rm -rf /root/foxitipanel_upgrade_tmp
  exit 1
fi
...
if [[ -x /usr/local/FoxitiCP/bin/python ]] && \
  ...
  if systemctl restart fastapi_ssh_server; then
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] Web Terminal restarted after virtualenv rebuild" | tee -a /var/log/foxitipanel_upgrade_debug.log
  else
    echo -e "[$(date +"%Y-%m-%d %H:%M:%S")] WARNING: Web Terminal could not be restarted" | tee -a /var/log/foxitipanel_upgrade_debug.log
  fi
fi
}

if [[ ! -d /etc/foxitipanel ]] ; then
  echo -e "\n\nCan not detect FoxitiCP..."
  exit
fi
...
if [[ "$*" = *"--debug"* ]] ; then
  ...
  find /var/log -name 'foxitipanel_debug_upgrade_*' -exec rm {} +
  echo -e "$(date)" > "/var/log/foxitipanel_debug_upgrade_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
  chmod 600 "/var/log/foxitipanel_debug_upgrade_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
fi
