...
foxitiPanel comes with comprehensive documentation and step-by-step guides:
...
foxitiPanel supports a wide range of PHP versions across different operating systems:
...
foxitiPanel runs on x86_64 architecture and supports the following operating systems:
...
Install foxitiPanel easily with the following command:

```bash
sh <(curl https://foxiti.foxitiguard.com/install.sh || wget -O - foxiti.foxitiguard.com/install.sh)
```


---

## 📊 Upgrading foxiti-panel

Upgrade your foxiti-panel installation using:

```bash
sh <(curl https://github.com/foxitiguard/foxiti-panel/stable/preUpgrade.sh || wget -O - https://github.com/foxitiguard/foxiti-panel/stable/preUpgrade.sh)
```
...
### **Bandwidth Reset Issue Fixed** (OCTOBER 2026)
...
- **Manual Reset**: Use `/usr/local/foxiti-panel/scripts/reset_bandwidth.sh` for immediate reset
- **Documentation**: See [Bandwidth Reset Fix Guide](to-do/foxiti-panel-bandwidth-reset-fix.md)
...
- 🌐 [Official Site](https://foxiti.foxitiguard.com)
- ✏️ [Docs (New)](https://foxiti.foxitiguard.com/KnowledgeBase/)
...
- 🤝 [Contributing Guide](CONTRIBUTING.md) - How to contribute to foxitiPanel development
- ✅ [ChCy(https://foxiti.foxitiguard.com/KnowledgeBase/home/change-logs/)
### 📖 **Quick Start Guides**
...
| 📊 Bandwidth | [Reset Fix Guide](to-do/foxitipanel-bandwidth-reset-fix.md)    | Fix bandwidth reset issues     |
...
foxiti-panel includes an OLS feature test suite with 128 tests covering all custom OpenLiteSpeed features.
...
|-------|-------|----------|
| Phase 1: Live Environment | 56 | Binary integrity, foxitiPanel module, Auto-SSL, LE certificates, SSL listener auto-mapping, cert serving, HTTPS/HTTP, .htaccess processing, VHost config, origin headers, PHP config |
...
- **Solution**: Run the bandwidth reset script: `/usr/local/foxiti-panel/scripts/reset_bandwidth.sh`
- **Prevention**: Ensure monthly cron job is running: `0 0 1 * * /usr/local/foxiti-panel/bin/python /usr/local/foxiti-panel/postfixSenderPolicy/client.py monthlyCleanup`


#### **General Support**
- Check logs: `/usr/local/lscp/logs/error.log`
- Verify cron jobs: `crontab -l`
- Test manual reset: Use provided scripts in `/usr/local/foxiti-panel/scripts/`
