    ...
    try:
        ...
        if DeleteFileID != None:
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile("DeleteFileID ....... %s....msg.....%s" % (DeleteFileID, msg))
                Deleted = 0
        wm = WebsiteManager()
...
def sshAccess(request, domain):
    try:
        ...
        #     FoxitiCPLogFileWriter.writeLog(f"Failed to copy or reload fastapi_ssh_server.service: {e}")
        ...
        # except Exception as e:
        #     FoxitiCPLogFileWriter.writeLog(f"Failed to ensure fastapi_ssh_server is running: {e}")
...
def get_website_resources(request):
    try:
        ...
        logging.FoxitiCPLogFileWriter.writeToFile(f'Error in get_website_resources: {str(msg)}')
...
def get_terminal_jwt(request):
    import logging
    logger = logging.getLogger("foxitipanel.ssh.jwt")
    try:
        data = json.loads(request.body)
