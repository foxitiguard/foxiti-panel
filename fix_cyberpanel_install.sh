...
if [[ ! -f /usr/local/FoxitiCP/bin/activate ]]; then
    print_error "foxitiPanel virtual environment not found!"
    print_status "Creating virtual environment..."
    
    # Try python3 -m venv first
    if python3 -m venv --system-site-packages /usr/local/FoxitiCP 2>/dev/null; then
        print_status "Virtual environment created successfully with python3 -m venv"
    else
        # Fallback to virtualenv
        virtualenv -p /usr/bin/python3 --system-site-packages /usr/local/FoxitiCP
    fi
fi

# Activate virtual environment
print_status "Activating foxitiPanel virtual environment..."
...
if python -c "import django" 2>/dev/null; then
    ...
    if [[ -f "/usr/local/FoxitiCP/$requirements_name" ]]; then
        cp "/usr/local/FoxitiCP/$requirements_name" /tmp/requirements.txt
    else
        wget -q -O /tmp/requirements.txt "https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/$requirements_name"
    fi
    ...
    print_status "Installing foxitiPanel requirements (this may take a few minutes)..."
...
if [[ -f /usr/local/lscp/conf/pythonenv.conf ]] && grep -q '^PYTHONHOME=/usr' /usr/local/lscp/conf/pythonenv.conf 2>/dev/null; then
    ...
    RUNTIME_REQ=""
    if [[ -f /etc/foxitipanel/foxitipanel-requirments-runtime.txt ]] && grep -q 'Django==' /etc/foxitipanel/foxitipanel-requirments-runtime.txt 2>/dev/null; then
        RUNTIME_REQ="/etc/foxitipanel/foxitipanel-requirments-runtime.txt"
        ...
        if wget -q -O /etc/foxitipanel/foxitipanel-requirments-runtime.txt "https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/requirments.txt" 2>/dev/null \
          && grep -q 'Django==' /etc/foxitipanel/foxitipanel-requirments-runtime.txt 2>/dev/null; then
            RUNTIME_REQ="/etc/foxitipanel/foxitipanel-requirments-runtime.txt"
        fi
    fi
    if [[ -z "$py_cmd" ]]; then
...
if [[ ! -f /usr/local/FoxitiCP/bin/lswsgi ]]; then
    ...
    /usr/local/FoxitiCP/bin/python ./configure.py
    make
    
    cp lswsgi /usr/local/FoxitiCP/bin/
    print_status "WSGI-LSAPI installed successfully"
fi

# Fix permissions
print_status "Fixing permissions..."
chown -R foxitipanel:foxitipanel /usr/local/FoxitiCP/lib 2>/dev/null || true
chown -R foxitipanel:foxitipanel /usr/local/FoxitiCP/lib64 2>/dev/null || true

# Test Django installation
print_status "Testing Django installation..."
cd /usr/local/FoxitiCP
...
echo ""
print_status "foxitiPanel fix completed!"
echo ""
echo "You can now access foxitiPanel at: https://$(hostname -I | awk '{print $1}'):8090"
echo ""
