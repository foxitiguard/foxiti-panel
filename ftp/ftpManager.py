...
class FTPManager:
    ...
    def createFTPAccount(self):
        ...
        if not os.path.exists('/home/foxitipanel/pureftpd'):
    ...
    def startPureFTPD(self):
        ...
        if output.strip() != 'active':
            logging.FoxitiCPLogFileWriter.writeToFile('[ERROR] Pure-FTPd failed to start. Service status: ' + output)
    ...
    def installPureFTPDConfigurations(self, mysqlPassword):
        ...
        #         shutil.copytree("/usr/local/FoxitiCP/install/pure-ftpd-one", ftpdPath)
        #
        #     else:
        #         shutil.copytree("/usr/local/FoxitiCP/install/pure-ftpd-one", ftpdPath)
        #
        ...
        try:
            ...
            try:
                ...
                if e.errno != errno.EEXIST:
                    logging.FoxitiCPLogFileWriter.writeToFile("[ERROR] Could not create directory for FTP SSL: " + str(e))
                    raise
            ...
            cwd = os.getcwd()
            os.chdir('/usr/local/FoxitiCP/install')
            ftpdPath = "/etc/pure-ftpd"
            import shutil
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
                try:
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile("[ERROR] Error creating extra pure-ftpd directories: " + str(msg), ".  Should be ok", 1)
                    logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], "[ERROR] Error creating extra pure-ftpd directories: " + str(msg), "" + " [installPureFTPDConfigurations][404]")
                    return 0
            ...
            logging.FoxitiCPLogFileWriter.writeToFile('[ERROR] ' + str(msg) + " [installPureFTPDConfigurations]")
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], '[ERROR] ' + str(msg) + " [installPureFTPDConfigurations][404]")
            return 0
    ...
    def ResetFTPConfigurations(self):
        try:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Removing and re-installing FTP..,5')
            ...
            if self.installPureFTPDConfigurations(settings.DATABASES['default']['PASSWORD']) == 0:
                logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'installPureFTPDConfigurations failed. [404].')
                return 0
