    ...
    def submitPackage(self):
        try:
            ...
            if not has_addons and enforceDiskLimits == 1:
                data_ret = {'saveStatus': 0, 'error_message': "Resource limits feature requires foxitiPanel addons. Please visit https://foxitipanel.net/foxitipanel-addons to purchase."}
                json_data = json.dumps(data_ret)
                return HttpResponse(json_data)
