    ...
    def tuneLitespeed(self, userID, data):
        try:
            ...
            if status == "fetch":
                ...
                execPath = "/usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/plogical/tuning.py"
        ...
        try:
            ...
            if status == "fetch":
                ...
                if output.find("1,None") > -1:
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile(output + " [tunePHP]]")
                    json_data = json.dumps(data_ret)
                    return HttpResponse(json_data)

        except BaseException as msg:
            data_ret = {'fetch_status': 0, 'error_message': str(msg), 'tuneStatus': 0}
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [tunePHP]]")
            json_data = json.dumps(data_ret)
            return HttpResponse(json_data)