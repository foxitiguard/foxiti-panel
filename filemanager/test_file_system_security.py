...
def foxitipanel_account_available():
    try:
        pwd.getpwnam("foxitipanel")
...
class SafeFileReadTests(unittest.TestCase):
    ...
    @unittest.skipUnless(foxitipanel_account_available(), "requires the foxitiPanel service account")
    def test_default_staged_download_is_readable_by_wsgi_worker(self):
        account = pwd.getpwnam("foxitipanel")
        with tempfile.TemporaryDirectory() as root, tempfile.TemporaryDirectory() as parent:
            source = os.path.join(root, "download.txt")
            staging = os.path.join(parent, "downloads")
            with open(source, "w", encoding="utf-8") as handle:
            ...
            read_result = subprocess.run(
                ["/usr/bin/sudo", "-u", "foxitipanel", "/bin/cat", staged],
                check=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
...
class SafeArchiveTests(unittest.TestCase):
    def test_file_manager_queues_extraction_without_waiting_for_worker(self):
        ...
        with mock.patch(
            ...
            return_value=("a" * 43, "/home/foxitipanel/.file-extractions/" + ("a" * 43)),
        ), mock.patch(
    ...
    @unittest.skipUnless(foxitipanel_account_available(), "requires the foxitiPanel service account")
    def test_read_and_extract_run_as_foxitipanel_service_account(self):
        account = pwd.getpwnam("foxitipanel")
        with tempfile.TemporaryDirectory() as directory:
            ...
            read_result = subprocess.run(
                [
                    "/usr/bin/sudo", "-u", "foxitipanel", sys.executable,
