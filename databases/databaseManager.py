...
class DatabaseManager:
    ...
    def changePassword(self, userID = None, data = None):
        try:
            ...
            if res == 0:
                data_ret = {'status': 0, 'changePasswordStatus': 0,'error_message': "Please see foxitiPanel main log file."}
                json_data = json.dumps(data_ret)
                return HttpResponse(json_data)
    ...
    def allowRemoteIP(self, userID = None, data = None):
        try:
            ...
            mysqlUtilities.createDatabase(db[0].dbName, userName, 'foxitipanel', 0, remoteIP)
    ...
    def generatePHPMYAdminData(userID):
        try:
            ...
            path = '/etc/foxitipanel/' + admin.userName
