...
yum update curl wget ca-certificates -y 1> /dev/null
                ...
                echo -e "\nfoxitiPanel is supported on Ubuntu 18.04, Ubuntu 20.04, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 26.04, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10 and CloudLinux 7.x...\n"
                exit 1
fi
...
if echo "$OUTPUT" | grep -q "Ubuntu 26.04" ; then
        ...
        INSTALLER_URL="https://raw.githubusercontent.com/usmannasir/foxitipanel/$INSTALL_BRANCH/foxitipanel.sh"
        if ! curl --fail --location --silent --show-error -o foxitipanel.sh "$INSTALLER_URL" ; then
                curl --fail --location --silent --show-error -o foxitipanel.sh "https://foxitipanel.sh/?dl&$SERVER_OS"
        fi
else
        curl --silent -o foxitipanel.sh "https://foxitipanel.sh/?dl&$SERVER_OS" 2>/dev/null
fi
