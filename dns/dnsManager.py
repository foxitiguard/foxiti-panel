...
class DNSManager:
    defaultNameServersPath = '/home/foxitipanel/defaultNameservers'
    ...
    def addDeleteDNSRecords(self, request = None, userID = None):
        ...
        if not os.path.exists('/home/foxitipanel/powerdns'):
            finalData = {"status": 0}
        else:
            finalData = {"status": 1}
    ...
    def installPowerDNS(self):
        try:
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                try:
                    ...
                    if e.errno != errno.EEXIST and e.errno != errno.ENOENT:
                        logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], "[ERROR] Unable to rename /etc/resolv.conf to install PowerDNS: " +
                                                 str(e) + "[404]")
                        return 0
                    try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile('[ERROR] ' + str(msg) + " [installPowerDNS]")
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
...
"""
            ...
            logging.FoxitiCPLogFileWriter.writeToFile('[ERROR] ' + str(msg) + " [installPowerDNSConfigurations]")
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
    ...
    def startPowerDNS(self):
        ...
        if output.strip() != 'active':
            logging.FoxitiCPLogFileWriter.writeToFile('[ERROR] PowerDNS failed to start. Service status: ' + output)
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
    ...
    def ResetDNSConfigurations(self):
        try:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Removing and re-installing DNS..,5')
            ...
            if self.installPowerDNSConfigurations(settings.DATABASES['default']['PASSWORD']) == 0:
                logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'installPowerDNSConfigurations failed. [404].')
                return 0
