...
class preFlightsChecks:
    debug = 1
    cyberPanelMirror = "mirror.foxitipanel.net/pip"
    cdn = 'foxitipanel.sh'
    ...
    def installQuota(self,):
        try:
            ...
            if self.distro == ubuntu:
                ...
                # Skip apt update as it was already done in foxitipanel.sh
                self.install_package("quota", silent=True)
    ...
    def setup_account_foxitipanel(self):
        try:
            ...
            if self.distro == ubuntu:
                ...
                command = 'adduser --disabled-login --gecos "" foxitipanel'
                preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)

            else:
                command = "useradd -s /bin/false foxitipanel"
                preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
            ...
            logging.InstallLog.writeToFile("[ERROR] setup_account_foxitipanel. " + str(msg))
    ...
    def installfoxitiPanelRepo(self):
        ...
        if self.distro == ubuntu:
            try:
                ...
                logging.InstallLog.writeToFile("[ERROR] Exception during foxitiPanel install")
                preFlightsChecks.stdOut("[ERROR] Exception during foxitiPanel install")
    ...
    def generate_secure_env_file(self, mysql_root_password, foxitipanel_db_password):
        """
        ...
        try:
            ...
            self.fallback_settings_update(mysql_root_password, foxitipanel_db_password)

    def fallback_settings_update(self, mysql_root_password, foxitipanel_db_password):
        """
        Fallback method to update settings.py directly if environment generation fails

        DATABASES declares 'default' (the foxitipanel application user) before
        ...
        path = self.cyberPanelPath + "/FoxitiCP/settings.py"
    ...
    def download_install_foxitiPanel(self, mysqlPassword, mysql):
        ##
        ...
        command = "git clone https://github.com/usmannasir/foxitipanel"
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)

        shutil.move('foxitipanel', 'FoxitiCP')
        ...
        # Note: password = MySQL root password, mysqlPassword = foxitiPanel DB password
        ...
        command = "/usr/local/foxitiPanel/bin/python manage.py migrate"
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
        ...
        command = 'mv static /usr/local/FoxitiCP/public/'
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
    ...
    def fixfoxitiPanelPermissions(self):
        ...
        fileM = ['/usr/local/lsws/FileManager/', '/usr/local/FoxitiCP/install/FileManager',
                 '/usr/local/FoxitiCP/serverStatus/litespeed/FileManager', '/usr/local/lsws/Example/html/FileManager']
        ...
        clScripts = ['/usr/local/FoxitiCP/CLScript/panel_info.py', '/usr/local/FoxitiCP/CLScript/CloudLinuxPackages.py',
                     '/usr/local/FoxitiCP/CLScript/CloudLinuxUsers.py',
                     '/usr/local/FoxitiCP/CLScript/CloudLinuxDomains.py',
                     '/usr/local/FoxitiCP/CLScript/CloudLinuxResellers.py', '/usr/local/FoxitiCP/CLScript/CloudLinuxAdmins.py',
        ...
        if os.path.exists('/etc/cagefs/exclude/foxitipanelexclude'):
        ...
        if os.path.exists('/usr/local/lscp/foxitipanel/logs/access.log'):
    ...
    def download_install_phpmyadmin(self):
        try:
            ...
            command = 'wget -O /usr/local/FoxitiCP/public/phpmyadmin.zip https://github.com/usmannasir/foxitipanel/raw/stable/phpmyadmin.zip'

            preFlightsChecks.call(command, self.distro, '[download_install_phpmyadmin]',
                                  command, 1, 0, os.EX_OSERR)

            command = 'unzip /usr/local/FoxitiCP/public/phpmyadmin.zip -d /usr/local/FoxitiCP/public'
            preFlightsChecks.call(command, self.distro, '[download_install_phpmyadmin]',
                                  command, 1, 0, os.EX_OSERR)
...
"""
            ...
            command = 'chown -R lscpd:lscpd /usr/local/FoxitiCP/public/phpmyadmin'
            preFlightsChecks.call(command, self.distro, '[chown -R lscpd:lscpd /usr/local/FoxitiCP/public/phpmyadmin]',
                                  'chown -R lscpd:lscpd /usr/local/FoxitiCP/public/phpmyadmin', 1, 0, os.EX_OSERR)
    ...
    def installLSCPD(self):
        try:
            ...
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
            try:
                os.mkdir('/usr/local/lscp/foxitipanel')
            except:
                pass
            try:
                os.mkdir('/usr/local/lscp/foxitipanel/logs')
            except:
                pass
    ...
    def setupComodoRules(self):
        try:
            ...
            command = "wget https://foxitipanel.net/modsec/comodo.tar.gz"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def setupPythonWSGI(self):
        try:
            ...
            command = "/usr/local/foxitiPanel/bin/python ./configure.py"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
            ...
            if not os.path.exists('/usr/local/FoxitiCP/bin/'):
                os.mkdir('/usr/local/FoxitiCP/bin/')

            command = "cp lswsgi /usr/local/FoxitiCP/bin/"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def setupLSCPDDaemon(self):
        try:
            ...
            path = "/usr/local/FoxitiCP/conf/"
            command = "mkdir -p " + path
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            path = "/usr/local/FoxitiCP/conf/token_env"
...
"""
    ...
    def install_default_keys(self):
        try:
            ...
            command = "ssh-keygen -f /root/.ssh/foxitipanel -t rsa -N ''"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def test_Requests(self):
        try:
            import requests
            getVersion = requests.get('https://foxitipanel.net/version.txt')
            ...
            command = f"pip install{pip_flags} http://mirror.foxitipanel.net/urllib3-1.22.tar.gz"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            command = f"pip install{pip_flags} http://mirror.foxitipanel.net/requests-2.18.4.tar.gz"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

    def installation_successfull(self):
        print("###################################################################")
        print("                foxitiPanel Successfully Installed                  ")
        print("                                                                   ")
...
"""
    ...
    def setUpFirstAccount():
        try:
            command = 'python /usr/local/FoxitiCP/plogical/adminPass.py --password 1234567'
            subprocess.call(shlex.split(command))
        except:
            pass
    ...
    def installRestic(self):
        try:
            ...
            if os.path.exists(CentOSPath) or os.path.exists(openEulerPath):
                ...
                # Skip apt-get update as it was already done in foxitipanel.sh
...
"""
    ...
    def installDNS_foxitiPanelACMEFile(self):

        os.chdir(self.cwd)

        filePath = '/root/.acme.sh/dns_foxitipanel.sh'
        shutil.copy('dns_foxitipanel.sh', filePath)
    ...
    def startDeferredServices(self):
        ...
        if os.path.exists('/home/foxitipanel/powerdns'):
        ...
        if os.path.exists('/home/foxitipanel/pureftpd'):
...
def configure_jwt_secret():
    try:
        sys.path.insert(0, '/usr/local/FoxitiCP')
...
def main():
    ...
    logging.InstallLog.writeToFile("Starting foxitiPanel installation..,10")
    preFlightsChecks.stdOut("Starting foxitiPanel installation..")
    ...
    try:
        os.mkdir("/etc/foxitipanel")
    except:
        pass
    os.chmod("/etc/foxitipanel", 0o755)

    machineIP = open("/etc/foxitipanel/machineIP", "w")
    machineIP.writelines(args.publicip)
    machineIP.close()
    os.chmod("/etc/foxitipanel/machineIP", 0o644)
    ...
    distro = get_distro()
    checks = preFlightsChecks("/usr/local/lsws/", args.publicip, "/usr/local", cwd, "/usr/local/FoxitiCP", distro,
    ...
    checks.setup_account_foxitipanel()
    checks.installfoxitiPanelRepo()

    import installfoxitiPanel

    if distro == ubuntu:
        checks.setupLSCPDAccount()

    if ent == 0:
        installfoxitiPanel.Main(cwd, mysql, distro, ent, None, port, args.ftp, args.powerdns, args.publicip, remotemysql,
                               mysqlhost, mysqldb, mysqluser, mysqlpassword, mysqlport)
    else:
        installfoxitiPanel.Main(cwd, mysql, distro, ent, serial, port, args.ftp, args.powerdns, args.publicip,
                               remotemysql, mysqlhost, mysqldb, mysqluser, mysqlpassword, mysqlport)
    ...
    if args.postfix is None:
        ...
        checks.setup_email_Passwords(installfoxitiPanel.InstallfoxitiPanel.mysqlPassword, mysql)
        checks.setup_postfix_dovecot_config(mysql)
        installfoxitiPanel.InstallfoxitiPanel.setupWebmail()
    else:
        if args.postfix == 'ON':
            checks.install_postfix_dovecot()
            checks.setup_email_Passwords(installfoxitiPanel.InstallfoxitiPanel.mysqlPassword, mysql)
    ...
    checks.download_install_foxitiPanel(installfoxitiPanel.InstallfoxitiPanel.mysqlPassword, mysql)
    ...
    checks.installDNS_foxitiPanelACMEFile()
    ...
    checks.fixfoxitiPanelPermissions()
    ...
    logging.InstallLog.writeToFile("foxitiPanel installation successfully completed!,80")
