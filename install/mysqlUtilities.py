...
class mysqlUtilities:
    ...
    def _load_admin_connection():
        ...
        installfoxitiPanel.Main(); local ones store the root password on the
        first line of the same file.
        """
        pass_file = "/etc/foxitipanel/mysqlPassword"
        try:
    ...
    def _connect(params):
        ...
        try:
            ...
            raise MySQLSetupError(
                ...
                "in requirments.txt and must be installed into the foxitiPanel "
    ...
    def createDatabase(dbname, dbuser, dbpassword, publicip):
        """Create the foxitiPanel database and its application account.
        ...
        try:
            ...
            try:
                ...
                if remote:
                    ...
                    # for the client version shipped with foxitiPanel.
            ...
            mysqlUtilities._report(
                "Unexpected failure preparing the foxitiPanel database: %s" % e)
            return 0
