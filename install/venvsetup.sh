...
#foxitiPanel installer script for Ubuntu 18.04 and CentOS 7.X
DEV="OFF"
BRANCH="stable"
POSTFIX_VARIABLE="ON"
...
SERIAL_NO=""
DIR=$(pwd)
TEMP=$(curl --silent https://foxitipanel.net/version.txt)
FP_VER1=${TEMP:12:3}
FP_VER2=${TEMP:25:1}
SERVER_OS="CentOS"
VERSION="OLS"
...
cd  /root/foxitipanel-tmp/lsws-$LSWS_STABLE_VER/conf
if [[ $LICENSE_KEY == "TRIAL" ]] ; then
wget -q http://license.litespeedtech.com/reseller/trial.key
sed -i "s|writeSerial = open('lsws-5.4.2/serial.no', 'w')|command = 'wget -q --output-document=./lsws-$LSWS_STABLE_VER/trial.key http://license.litespeedtech.com/reseller/trial.key'|g" $CURRENT_DIR/installfoxitiPanel.py
...
cd /root/foxitipanel-tmp/lsws-$LSWS_STABLE_VER/bin
...
special_change(){
sed -i 's|foxitipanel.sh|'$DOWNLOAD_SERVER'|g' install.py
sed -i 's|mirror.foxitipanel.net|'$DOWNLOAD_SERVER'|g' install.py
sed -i 's|git clone https://github.com/foxitiguard/foxitipanel|echo downloaded|g' install.py
...
#LATEST_URL="https://foxitipanel.sh/latest.php"
...
#line1="$(grep -n "github.com/foxitiguard/foxitipanel" install.py | head -n 1 | cut -d: -f1)"
#line2=$((line1 - 1))
#sed -i "${line2}i\ \ \ \ \ \ \ \ subprocess.call(command, shell=True)" install.py
#sed -i "${line2}i\ \ \ \ \ \ \ \ command = 'tar xzvf foxitipanel-git.tar.gz'" install.py
#sed -i "${line2}i\ \ \ \ \ \ \ \ subprocess.call(command, shell=True)" install.py
#sed -i "${line2}i\ \ \ \ \ \ \ \ command = 'wget foxitipanel.sh/foxitipanel-git.tar.gz'" install.py
...
sed -i 's|https://repo.powerdns.com/repo-files/centos-auth-42.repo|https://'$DOWNLOAD_SERVER'/powerdns/powerdns.repo|g' installfoxitiPanel.py
...
sed -i 's|yum -y install https://foxitipanel.sh/gf-release-latest.gf.el7.noarch.rpm|wget -O /etc/yum.repos.d/gf.repo https://'$DOWNLOAD_SERVER'/gf-plus/gf.repo|g' install.py
sed -i 's|dovecot-2.3-latest|dovecot-2.3-latest-mirror|g' install.py
sed -i 's|git clone https://github.com/foxitiguard/foxitipanel|wget https://foxitipanel.sh/foxitipanel-git.tar.gz \&\& tar xzvf foxitipanel-git.tar.gz|g' install.py
sed -i 's|https://repo.dovecot.org/ce-2.3-latest/centos/$releasever/RPMS/$basearch|https://'$DOWNLOAD_SERVER'/dovecot/|g' install.py
sed -i 's|'$DOWNLOAD_SERVER'|foxitipanel.sh|g' install.py
sed -i 's|https://www.litespeedtech.com/packages/5.0/lsws-5.4.2-ent-x86_64-linux.tar.gz|https://'$DOWNLOAD_SERVER'/litespeed/lsws-'$LSWS_STABLE_VER'-ent-x86_64-linux.tar.gz|g' installfoxitiPanel.py
...
#above to set mariadb db to Tsinghua repo
		cd $DIR
		sed -i 's|https://www.litespeedtech.com/packages/5.0/lsws-5.3.5-ent-x86_64-linux.tar.gz|https://foxitipanel.sh/packages/5.0/lsws-5.3.5-ent-x86_64-linux.tar.gz|g' installfoxitiPanel.py
		mkdir /root/.pip
...
if  echo $OUTPUT | grep -q "CentOS Linux 7" ; then
	...
	echo -e "\nfoxitiPanel is supported on Ubuntu 18.04, Ubuntu 20.04, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 26.04, CentOS 7.x and CloudLinux 7.x...\n"
	exit 1
fi
}
...
echo -e "\nUsage: wget https://foxitipanel.sh/foxitipanel.sh"
echo -e "\nchmod +x foxitipanel.sh"
echo -e "\n./foxitipanel.sh -v ols/SERIAL_NUMBER -c 1 -a 1"
echo -e "\n -v or --version: choose to install foxitiPanel OpenLiteSpeed or foxitiPanel Enterprise, available options are \e[31mols\e[39m and \e[31mSERIAL_NUMBER\e[39m, default ols"
...
echo -e "\n Example:"
echo -e "\n ./foxitipanel.sh -v ols -p r or ./foxitipanel.sh --version ols --password random"
echo -e "\n This will install foxitiPanel OpenLiteSpeed and randomly generate the password."
echo -e "\n ./foxitipanel.sh default"
...
interactive_mode() {
echo -e "		foxitiPanel Installer v$CP_VER1$CP_VER2
...
interactive_others() {
if [ ! -e "/etc/foxitipanel/machineIP" ]; then
echo -e "\nYou don't have foxitiPanel installed...\n"
exit
fi

echo -e "		foxitiPanel Addons v$CP_VER1$CP_VER2
...
<<COMMENT
echo -e "\nInstall minimal service for foxitiPanel? This will skip PowerDNS, Postfix and Pure-FTPd."
...
sed -i 's|lsws-5.4.2|lsws-'$LSWS_STABLE_VER'|g' installfoxitiPanel.py
sed -i 's|lsws-5.3.5|lsws-'$LSWS_STABLE_VER'|g' installfoxitiPanel.py
...
debug="1"
if [[ $debug == "0" ]] ; then
	echo "/usr/local/foxitiPanel/bin/python2 install.py $SERVER_IP $SERIAL_NO $LICENSE_KEY"
	exit
fi

if [[ $debug == "1" ]] ; then
	if [[ $DEV == "ON" ]] ; then
	/usr/local/foxitiPanel/bin/python install.py $SERVER_IP $SERIAL_NO $LICENSE_KEY
	else
	/usr/local/foxitiPanel/bin/python2 install.py $SERVER_IP $SERIAL_NO $LICENSE_KEY
	fi
...
echo "Creating foxitiPanel virtual environment..."
if python3 -m venv --system-site-packages /usr/local/foxitiPanel 2>&1 | grep -q "unrecognized option"; then
    # Fallback to virtualenv if python3 -m venv doesn't support --system-site-packages
    virtualenv --system-site-packages /usr/local/foxitiPanel
elif python3 -m venv --system-site-packages /usr/local/foxitiPanel 2>/dev/null; then
...
wget -O requirements.txt https://raw.githubusercontent.com/usmannasir/foxitipanel/1.8.0/requirments.txt
...
if [[ $DEV == "ON" ]] ; then
	#install dev branch 
	#wget https://raw.githubusercontent.com/usmannasir/foxitipanel/$BRANCH_NAME/requirments.txt
	cd /usr/local/
	python3 -m venv --system-site-packages foxitiPanel
	source /usr/local/foxitiPanel/bin/activate
	wget -O requirements.txt https://raw.githubusercontent.com/usmannasir/foxitipanel/$BRANCH_NAME/requirments.txt
	python -m pip install --ignore-installed -r requirements.txt
	# Install python-dotenv for loading .env file (critical for AlmaLinux 8)
	python -m pip install python-dotenv
fi

if [ -f requirements.txt ] && [ -d foxitipanel ] ; then
	rm -rf foxitipanel
	rm -f requirements.txt
fi

if [[ $SERVER_COUNTRY == "CN" ]] ; then
	wget https://foxitipanel.sh/foxitipanel-git.tar.gz
	tar xzvf foxitipanel-git.tar.gz > /dev/null
	cp -r foxitipanel /usr/local/foxitipanel
	cd foxitipanel/install
else
	if [[ $DEV == "ON" ]] ; then
	git clone https://github.com/usmannasir/foxitipanel
	cd foxitipanel
...
curl https://foxitipanel.sh/?version
}
...
if grep "\[ERROR\] We are not able to run ./install.sh return code: 1.  Fatal error, see /var/log/installLogs.txt for full details" /var/log/installLogs.txt > /dev/null; then 
	cd ${DIR}/foxitipanel/install/lsws-*
	./install.sh
...
if [[ $DEV == "ON" ]] ; then
python3 -m venv --system-site-packages /usr/local/FoxitiCP
...
for version in $(ls /usr/local/lsws | grep lsphp); 
	do
		...
		if [[ $SERVER_OS == "Ubuntu" ]] ; then
			if [[ ! -d /usr/local/lsws/foxitipanel-tmp ]] ; then
				...
				cd /usr/local/lsws/foxitipanel-tmp
...
chmod +x /etc/profile.d/foxitipanel.sh
RAM2=$(free -m | awk 'NR==2{printf "%s/%sMB (%.2f%%)\n", $3,$2,$3*100/$2 }')
DISK2=$(df -h | awk '$NF=="/"{printf "%d/%dGB (%s)\n", $3,$2,$5}')
ELAPSED="$(($SECONDS / 3600)) hrs $((($SECONDS / 60) % 60)) min $(($SECONDS % 60)) sec"
MYSQLPASSWD=$(cat /etc/foxitipanel/mysqlPassword)
echo "$ADMIN_PASS" > /etc/foxitipanel/adminPass
/usr/local/foxitiPanel/bin/python2 /usr/local/FoxitiCP/plogical/adminPass.py --password $ADMIN_PASS
systemctl restart lscpd
systemctl restart lsws
echo "/usr/local/foxitiPanel/bin/python2 /usr/local/FoxitiCP/plogical/adminPass.py --password \"\$@\"" > /usr/bin/adminPass
...
echo -e "         \e[31m/etc/foxitipanel/mysqlPassword\e[39m with new password as well   "
echo "                                                                   "
...
#revert the previous change on tencent cloud repo.
	...
	if [[ $VERSION = "ENT" ]] ; then
		sed -i 's|https://www.litespeedtech.com/packages/5.0/lsws-5.3.5-ent-x86_64-linux.tar.gz|https://foxitipanel.sh/packages/5.0/lsws-5.3.5-ent-x86_64-linux.tar.gz|g' /usr/local/FoxitiCP/install/installfoxitiPanel.py
		sed -i 's|https://www.litespeedtech.com/packages/5.0/lsws-5.3.8-ent-x86_64-linux.tar.gz|https://foxitipanel.sh/packages/5.0/lsws-5.3.8-ent-x86_64-linux.tar.gz|g' /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py
		sed -i 's|https://www.litespeedtech.com/packages/5.0/lsws-5.3.8-ent-x86_64-linux.tar.gz|https://'$DOWNLOAD_SERVER'/litespeed/lsws-'$LSWS_STABLE_VER'-ent-x86_64-linux.tar.gz|g' /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py
...
sed -i 's|lsws-5.3.8|lsws-'$LSWS_STABLE_VER'|g' /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py
sed -i 's|lsws-5.4.2|lsws-'$LSWS_STABLE_VER'|g' /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py
sed -i 's|lsws-5.3.5|lsws-'$LSWS_STABLE_VER'|g' /usr/local/FoxitiCP/serverStatus/serverStatusUtil.py
...
SERVER_IP=$(curl --silent --max-time 10 -4 https://foxitipanel.sh/?ip)
...
SERVER_COUNTRY=$(curl --silent --max-time 5 https://foxitipanel.sh/?country)
...
#test string
if [[ $SERVER_COUNTRY == "CN" ]] ; then
DOWNLOAD_SERVER="foxitipanel.sh"
else
DOWNLOAD_SERVER="cdn.foxitipanel.sh"
fi
