...
BOOTSTRAP = INSTALLER.parents[1] / 'foxitipanel.sh'
...
class LiteSpeedEL10KeyTests(unittest.TestCase):
    ...
    def test_el10_skips_removed_mariadb_module_commands(self):
        tree = ast.parse(
            (INSTALLER.with_name('installfoxitiPanel.py')).read_text()
        )
        installer_class = next(
            node for node in tree.body
            if isinstance(node, ast.ClassDef) and node.name == 'InstallfoxitiPanel'
        )
        method = next(
            node for node in installer_class.body
        ...
        source = ast.get_source_segment(
            INSTALLER.with_name('installfoxitiPanel.py').read_text(), method
        )
