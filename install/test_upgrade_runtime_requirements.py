...
SOURCE = (ROOT / 'foxitipanel_upgrade.sh').read_text()
...
class UpgradeRuntimeRequirementsTests(unittest.TestCase):
    def setUp(self):
        ...
        self.environment = dict(os.environ, PATH=str(self.commands)+':/usr/bin:/bin',
                                ...
                                foxitiPanel_Python=str(self.system_runtime),
...
''')
    ...
    def function(self, name):
        ...
        result = SOURCE[start:end].replace('/var/log/foxitipanel_upgrade_debug.log', str(self.log))
        result = result.replace('/tmp/foxitipanel-lswsgi.XXXXXX', str(self.root/'build.XXXXXX'))
        return result

    def run_shell(self, code, **environment):
        definitions = '\n'.join(self.function(name) for name in (
            'Run_Upgrade_Command', 'Validate_Python_Requirements',
            'Install_Panel_Runtime_Requirements', 'Install_FoxitiCP_Runtime_Python_Requirements',
    ...
    def install_system(self, **environment):
        return self.run_shell('Install_FoxitiCP_Runtime_Python_Requirements '+shlex.quote(str(self.requirements)), **environment)
    ...
    def test_main_runtime_failures_survive_later_successful_repair_steps(self):
        ...
        tail=main[start:]
        tail=tail.replace('/usr/local/FoxitiCP/bin/python',str(self.runtime)).replace('/usr/local/FoxitiCP/bin/lswsgi',str(self.root/'lswsgi')).replace('/usr/local/requirments.txt',str(self.requirements))
...
'''+tail+'\nUPGRADE_FAILED=0\nMain_Upgrade\n'
    ...
    def test_failure_banner_cannot_claim_success_even_when_panel_answers(self):
        ...
        banner=banner.replace('/usr/local/lscp/conf/bind.conf',str(bind)).replace('/root/foxitipanel_upgrade_tmp',str(self.root/'unused-upgrade-temp'))
        for failed,http in ((1,200),(0,500)):
            with self.subTest(failed=failed,http=http):
                ...
                self.assertNotEqual(0,result.returncode)
                self.assertNotIn('foxitiPanel Upgraded',result.stdout)
...
'''+tail
    ...
    def test_system_callers_reject_stale_requirements_and_preserve_failures(self):
        ...
        cached=cache_directory/'foxitipanel-requirments-runtime.txt'
        python_environment=self.root/'pythonenv.conf'
        python_environment.write_text('PYTHONHOME=/usr\n')
        selected=selected.replace('/etc/foxitipanel',str(cache_directory)).replace('/usr/local/requirments.txt',str(self.requirements)).replace('/usr/local/lscp/conf/pythonenv.conf',str(python_environment))
