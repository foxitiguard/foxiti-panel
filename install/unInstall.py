...
class unInstallfoxitiPanel:

    def unInstallfoxitiPanelRepo(self):

        try:
            copyPath = "/etc/yum.repos.d/foxitipanel.repo"
            os.remove(copyPath)

        except OSError as msg:
            logging.InstallLog.writeToFile(f"{str(msg)} [unInstallfoxitiPanelRepo]")
    ...
    def removefoxitiPanel(self):
        try:

           shutil.rmtree("/usr/local/FoxitiCP")
           os.remove("/usr/local/FoxitiCP2.tar.gz")
           shutil.rmtree("/etc/foxitipanel")

        except OSError as msg:
            logging.InstallLog.writeToFile(str(msg) + " [removefoxitiPanel]")
            return 0
        except ValueError as msg:
            logging.InstallLog.writeToFile(str(msg) + " [removefoxitiPanel]")
            return 0
