...
class PhpMyAdminSessionSetupTests(unittest.TestCase):
        ...
        for relative in ('install/install.py', 'plogical/upgrade.py'):
            with self.subTest(relative=relative):
                ...
                begin = next(i for i, n in enumerate(statements) if isinstance(n, ast.Assign)
                           ...
                           and n.value.value.startswith('cp /usr/local/FoxitiCP/plogical/phpmyadminsession.php '))
                with tempfile.TemporaryDirectory() as temporary:
                    target = Path(temporary)
                    prefix = '/usr/local/FoxitiCP/public/phpmyadmin/'
                    ...
                    def command(command, *args):
                        if command.startswith('chown -R lscpd:lscpd '):
                            self.assertEqual('chown -R lscpd:lscpd /usr/local/FoxitiCP/public/phpmyadmin', command)
                            return
                        ...
                        self.assertEqual('/usr/local/FoxitiCP/plogical/' + name, fields[1])
    ...
    def test_remote_host_port_rendering_preserves_new_session_binding(self):
        ...
        self.assertIn("$_SESSION['PMA_panel_session'] = $_COOKIE['foxitipanel_sessionid']", rendered)
