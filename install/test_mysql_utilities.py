...
class CreateDatabaseTests(unittest.TestCase):
    ...
    def test_unreachable_admin_connection_fails_and_explains_itself(self):
        ...
        result = mysqlUtilities.createDatabase('foxitipanel', 'foxitipanel',
                                               'pw', '203.0.113.4')
    ...
    def test_password_is_bound_never_interpolated(self):
        cursor = FakeCursor()
        self._with_connection(cursor)
        mysqlUtilities.createDatabase('foxitipanel', 'foxitipanel',
                                      "p'w#1 $x", '203.0.113.4')
    ...
    def test_no_shell_and_no_password_in_argv(self):
        ...
        with mock.patch.object(mu.subprocess, 'call') as call, \
            ...
            mysqlUtilities.createDatabase('foxitipanel', 'foxitipanel', 'pw',
                                          '203.0.113.4')
    ...
    def test_unsafe_identifiers_are_refused(self):
        for name, user in (('cyber panel', 'foxitipanel'),
                           ('foxitipanel', 'user`; DROP'),
                           ('cyber`panel', 'foxitipanel'),
                           ('', 'foxitipanel')):
            self.reported[:] = []
    ...
    def test_rerun_refreshes_the_existing_application_password(self):
        ...
        self.assertIn(
            ('foxitipanel', '203.0.113.4', 'NewAppPw456'), changes)
