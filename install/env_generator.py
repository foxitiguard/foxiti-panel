#!/usr/bin/env python3
"""
foxitiPanel Environment Configuration Generator
...
def format_env_value(value):
    ...
    text = text.replace('${', '${FOXITIPANEL_LITERAL_DOLLAR:-$}{')
    escaped = (
        text.replace('\\', '\\\\')
            .replace('"', '\\"')
            .replace('\n', '\\n')
...
def create_env_file(foxitipanel_path, mysql_root_password=None, foxitipanel_db_password=None,
                    database_config=None):
    """
    Create .env file with generated secure credentials
    
    Args:
        foxitipanel_path: Path to foxitiPanel installation directory
        mysql_root_password: Optional MySQL root password (will generate if None)
        foxitipanel_db_password: Optional foxitiPanel DB password (will generate if None)
    ...
    if not foxitipanel_db_password:
        foxitipanel_db_password = generate_secure_password(24)
    ...
    # This ensures foxitiPanel works regardless of how the server is accessed
    ...
    # Create .env content
    env_content = f"""# foxitiPanel Environment Configuration
...
"""
    ...
    # Write .env file
    env_file_path = os.path.join(foxitipanel_path, '.env')
    ...
    try:
        import shutil
        shutil.chown(env_file_path, user='root', group='foxitipanel')
    ...
    return {
        ...
        'foxitipanel_db_password': foxitipanel_db_password,
        'secret_key': secret_key
    }

def create_env_backup(foxitipanel_path, credentials):
    """
    ...
    """
    backup_content = f"""# foxitiPanel Credentials Backup
...
foxitiPanel Database Password: {credentials['foxitipanel_db_password']}
Django Secret Key: {credentials['secret_key']}

# To restore these credentials, copy them to your .env file
"""
    
    backup_file_path = os.path.join(foxitipanel_path, '.env.backup')
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python env_generator.py <foxitipanel_path> [mysql_root_password] [foxitipanel_db_password]")
        sys.exit(1)
    
    foxitipanel_path = sys.argv[1]
    mysql_root_password = sys.argv[2] if len(sys.argv) > 2 else None
    foxitipanel_db_password = sys.argv[3] if len(sys.argv) > 3 else None
    
    if not os.path.exists(foxitipanel_path):
        print(f"Error: foxitiPanel path does not exist: {foxitipanel_path}")
        sys.exit(1)
    
    try:
        credentials = create_env_file(foxitipanel_path, mysql_root_password, foxitipanel_db_password)
        create_env_backup(foxitipanel_path, credentials)
