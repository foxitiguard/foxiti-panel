...
import installfoxitiPanel  # noqa: E402
...
class RemoteClientPackageTests(unittest.TestCase):
    def _installer(self, distro):
        return installfoxitiPanel.InstallfoxitiPanel(
    ...
    def test_remote_ubuntu_installs_only_the_client(self):
        installer = self._installer(installfoxitiPanel.ubuntu)
        with mock.patch.object(installfoxitiPanel.install_utils, 'call') as call:
    ...
    def test_remote_rhel_installs_only_the_client(self):
        installer = self._installer(installfoxitiPanel.cent8)
        with mock.patch.object(installfoxitiPanel.install_utils, 'call') as call:
            installer.installMySQL('One')
    ...
    def test_phpmyadmin_setup_does_not_interpolate_host_into_shell(self):
        installer_source = pathlib.Path(
            installfoxitiPanel.__file__).with_name('install.py').read_text(
                encoding='utf-8')
