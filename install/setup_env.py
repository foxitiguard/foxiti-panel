#!/usr/bin/env python3
"""
Environment setup script for foxitiPanel
...
def setup_env_file(foxitipanel_root="/usr/local/FoxitiCP"):
    """Set up the .env file with auto-detected IP addresses"""
    env_template_path = os.path.join(foxitipanel_root, ".env.template")
    env_path = os.path.join(foxitipanel_root, ".env")
...
if __name__ == "__main__":
    ...
    foxitipanel_root = sys.argv[1] if len(sys.argv) > 1 else "/usr/local/FoxitiCP"

    success = setup_env_file(foxitipanel_root)
