...
class InstallfoxitiPanel:
    ...
    def install_package(self, package_name, options=""):
        ...
        # InstallfoxitiPanel always uses verbose mode (no silent option)
    ...
    def OSFlags():
        if os.path.exists("/etc/redhat-release"):
            ...
            if data.find('CloudLinux 8') > -1 or data.find('cloudlinux 8') > -1:
                InstallfoxitiPanel.CloudLinux8 = 1
    ...
    def getLatestLSWSVersion():
        ...
        try:
            ...
            if versions:
                ...
                InstallfoxitiPanel.stdOut(f"Found latest LSWS Enterprise version: {latest_version}", 1)
                return latest_version
            else:
                InstallfoxitiPanel.stdOut("Could not find version pattern in HTML, using fallback", 1)

        except Exception as e:
            InstallfoxitiPanel.stdOut(f"Failed to fetch latest LSWS version: {str(e)}, using fallback", 1)
    ...
    def detectPlatform(self):
        ...
        try:
            ...
            InstallfoxitiPanel.stdOut("WARNING: Could not detect platform, defaulting to rhel9", 1)
            return 'rhel9'

        except Exception as msg:
            logging.InstallLog.writeToFile(str(msg) + " [detectPlatform]")
            InstallfoxitiPanel.stdOut(f"ERROR detecting platform: {msg}, defaulting to rhel9", 1)
            return 'rhel9'

    def downloadCustomBinary(self, url, destination):
        """Download custom binary file"""
        try:
            InstallfoxitiPanel.stdOut(f"Downloading {os.path.basename(destination)}...", 1)
            ...
            if os.path.exists(destination):
                ...
                if file_size > 10240:  # 10KB
                    if file_size > 1048576:  # 1MB
                        InstallfoxitiPanel.stdOut(f"Downloaded successfully ({file_size / (1024*1024):.2f} MB)", 1)
                    else:
                        InstallfoxitiPanel.stdOut(f"Downloaded successfully ({file_size / 1024:.2f} KB)", 1)

                    return True
                else:
                    InstallfoxitiPanel.stdOut(f"ERROR: Downloaded file too small ({file_size} bytes)", 1)
                    return False
            else:
                InstallfoxitiPanel.stdOut("ERROR: Download failed - file not found", 1)
                return False

        except Exception as msg:
            logging.InstallLog.writeToFile(str(msg) + " [downloadCustomBinary]")
            InstallfoxitiPanel.stdOut(f"ERROR: {msg}", 1)
            return False
    ...
    def verifyChecksum(self, file_path, expected_sha256):
        ...
        try:
            ...
            if actual.lower() == expected_sha256.lower():
                InstallfoxitiPanel.stdOut(f"SHA256 verified: {os.path.basename(file_path)}", 1)
                return True
            InstallfoxitiPanel.stdOut(f"ERROR: SHA256 mismatch for {os.path.basename(file_path)}", 1)
            InstallfoxitiPanel.stdOut(f"  expected: {expected_sha256}", 1)
            InstallfoxitiPanel.stdOut(f"  actual:   {actual}", 1)
            return False
        except Exception as msg:
            logging.InstallLog.writeToFile(str(msg) + " [verifyChecksum]")
            InstallfoxitiPanel.stdOut(f"ERROR: {msg} [verifyChecksum]", 1)
            return False
    ...
    def checkGlibcCompat(self, binary_path):
        ...
        try:
            ...
            if 'not found' in output:
                InstallfoxitiPanel.stdOut("ERROR: Downloaded binary has unresolved libraries (incompatible with this OS):", 1)
                for line in output.splitlines():
                    if 'not found' in line:
                        InstallfoxitiPanel.stdOut(f"  {line.strip()}", 1)
                return False
            return True
        except Exception as msg:
            InstallfoxitiPanel.stdOut(f"WARNING: Could not run ldd pre-check ({msg}); continuing", 1)
            return True

    def installCustomOLSBinaries(self):
        """Install custom OpenLiteSpeed binaries with PHP config support"""
        try:
            InstallfoxitiPanel.stdOut("Installing Custom OpenLiteSpeed Binaries", 1)
            InstallfoxitiPanel.stdOut("=" * 50, 1)

            # Check architecture
            if not self.detectArchitecture():
                InstallfoxitiPanel.stdOut("WARNING: Custom binaries only available for x86_64", 1)
                InstallfoxitiPanel.stdOut("Skipping custom binary installation", 1)
                InstallfoxitiPanel.stdOut("Standard OLS will be used", 1)
                return True  # Not a failure, just skip

            # Detect platform
            platform = self.detectPlatform()
            InstallfoxitiPanel.stdOut(f"Detected platform: {platform}", 1)
            ...
            if platform == 'skip':
                InstallfoxitiPanel.stdOut("Custom binary installation skipped for this platform; using standard OLS", 1)
                return True  # Not a failure, just skip
            ...
            if not config:
                InstallfoxitiPanel.stdOut(f"ERROR: No binaries available for platform {platform}", 1)
                InstallfoxitiPanel.stdOut("Skipping custom binary installation", 1)
            ...
            with tempfile.TemporaryDirectory(prefix='foxitipanel-ols-') as staging:
                ...
                for kind, url, target, mode, backup_name in required:
                    ...
                    if (not url or not self.downloadCustomBinary(url, candidate) or
                        ...
                        InstallfoxitiPanel.stdOut('ERROR: Required %s failed download, checksum or ABI verification; keeping the existing OLS bundle' % kind, 1)
                        return False
                ...
                try:
                    ...
                    InstallfoxitiPanel.stdOut('Custom OLS bundle installed; backup: ' + backup_dir, 1)
                    return True
                except Exception as error:
                    InstallfoxitiPanel.stdOut('ERROR: %s; restoring the previous OLS bundle' % error, 1)
                    try:
                        if touched:
                        ...
                        InstallfoxitiPanel.stdOut('ERROR: OLS rollback requires attention: %s; backup: %s' %
                                      (rollback_error, backup_dir), 1)
                    return False
        except Exception as error:
            InstallfoxitiPanel.stdOut('ERROR: Custom OLS overlay aborted: %s; existing bundle retained' % error, 1)
            return False

    def configureCustomModule(self):
        """Configure foxitiPanel module in OpenLiteSpeed config"""
        try:
            InstallfoxitiPanel.stdOut("Configuring foxitiPanel module...", 1)

            CONFIG_FILE = "/usr/local/lsws/conf/httpd_config.conf"

            if not os.path.exists(CONFIG_FILE):
                InstallfoxitiPanel.stdOut("WARNING: Config file not found", 1)
                InstallfoxitiPanel.stdOut("Module will be auto-loaded", 1)
                return True
            ...
            if 'foxitipanel_ols' in content:
                ...
                if new_content != content:
                    ...
                    with open(CONFIG_FILE, 'w') as f:
                        f.write(new_content)
                    InstallfoxitiPanel.stdOut("Module was disabled (ls_enabled 0); re-enabled LSCache module", 1)
                else:
                    InstallfoxitiPanel.stdOut("Module already configured", 1)
                return True
...
"""
            ...
            InstallfoxitiPanel.stdOut("Module configured successfully", 1)
            return True

        except Exception as msg:
            logging.InstallLog.writeToFile(str(msg) + " [configureCustomModule]")
            InstallfoxitiPanel.stdOut(f"WARNING: Module configuration failed: {msg}", 1)
            InstallfoxitiPanel.stdOut("Module may still work via auto-load", 1)
    ...
    def installLiteSpeed(self):
        if self.ent == 0:
                ...
                if os.path.exists(conf_path):
                    ...
                    if 'autoSSL' not in content:
                        ...
                        with open(conf_path, 'w') as f:
                            f.write(content)
                        InstallfoxitiPanel.stdOut("Auto-SSL enabled in httpd_config.conf", 1)
            except Exception as e:
                InstallfoxitiPanel.stdOut(f"WARNING: Could not enable Auto-SSL: {e}", 1)
            ...
            try:
                ...
                lsws_version = InstallfoxitiPanel.getLatestLSWSVersion()

                if InstallfoxitiPanel.ISARM():
                ...
                if self.serial == '1111-2222-3333-4444':
                    command = f'wget -q --output-document=/root/foxitipanel/install/lsws-{lsws_version}/trial.key http://license.litespeedtech.com/reseller/trial.key'
                    install_utils.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
                else:
    ...
    def fix_ols_configs(self):
        try:
            ...
            InstallfoxitiPanel.stdOut("Fixing OpenLiteSpeed configurations!", 1)
            ...
            InstallfoxitiPanel.stdOut("OpenLiteSpeed Configurations fixed!", 1)
    ...
    def changePortTo80(self):
        try:
            InstallfoxitiPanel.stdOut("Changing default port to 80..", 1)

            file_path = self.server_root_path + "conf/httpd_config.conf"
            if self.modify_file_content(file_path, {"*:8088": "*:80"}):
                InstallfoxitiPanel.stdOut("Default port is now 80 for OpenLiteSpeed!", 1)
            else:
                return 0
    ...
    def installAllPHPVersions(self):
        ...
        if self.distro == ubuntu:
            ...
            InstallfoxitiPanel.stdOut("LiteSpeed PHPs successfully installed!", 1)
    ...
    def installSieve(self):
        ...
        try:
            InstallfoxitiPanel.stdOut("Installing Sieve (Dovecot Sieve) for email filtering...", 1)
...
""")
            ...
            InstallfoxitiPanel.stdOut("Sieve successfully installed and configured!", 1)
            return 1
    ...
    def setupWebmail():
        ...
        try:
            ...
            if not os.path.exists('/etc/dovecot/dovecot.conf'):
                InstallfoxitiPanel.stdOut("Dovecot not installed, skipping webmail setup.", 1)
                return 1

            # Skip if already configured
            if os.path.exists('/etc/foxitipanel/webmail.conf') and os.path.exists('/etc/dovecot/master-users'):
                InstallfoxitiPanel.stdOut("Webmail master user already configured.", 1)
                return 1

            InstallfoxitiPanel.stdOut("Setting up webmail master user for SSO...", 1)
            ...
            # Ensure /etc/foxitipanel/ exists
            os.makedirs('/etc/foxitipanel', exist_ok=True)
...
"""
            ...
            InstallfoxitiPanel.stdOut("Webmail master user setup complete!", 1)
            return 1
...
"""
            ...
            # foxitiPanel never installs (the RHEL path below disables MaxScale for the
...
"""
            ...
            if self.distro == ubuntu:
                if get_Ubuntu_release() >= 26.0:
                    passwordCMD = "DROP DATABASE IF EXISTS test;DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%%';ALTER USER 'root'@'localhost' IDENTIFIED VIA mysql_native_password USING PASSWORD('%s');GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost' WITH GRANT OPTION;flush privileges;" % (
                        InstallfoxitiPanel.mysql_Root_password)
                else:
    ...
    def installPureFTPD(self):
        if self.distro == ubuntu:
            ...
            if get_Ubuntu_release() == 18.10:
                ...
                packages = [
                    ('pure-ftpd-common_1.0.47-3_all.deb', 'wget https://rep.foxitipanel.net/pure-ftpd-common_1.0.47-3_all.deb'),
                    ('pure-ftpd-mysql_1.0.47-3_amd64.deb', 'wget https://rep.foxitipanel.net/pure-ftpd-mysql_1.0.47-3_amd64.deb')
                ]
    ...
    def startPureFTPD(self):
        ...
        InstallfoxitiPanel.stdOut("Pure-FTPd enabled for startup.", 1)
        InstallfoxitiPanel.stdOut("Note: Pure-FTPd will start after database setup is complete.", 1)
    ...
    def installPureFTPDConfigurations(self, mysql):
        try:
            ...
            InstallfoxitiPanel.stdOut("Configuring PureFTPD..", 1)
            ...
            dataWritten = "MYSQLPassword " + InstallfoxitiPanel.mysqlPassword + '\n'
            for items in data:
            ...
            InstallfoxitiPanel.stdOut("PureFTPD configured!", 1)
            ...
            if (self.distro == ubuntu or self.distro == cent8 or self.distro == openeuler) and not keep_resolved:
                ...
                try:
                    ...
                    if e.errno != errno.EEXIST and e.errno != errno.ENOENT:
                        InstallfoxitiPanel.stdOut("[ERROR] Unable to rename /etc/resolv.conf to install PowerDNS: " +
                ...
                try:
                    ...
                    InstallfoxitiPanel.stdOut("Created temporary /etc/resolv.conf with Google DNS", 1)
                except IOError as e:
                    InstallfoxitiPanel.stdOut("[ERROR] Unable to create /etc/resolv.conf: " + str(e), 1, 1, os.EX_OSERR)
    ...
    def installPowerDNSConfigurations(self, mysqlPassword, mysql):
        try:
            ...
            InstallfoxitiPanel.stdOut("Configuring PowerDNS..", 1)
            ...
            try:
                ...
                InstallfoxitiPanel.stdOut("[ERROR] Failed to copy PowerDNS config: " + str(e), 1)
            ...
            with open(dnsPath, "r") as f:
                ...
                if not content or "launch=gmysql" not in content:
                    InstallfoxitiPanel.stdOut("[WARNING] PowerDNS config appears empty or incomplete, attempting to fix...", 1)
                    ...
                    try:
                        ...
                        InstallfoxitiPanel.stdOut("[WARNING] Failed to re-copy config: " + str(copy_error), 1)
                        
                        # Fallback: directly write the essential MySQL configuration
                        InstallfoxitiPanel.stdOut("[INFO] Directly writing MySQL backend configuration...", 1)
                        try:
                            mysql_config = f"""# PowerDNS MySQL Backend Configuration
launch=gmysql
gmysql-host=localhost
gmysql-port=3306
gmysql-user=foxitipanel
gmysql-password={mysqlPassword}
gmysql-dbname=foxitipanel
...
"""
                            ...
                            InstallfoxitiPanel.stdOut("[SUCCESS] MySQL backend configuration written directly", 1)
                        except Exception as write_error:
                            InstallfoxitiPanel.stdOut("[ERROR] Failed to write MySQL config: " + str(write_error), 1)
                            raise
            
            InstallfoxitiPanel.stdOut("PowerDNS config file prepared at: " + dnsPath, 1)
            ...
            InstallfoxitiPanel.stdOut("PowerDNS configured!", 1)
    ...
    def startPowerDNS(self):
        ...
        # It will be started after Django migrations create the required tables
        InstallfoxitiPanel.stdOut("PowerDNS enabled for startup.", 1)
        InstallfoxitiPanel.stdOut("Note: PowerDNS will start after database setup is complete.", 1)
...
def Main(cwd, mysql, distro, ent, serial=None, port="8090", ftp=None, dns=None, publicip=None, remotemysql=None,
    ...
    InstallfoxitiPanel.mysqlPassword = install_utils.generate_pass()
    InstallfoxitiPanel.mysql_Root_password = install_utils.generate_pass()

    file_name = '/etc/foxitipanel/mysqlPassword'

    if remotemysql == 'OFF':
        if os.access(file_name, os.F_OK):
            password = open(file_name, 'r')
            InstallfoxitiPanel.mysql_Root_password = password.readline()
            password.close()
        else:
            password = open(file_name, "w")
            password.writelines(InstallfoxitiPanel.mysql_Root_password)
            password.close()
    else:
    ...
    try:
        ...
        install_utils.call(command, distro, '[chmod]',
                                      '',
                                      1, 0, os.EX_OSERR)
        command = 'chown root:foxitipanel %s' % (file_name)
        install_utils.call(command, distro, '[chmod]',
                                      '',
                                      1, 0, os.EX_OSERR)
    except:
    ...
    if distro in [centos, cent8, openeuler]:
        ...
        InstallfoxitiPanel.mysqlPassword = InstallfoxitiPanel.mysql_Root_password

    installer = InstallfoxitiPanel("/usr/local/lsws/", cwd, distro, ent, serial, port, ftp, dns, publicip, remotemysql,
    ...
    if mysqlUtilities.createDatabase("foxitipanel", "foxitipanel",
                                     InstallfoxitiPanel.mysqlPassword,
                                     publicip) != 1:
        logging.InstallLog.writeToFile(
            "[ERROR] Could not create the foxitiPanel database or its "
    ...
    if dns is None:
        ...
        installer.installPowerDNSConfigurations(InstallfoxitiPanel.mysqlPassword, mysql)
