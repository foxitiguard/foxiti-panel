...
class DeveloperInstallerPythonTests(unittest.TestCase):
    ...
    def test_post_install_php_cli_falls_back_without_changing_legacy_priority(self):
        ...
        script = (root / 'foxitipanel.sh').read_text(encoding='utf-8')
    ...
    def test_install_and_upgrade_use_native_webmail_setup(self):
        ...
        self.assertIn(
            'installfoxitiPanel.InstallfoxitiPanel.setupWebmail()', installer
        )
        self.assertIn('Upgrade.setupWebmail()', upgrade)
        self.assertNotIn('snappymail_foxitipanel.php', installer + upgrade)

    def test_upgrade_skips_unavailable_legacy_php_development_package(self):
        root = pathlib.Path(__file__).parents[1]
        upgrade_script = (root / 'foxitipanel_upgrade.sh').read_text(
            encoding='utf-8'
        )
    ...
    def test_version_parsers_do_not_depend_on_fixed_json_offsets(self):
        root = pathlib.Path(__file__).parents[1]
        for script_path in (root / 'foxitipanel.sh', root / 'foxitipanel_upgrade.sh'):
    ...
    def test_upgrade_stages_runtime_version_module_with_upgrader(self):
        ...
        self.assertIn(
            'Download_Upgrade_Source "foxitipanel_version.py" '
            '"foxitipanel_version.py"',
            upgrade_script,
        )
        with tempfile.TemporaryDirectory() as stage_dir:
            ...
            shutil.copy2(
                root / 'foxitipanel_version.py',
                stage / 'foxitipanel_version.py',
            )
            result = subprocess.run(
                [
                    'python3',
            ...
            version = (root / 'foxitipanel_version.py').read_text(
                encoding='utf-8'
            )
            namespace = {}
            exec(compile(version, 'foxitipanel_version.py', 'exec'), namespace)
    ...
    def test_remote_mysql_password_is_not_passed_on_the_command_line(self):
        root = pathlib.Path(__file__).parents[1]
        shell_installer = (root / 'foxitipanel.sh').read_text(encoding='utf-8')
    ...
    def test_settings_fallback_uses_python_literals_for_database_values(self):
        ...
        self.assertNotIn("+ foxitipanel_db_password +", installer)
    ...
    def test_upgrade_virtualenv_uses_matching_branch_requirements(self):
        ...
        self.assertIn(
            'Validate_Python_Requirements /usr/local/foxitiPanel/bin/python '
            '/usr/local/requirments.txt',
            upgrade_script,
        )
        self.assertIn(
            'Validate_Python_Requirements /usr/local/foxitiPanelTemp/bin/python '
            '/usr/local/requirments.txt',
            upgrade_script,
        )
    ...
    def test_ubuntu_24_lscpd_keeps_virtualenv_packages_visible(self):
        root = pathlib.Path(__file__).parents[1]
        script = (root / 'foxitipanel_upgrade.sh').read_text(encoding='utf-8')
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            temporary_root = pathlib.Path(temporary_directory)
            runtime_root = temporary_root / 'FoxitiCP'
    ...
    def test_repair_script_does_not_download_old_release_requirements(self):
        root = pathlib.Path(__file__).parents[1]
        repair_script = (root / 'fix_foxitipanel_install.sh').read_text(
            encoding='utf-8'
        )
        self.assertNotIn('/v2.4.5/requirments', repair_script)
        self.assertIn('/usr/local/FoxitiCP/$requirements_name', repair_script)
    ...
    def test_ubuntu_dependencies_require_installable_pcre_candidate(self):
        root = pathlib.Path(__file__).parents[1]
        for script_path in (root / 'foxitipanel.sh', root / 'install/venvsetup.sh'):
            script = script_path.read_text(encoding='utf-8')
            self.assertIn(
    ...
    def test_memcached_falls_back_when_pcre_one_is_unavailable(self):
        root = pathlib.Path(__file__).parents[1]
        installer = (root / 'foxitipanel.sh').read_text(encoding='utf-8')
    ...
    def test_dns_handoff_waits_for_name_resolution(self):
        ...
        self.assertIn('getent ahostsv4 foxitipanel.sh', installer)
    ...
    def test_panel_credentials_keep_worker_only_permissions(self):
        ...
        self.assertIn("group='foxitipanel'", generator)
        for script in (installer, upgrader):
            self.assertIn("'/usr/local/FoxitiCP/.env'", script)
            self.assertIn("'/usr/local/FoxitiCP/secret_key'", script)
            self.assertIn('chown root:foxitipanel %s', script)
            self.assertIn("backup_env = '/usr/local/FoxitiCP/.env.backup'", script)
    ...
    def test_ubuntu_26_bootstrap_uses_requested_release_branch(self):
        ...
        self.assertIn(
            'raw.githubusercontent.com/usmannasir/foxitipanel/'
            '$INSTALL_BRANCH/foxitipanel.sh',
            bootstrap,
        )
        self.assertIn(
            'else\n        curl --silent -o foxitipanel.sh '
            '"https://foxitipanel.sh/?dl&$SERVER_OS"',
            bootstrap,
        )
