...
class NativeWebmailProvisioningTests(unittest.TestCase):
    ...
    def test_install_and_upgrade_configure_native_webmail(self):
        ...
        self.assertEqual(
            installer.count('installfoxitiPanel.InstallfoxitiPanel.setupWebmail()'),
            2,
        )
    ...
    def test_permission_repairs_do_not_create_legacy_data(self):
        ...
        upgrade_script = self.read('foxitipanel_upgrade.sh')

        self.assertNotIn('mkdir', helper)
        self.assertNotIn('mkdir -p /usr/local/lscp/foxitipanel/snappymail', upgrade_script)
        self.assertIn('if [[ -d "$legacy_data_path" ]]', upgrade_script)

        data_paths = (
            '/usr/local/lscp/foxitipanel/snappymail/data',
            '/usr/local/lscp/foxitipanel/rainloop/data',
        )
        expected_commands = []
