...
class CustomOLSPlatformTests(unittest.TestCase):
    ...
    def test_almalinux_10_release_set_is_abi_matched(self):
        for path, class_name in (
            (self.install_path, 'InstallfoxitiPanel'),
            ...
            with self.subTest(path=path.name):
                ...
                self.assertTrue(config['module_url'].endswith('foxitipanel_ols-2.7.7-x86_64-rhel10.so'))
        ...
        for path, class_name, is_static in cases:
            with self.subTest(path=path.name):
                ...
                self.assertTrue(config['module_url'].endswith(
                    'foxitipanel_ols-2.7.7-x86_64-ubuntu.so'
                ))
        ...
        for path, class_name in (
            ...
            for platform, (core_version, module_version) in expected[path].items():
                with self.subTest(path=path.name, platform=platform):
                    ...
                    self.assertIn(
                        f'foxitipanel_ols-{module_version}-x86_64-{platform}.so',
