#!/usr/bin/env python3
"""Define the design tokens that foxitiPanel templates REFERENCE but that were
...
collects those tokens and emits foxitipanel-tokens.css with:
...
ROOT = "/Users/cyberpersons/foxitipanel"
...
for c in glob.glob(os.path.join(ROOT, "baseTemplate/static/baseTemplate/css/*.css")):
    if c.endswith("foxitipanel-tokens.css"):
        continue
...
out = []
out.append("/* ====================================================================")
out.append("   foxitiPanel Missing Design Tokens  (AUTO-GENERATED — issue #1804)")
out.append("   --------------------------------------------------------------------")
...
dest = os.path.join(ROOT, "baseTemplate/static/baseTemplate/css/foxitipanel-tokens.css")
open(dest, "w").write("\n".join(out) + "\n")
