...
// Using existing FoxitiCP module
app.controller('backupPlanNowOneClick', function($scope, $http) {
    $scope.foxitipanelLoading = true;
    ...
    $scope.sendVerificationCode = function() {
        $scope.foxitipanelLoading = false;
        ...
        $http.post('https://cp.foxitiguard.com/Billing/SendBackupVerificationCode', {
    ...
    $scope.verifyCode = function() {
        ...
        $http.post('https://cp.foxitiguard.com/Billing/VerifyBackupCode', {
            email: $scope.verificationEmail,
            code: $scope.verificationCode
        }, config).then(function(response) {
            if (response.data.status == 1) {
                // After successful verification, fetch Stripe subscriptions
                $http.post('https://cp.foxitiguard.com/Billing/FetchStripeSubscriptionsByEmail', {
                    email: $scope.verificationEmail,
                    code: $scope.verificationCode
    ...
    $scope.fetchBackupPlans = function() {
        ...
        $http.post('https://cp.foxitiguard.com/Billing/FetchBackupPlans', {
            email: $scope.verificationEmail
        }, config).then(function(response) {
            $scope.foxitipanelLoading = true;
            if (response.data.status == 1) {
    ...
    $scope.BuyNowBackupP = function (planName, monthlyPrice, yearlyPrice, months) {
        const baseURL = 'https://cp.foxitiguard.com/Billing/CreateOrderforBackupPlans';
        // Get the current URL
    ...
    $scope.PaypalBuyNowBackup = function (planName, monthlyPrice, yearlyPrice, months) {
        const baseURL = 'https://cp.foxitiguard.com/Billing/PaypalCreateOrderforBackupPlans';
        // Get the current URL
        var currentURL = window.location.href;
...
app.controller('googleDrive', function ($scope, $http) {
    ...
    $scope.setupAccount = function () {
        window.open("https://cp.foxitiguard.com/gDrive?name=" + $scope.accountName + '&server=' + window.location.href + 'Setup');
    };
