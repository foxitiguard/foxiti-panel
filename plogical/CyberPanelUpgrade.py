...
class UpgradefoxitiPanel:

    LogURL = "https://cp.foxitiguard.com/settings/RecvData"
    ...
    def PostStatus(self, message):
        finalData = json.dumps({'ipAddress': self.ipAddress, "UpgradefoxitiPanelStatus": message})

        try:
            resp = requests.post(UpgradefoxitiPanel.LogURL, data=finalData, timeout=10)
        except:
            pass
    ...
    def RestoreOldCP(self):
        ...
        command = 'rm -rf /usr/local/FoxitiCP'
        Upgrade.executioner(command, command)

        command = 'mv /usr/local/FoxitiCPBak /usr/local/FoxitiCP'
        Upgrade.executioner(command, command)
    ...
    def UpgardeNow(self):
        ...
        ## Backup current FoxitiCP, incase of failure restore

        self.PostStatus('Backing up current installation..,5')

        command = 'cp -R /usr/local/FoxitiCP /usr/local/FoxitiCPBak'
        Upgrade.executioner(command, command)
        ...
        self.PostStatus('foxitiPanel is now on %s..,40' % (self.branch))
        ...
        self.PostStatus('foxitiPanel Upgraded/Downgraded to %s. [200]' % (self.branch))
...
def main():
    ...
    uc = UpgradefoxitiPanel(args.branch,int(args.mail),int(args.dns),int(args.ftp))
