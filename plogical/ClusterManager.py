...
class ClusterManager:
    ...
    LogURL = "https://cp.foxitiguard.com/HighAvailability/RecvData"
    UptimeURL = "https://cp.foxitiguard.com/servers/UptimeReport"
    ClusterFile = '/home/foxitipanel/cluster'
    CloudConfig = '/home/foxitipanel/cloud'
    ...
    def SetupCluster(self):
        try:
            ...
            if self.type == 'Child':
                ...
                writeToFile.write('*/5 * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function CreatePendingVirtualHosts --type Child\n')
                writeToFile.close()
            else:
                ...
                writeToFile = open(cronPath, 'a')
                writeToFile.write('*/%s * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function SyncNow --type Master\n' % (str(self.config['syncTime'])))
                writeToFile.write('*/3 * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function PingNow --type Master\n')
                writeToFile.close()
    ...
    'rootdb': {
        ...
        'PORT': '%s',
            ...
            ## Update root password in foxitipanel file

            writeToFile = open('/etc/foxitipanel/mysqlPassword', 'w')
            writeToFile.write(rootdbpassword)
            writeToFile.close()

            ## Update root password in .my.cnf

            writeToFile = open('/home/foxitipanel/.my.cnf', 'w')
            content = """[mysqldump]
user=root
password=%s
...
password=%s""" % (rootdbpassword, rootdbpassword)
        ...
        try:
            ...
            for website in Websites.objects.all():
                ...
                if not os.path.exists(confPath):
                    ...
                    virtualHostUtilities.createVirtualHost(website.domain, website.adminEmail, website.phpSelection, website.externalApp, 1, 1, 0, website.admin.userName, website.package.packageName, 0, '/home/foxitipanel/temp', 1, 0)
    ...
    def SyncNow(self):
        try:
            ...
            command = "rsync -avzp -e 'ssh -o StrictHostKeyChecking=no -p %s -i /root/.ssh/foxitipanel' /home root@%s:/" % (self.config['failoverServerSSHPort'], self.config['failoverServerIP'])
            ProcessUtilities.normalExecutioner(command)

            self.PostStatus('Syncing SSL certificates to fail over server..')

            command = "rsync -avzp -e 'ssh -o StrictHostKeyChecking=no -p %s -i /root/.ssh/foxitipanel' /etc/letsencrypt root@%s:/etc" % (
    ...
    def UptimeMonitor(self):
        try:
            ...
            writeToFile.write('*/3 * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/ClusterManager.py --function Uptime --type All\n')
            writeToFile.close()
    ...
    def SyncToMaster(self):
        try:
            ...
            command = "rsync -avzp -e 'ssh -o StrictHostKeyChecking=no -p %s -i /root/.ssh/foxitipanel' /home root@%s:/" % (self.config['masterServerSSHPort'], self.config['masterServerIP'])
            ProcessUtilities.normalExecutioner(command)
