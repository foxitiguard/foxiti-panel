...
class sslUtilities:
        ...
        try:
            ...
            with open(cert_path, 'rb') as cert_file:
                ...
                if san_extension:
                    ...
                    try:
                        logging.FoxitiCPLogFileWriter.writeToFile(f'Covered domains: {str(san_domains)}')
                    except:
                        pass
    ...
    def CheckIfSSLNeedsToBeIssued(virtualHostName):
        ...
        if os.path.exists(filePath):
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.FoxitiCPLogFileWriter.writeToFile(f'SSL provider for {virtualHostName} is {SSLProvider}.')
    ...
    def checkIfSSLMap(virtualHostName):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [IO Error with main config file [checkIfSSLMap]]")
            return 0
    ...
    def checkSSLListener():
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [IO Error with main config file [checkSSLListener]]")
            return str(msg)
        return 0
    ...
    def PatchVhostConf(virtualHostName):
        ...
        try:
            ...
            if not os.path.exists(completePathToConfigFile):
                logging.FoxitiCPLogFileWriter.writeToFile(f'Configuration file not found: {completePathToConfigFile}')
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(f'Error reading configuration file: {str(e)}')
            ...
            if DataVhost.find('/.well-known/acme-challenge') != -1:
                logging.FoxitiCPLogFileWriter.writeToFile(f'ACME challenge already configured for {virtualHostName}')
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(f'Error creating challenge directory: {str(e)}')
...
'''
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile(f'Error writing OLS configuration: {str(e)}')
                ...
                try:
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile(f'Error writing Apache configuration: {str(e)}')
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(f'Error restarting LiteSpeed: {str(e)}')
                return 0, f'Error restarting LiteSpeed: {str(e)}'

        except Exception as e:
            logging.FoxitiCPLogFileWriter.writeToFile(f'Unexpected error in PatchVhostConf: {str(e)}')
            return 0, f'Unexpected error: {str(e)}'

    @staticmethod
    def installSSLForDomain(virtualHostName, adminEmail='domain@foxitipanel.net'):

        try:
            website = Websites.objects.get(domain=virtualHostName)
            adminEmail = website.adminEmail
        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.writeToFile('%s [installSSLForDomain:72]' % (str(msg)))
        ...
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [installSSLForDomain]]")
                return 0
        else:
    ...
    """
                ...
                logging.FoxitiCPLogFileWriter.writeToFile('hello world aaa')
    ...
    def obtainSSLForADomain(virtualHostName, adminEmail, sslpath, aliasDomain=None, isHostname=False, forceIssue=False):
        ...
        if adminEmail and ('example.org' in adminEmail or 'example.com' in adminEmail):
            ...
            adminEmail = f'{clean_domain}@foxitipanel.net'
            logging.FoxitiCPLogFileWriter.writeToFile(f'Replacing invalid email with {adminEmail}')
                    ...
                    if result.returncode == 0:
                        ...
                        if result.returncode == 0:
                            ...
                            if install_result.returncode == 0:
                                ...
                                logging.FoxitiCPLogFileWriter.SendEmail(sender_email, adminEmail, result.stdout,
                                                                       'SSL Notification for %s.' % (virtualHostName))
                                return 1
                    return 0
                except Exception as e:
                    logging.FoxitiCPLogFileWriter.writeToFile(str(e))
                    return 0
            else:
                existingCertPath = '/etc/letsencrypt/live/' + virtualHostName
                if not os.path.exists(existingCertPath):
    ...
    try:
        ...
        if os.path.exists(existingCertPath):
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(f"Certificate for {domain} expires in {diff.days} days")
            except Exception as e:
                logging.FoxitiCPLogFileWriter.writeToFile(f"Could not check certificate expiry: {str(e)}")

            logging.FoxitiCPLogFileWriter.writeToFile(f"Certificate exists for {domain}, attempting renewal...")
            ...
            if os.path.exists(acmePath):
                ...
                # foxitiPanel issues ECC (ec-256) certificates, so both the issue and
                ...
                if result.returncode == 0:
                    logging.FoxitiCPLogFileWriter.writeToFile(f"Successfully renewed SSL for {domain}")
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile(f"Renewal failed for {domain}. Error: {error_details}")
                    logging.FoxitiCPLogFileWriter.writeToFile(f"Full error output: {error_output}")
        ...
        if sslUtilities.obtainSSLForADomain(domain, adminEmail, sslpath, aliasDomain, isHostname, forceIssue) == 1:
            ...
            if certificate_present or key_present:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile('%s: %s' % (domain, warning))
            ...
            if sslUtilities.installSSLForDomain(domain) == 1:
                logging.FoxitiCPLogFileWriter.writeToFile("Self signed SSL issued for " + domain + ".")
