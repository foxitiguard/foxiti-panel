...
class IncScheduler(multi.Thread):
    logPath = '/home/foxitipanel/incbackuplogs'
    gitFolder = '/home/foxitipanel/git'
    ...
    def sendBackupFailureEmail(domain, currentTime, detail=''):
        try:
            ...
            adminEmail = open('/home/foxitipanel/adminEmail', 'r').read().rstrip('\n')
...
""" % (sender, ", ".join(recipients), subject, domain, currentTime, detail)
            ...
            for website in os.listdir(IncScheduler.gitFolder):
                ...
                for file in os.listdir(finalPathInside):
                    ...
                    try:
                        ...
                        data['commitMessage'] = 'Auto commit by foxitiPanel %s cron on %s' % (
    ...
    def checkDiskUsage():
        ...
        try:
            ...
            message = '%s - Disk Usage Warning - foxitiPanel' % (ACLManager.fetchIP())
                ...
                if items.runTime == type:
                    ...
                    try:
                        ...
                        try:
                            ...
                            r = requests.post("https://cp.foxitiguard.com/refreshToken", data=finalData
                                              )
                ...
                if jobConfig[IncScheduler.frequency] == type:
                    ...
                    for site in websites:
                        ...
                        if retValues[0] == 0:
                            ...
                            SUBJECT = "Automatic backup failed for %s on %s." % (domain, currentTime)
                            adminEmailPath = '/home/foxitipanel/adminEmail'
...
""" % (sender, ", ".join(TO), SUBJECT, domain, currentTime)
                ...
                if jobConfig[IncScheduler.frequency] == type:
                    ...
                    #     'port'] + " -i /root/.ssh/foxitipanel " + destinationConfig['username'] + "@" + \
                    #           destinationConfig[
...
""" % (sender, ", ".join(TO), SUBJECT, domain, currentTime)
                            ...
                            command = "scp -o StrictHostKeyChecking=no -P " + destinationConfig[
                                'port'] + " -i /root/.ssh/foxitipanel " + backupPath + " " + destinationConfig[
                            ...
                            try:
                                ...
                                if ssh_commands_supported:
                                    ...
                                    try:
                                        ...
                                        logging.FoxitiCPLogFileWriter.writeToFile(f'SSH find command failed: {str(find_err)}, falling back to SFTP [IncScheduler.startNormalBackups]')
                                ...
                                if not backup_found:
                                    try:
                                        ...
                                        try:
                                            ...
                                            logging.FoxitiCPLogFileWriter.writeToFile(f'Backup directory not found: {finalPath} [IncScheduler.startNormalBackups]')
                                            files = []
                                        ...
                                        for f in files:
                                            ...
                                            if check_domain in f and f.endswith('.tar.gz'):
                                                ...
                                                try:
                                                    ...
                                                    logging.FoxitiCPLogFileWriter.writeToFile(f'Failed to stat file {file_path}: {str(stat_err)} [IncScheduler.startNormalBackups]')

                                        sftp.close()
                                    except Exception as sftp_err:
                                        logging.FoxitiCPLogFileWriter.writeToFile(f'SFTP verification failed: {str(sftp_err)} [IncScheduler.startNormalBackups]')

                                # Only send notification if backup was NOT found (backup failed)
                                if not backup_found:
                                    logging.FoxitiCPLogFileWriter.writeToFile(f'Backup NOT found for {check_domain}, sending failure notification [IncScheduler.startNormalBackups]')

                                    import requests

                                    # Define the URL of the endpoint
                                    url = 'https://cp.foxitiguard.com/Billing/BackupFailedNotify'
                                    ...
                                    try:
                                        ...
                                        if response.status_code == 200:
                                            ...
                                            if response_data.get('status') == 1:
                                                logging.FoxitiCPLogFileWriter.writeToFile(f'Failure notification sent successfully for {check_domain} [IncScheduler.startNormalBackups]')
                                            else:
                                                logging.FoxitiCPLogFileWriter.writeToFile(f'Failure notification API returned error for {check_domain}: {response_data.get("error_message")} [IncScheduler.startNormalBackups]')
                                        else:
                                            logging.FoxitiCPLogFileWriter.writeToFile(f'Failure notification API returned HTTP {response.status_code} for {check_domain} [IncScheduler.startNormalBackups]')
                                    except requests.exceptions.RequestException as e:
                                        logging.FoxitiCPLogFileWriter.writeToFile(f'Failed to send backup failure notification for {check_domain}: {str(e)} [IncScheduler.startNormalBackups]')
                                else:
                                    logging.FoxitiCPLogFileWriter.writeToFile(f'Backup verified successful for {check_domain}, no notification needed [IncScheduler.startNormalBackups]')

                            except Exception as msg:
                                logging.FoxitiCPLogFileWriter.writeToFile(f'Error checking backup status for site: {str(msg)} [IncScheduler.startNormalBackups]')
    ...
    def forceRunAWSBackup(planName):
        try:
            ...
            for items in plan.websitesinplan_set.all():
                ...
                extraArgs['path'] = '/home/foxitipanel/backups/%s/backup-' % (
    ...
    def v2Backups(function):
        try:
            ...
            import json
            import time
            if os.path.exists('/home/foxitipanel/v2backups'):
                for website in Websites.objects.all():
                    finalConfigPath = f'/home/foxitipanel/v2backups/{website.domain}'
...
""" % (sender, ", ".join(TO), SUBJECT, website.domain, time.strftime("%m.%d.%Y_%H-%M-%S"))
    ...
    def CheckHostName():
        try:
            ...
            virtualHostUtilities.OnBoardingHostName(CurrentHostName, '/home/foxitipanel/onboarding_temp_path', skipRDNSCheck)
        except BaseException as msg:
