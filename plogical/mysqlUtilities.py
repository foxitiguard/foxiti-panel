...
class mysqlUtilities:
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.FoxitiCPLogFileWriter.writeToFile('Local IP for MySQL: %s' % (mysqlUtilities.LOCALHOST))
    ...
    def createDatabase(dbname,dbuser,dbpassword, dbcreate = 1, host = None, progress=None):
        ...
        try:
            ...
            if dbcreate:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.FoxitiCPLogFileWriter.writeToFile(query)
            ...
            def createUserForHost(hostToUse):
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.FoxitiCPLogFileWriter.writeToFile(grant)
            ...
            if dbcreate == 1 and HostToUse != '127.0.0.1' and mysqlUtilities.REMOTEHOST in ('', 'localhost', '127.0.0.1'):
                try:
                    ...
                    if progress is not None:
                        raise
                    logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [createDatabase:127.0.0.1]")
            ...
            if dbcreate:
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.FoxitiCPLogFileWriter.writeToFile('Deleting database because failed to create %s' % (dbname))
                #mysqlUtilities.deleteDatabase(dbname, dbuser)
            logging.FoxitiCPLogFileWriter.writeToFile('SQL creation failed for %s [createDatabase]' % dbname)
            return 0
        finally:
            if connection:
                connection.close()
    ...
    def createDBUser(dbuser, dbpassword):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[createDBUser]")
            return 0
    ...
    def allowGlobalUserAccess(globalUser, dbName):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[createDatabase]")
            return 0
    ...
    def deleteDatabase(dbname, dbuser):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[deleteDatabase]")
            return str(msg)
    ...
    def createDatabaseBackup(databaseName, tempStoragePath, rustic=0, RusticRepoName = None,
        ...
        try:
            ...
            cnfPath = '/home/foxitipanel/.my.cnf'
...
""" % (password, password)
                ...
                if use_compression:
                    ...
                    with open(backup_file, 'w') as f:
                        ...
                        if result.returncode != 0:
                            ...
                            logging.FoxitiCPLogFileWriter.writeToFile(result.stderr.decode('utf-8'))
                            return 0
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[createDatabaseBackup]")
            return 0
    ...
    def restoreRusticDatabase(databaseName, rusticRepoName, externalApp,
        ...
        try:
            ...
            mysqlCommand = [
                'mysql', '--defaults-file=/home/foxitipanel/.my.cnf',
...
""" % (password, password)
                ...
                os.chmod(cnfPath, 0o600)
                command = 'chown foxitipanel:foxitipanel %s' % (cnfPath)
            ...
            if rustic == 0:
                ...
                mysql_command = ['mysql', '--defaults-file=/home/foxitipanel/.my.cnf',
                                 '-u', mysqluser, '--host=%s' % mysqlhost,
                                 '--port', str(mysqlport), databaseName]
                if backup_format['compressed']:
                ...
                if result != 0:
                    logging.FoxitiCPLogFileWriter.writeToFile('Database import failed for %s.' % databaseName)
                    return 0
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[restoreDatabaseBackup]")
            return 0
    ...
    def prepareDatabaseForRestore(dbName, dbUsername, website):
        ...
        if existing is None:
            return mysqlUtilities.createDatabaseAndRegister(dbName, dbUsername, 'foxitipanel', website)
    ...
    def submitDBDeletion(dbName):
        try:
            ...
            if result == 1:
                ...
                databaseToBeDeleted.delete()
                logging.FoxitiCPLogFileWriter.writeToFile('Deleted database with some errors. Error: %s' % (result))
                return 1,'None'
    ...
    def showStatus():
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[showStatus]")
            return 0
    ...
    def fetchVariables():
        try:
            ...
            for items in result:
                logging.FoxitiCPLogFileWriter.writeToFile(str(items))
    ...
    def fetchDatabases():
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[fetchDatabases]")
            return 0
    ...
    def fetchTableData(name):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[fetchTableData]")
            return 0
    ...
    def changePassword(userName, dbPassword, encrypt = None, host = None):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[mysqlUtilities.changePassword]")
            return 0
    ...
    def fetchuser(databaseName):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[mysqlUtilities.fetchuser]")
            return 0
    ...
    def allowRemoteAccess(dbName, userName, remoteIP):
        try:
            ...
            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/mysqlUtilities.py enableRemoteMYSQL"
            ProcessUtilities.executioner(execPath)

            return 1

        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[mysqlUtilities.allowRemoteAccess]")
            return 0
...
''' % (ipAddressLocal)
        ...
        try:
            ...
            if createUser:
                try:
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile('%s [addUserToDB:937]' % (str(msg)))
                    try:
                        cursor.execute("DROP USER '%s'@'%s'" % (user, mysqlUtilities.LOCALHOST))
                        cursor.execute(
                            "CREATE USER '" + user + "'@'%s' IDENTIFIED BY '" % (mysqlUtilities.LOCALHOST) + password + "'")
                    except BaseException as msg:
                        logging.FoxitiCPLogFileWriter.writeToFile('%s [addUserToDB:943]'  % (str(msg)))
            ...
            if mysqlUtilities.RDS == 0:
                ...
                try:
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile('%s [addUserToDB:953]' % (str(msg)))
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[addUserToDB]")
            return 0
    ...
    def UpgradeMariaDB(versionToInstall, tempStatusPath):
        ...
        if result.find(versionToInstall) > -1:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, f'MySQL is already {result}. [200]')
            return 0

        logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Creating backup of MySQL..,10')

        MySQLBackupDir = '/var/lib/mysql-backupcp'

        from os import getuid
        if getuid() != 0:
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'This function should run as root. [404]')
        ...
        if not os.path.exists(MySQLBackupDir):
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'MySQL backup created..,20')
...
"""
    ...
    def buildMysqldumpCommand(user, host, port, database, use_new_features, use_compression):
        ...
        base_cmd = f"mysqldump --defaults-file=/home/foxitipanel/.my.cnf -u {user} --host={host} --port {port}"
    ...
    def checkNewBackupFeatures():
        ...
        try:
            config_file = '/usr/local/FoxitiCP/plogical/backup_config.json'
            if not os.path.exists(config_file):
                # Try alternate location
                config_file = '/etc/foxitipanel/backup_config.json'
