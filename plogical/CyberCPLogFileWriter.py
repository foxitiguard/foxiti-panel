...
class FoxitiCPLogFileWriter:
    fileName = "/home/foxitipanel/error-logs.txt"
    ...
    def AddFromHeader(sender, message):
        try:
            ...
            FoxitiCPLogFileWriter.writeToFile(str(msg) + ' [AddFromHeader]')

    @staticmethod
    def SendEmail(sender, receivers, message, subject=None, type=None):
        try:
            smtpPath = '/home/foxitipanel/smtpDetails'
            ...
            if os.path.exists(smtpPath):
                ...
                message = FoxitiCPLogFileWriter.AddFromHeader(sender, message)
    ...
    def writeToFile(message, email=None):
        try:
            ...
            file = open(FoxitiCPLogFileWriter.fileName,'a')
            ...
            emailPath = '/usr/local/FoxitiCP/emailDebug'

            try:
                if os.path.exists(emailPath):
                    SUBJECT = "foxitiPanel log reporting"
...
""" % (
                    ...
                    if email == None or email == 1:
                        FoxitiCPLogFileWriter.SendEmail(sender, TO, message)
            except BaseException as msg:
                file = open(FoxitiCPLogFileWriter.fileName, 'a')
                file.writelines("[" + time.strftime(
    ...
    def statusWriter(tempStatusPath, mesg, append = None):
        try:
            ...
            FoxitiCPLogFileWriter.writeToFile(str(msg) + ' [statusWriter]')
