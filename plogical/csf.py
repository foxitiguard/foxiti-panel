...
class CSF(multi.Thread):
    installLogPath = "/home/foxitipanel/csfInstallLog"
    ...
    def run(self):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + ' [CSF.run]')
    ...
    def installCSF():
        try:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(CSF.installLogPath, 'Downloading CSF..\n', 1)
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(CSF.installLogPath, 'Extracting CSF..\n', 1)
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(CSF.installLogPath, 'Installing CSF..\n', 1)

            os.chdir('csf')

            ### manually update csf views.py because it does not load foxitiPanel properly in default configurations
...
def configservercsfiframe(request):
    ...
    try:
        ...
        tmp.write(qs)
        tmp.close()
        command = "/usr/local/csf/bin/foxitipanel.pl '" + tmp.name + "'"
...
'''
            ...
            WriteToFile = open('foxitipanel/configservercsf/views.py', 'w')
            WriteToFile.write(content)
            WriteToFile.close()

            ### now update content of signals.py
            WriteToFile = open('foxitipanel/configservercsf/signals.py', 'w')
            WriteToFile.close()
...
'''
            ...
            WriteToFile = open('foxitipanel/configservercsf/apps.py', 'w')
            WriteToFile.write(content)
            WriteToFile.close()
...
'''
            ...
            WriteToFile = open('foxitipanel/configservercsf/urls.py', 'w')
            WriteToFile.write(content)
            WriteToFile.close()
...
if [ ! -e "/etc/csf/csf.conf" ]; then
	cp -avf csf.foxitipanel.conf /etc/csf/csf.conf
fi
...
if [ ! -e "/etc/csf/csf.allow" ]; then
	cp -avf csf.foxitipanel.allow /etc/csf/csf.allow
fi
...
if [ ! -e "/etc/csf/csf.ignore" ]; then
	cp -avf csf.foxitipanel.ignore /etc/csf/csf.ignore
fi
if [ ! -e "/etc/csf/csf.pignore" ]; then
	cp -avf csf.foxitipanel.pignore /etc/csf/csf.pignore
fi
...
cp -avf uninstall.foxitipanel.sh /usr/local/csf/bin/uninstall.sh
...
chmod -v 700 auto.foxitipanel.pl
./auto.foxitipanel.pl $OLDVERSION
...
mkdir -vp /usr/local/FoxitiCP/public/static/configservercsf/
cp -avf csf/* /usr/local/FoxitiCP/public/static/configservercsf/
cp -avf csf/* foxitipanel/configservercsf/static/configservercsf/
chmod 755 /usr/local/FoxitiCP/public/static/configservercsf/

cp foxitipanel/foxitipanel.pl /usr/local/csf/bin/
chmod 700 /usr/local/csf/bin/foxitipanel.pl
cp -avf foxitipanel/configservercsf /usr/local/FoxitiCP/

mkdir /home/foxitipanel/plugins
touch /home/foxitipanel/plugins/configservercsf

if ! cat /usr/local/FoxitiCP/FoxitiCP/settings.py | grep -q configservercsf; then
    sed -i "/pluginHolder/ i \ \ \ \ 'configservercsf'," /usr/local/FoxitiCP/FoxitiCP/settings.py
fi
if ! cat /usr/local/FoxitiCP/FoxitiCP/urls.py | grep -q configservercsf; then
    sed -i "/pluginHolder/ i \ \ \ \ path('configservercsf/',include('configservercsf.urls'))," /usr/local/FoxitiCP/FoxitiCP/urls.py
fi
#if ! cat /usr/local/FoxitiCP/baseTemplate/templates/baseTemplate/index.html | grep -q configservercsf; then
#    sed -i "/url 'csf'/ i <li><a href='/configservercsf/' title='ConfigServer Security and Firewall'><span>ConfigServer Security \&amp; Firewall</span></a></li>" /usr/local/FoxitiCP/baseTemplate/templates/baseTemplate/index.html
#fi
#if ! cat /usr/local/FoxitiCP/baseTemplate/templates/baseTemplate/index.html | grep -q configserver; then
#    sed -i "/trans 'Plugins'/ i \{\% include \"/usr/local/FoxitiCP/configservercsf/templates/configservercsf/menu.html\" \%\}" /usr/local/FoxitiCP/baseTemplate/templates/baseTemplate/index.html
#fi
...
'''
            ...
            WriteToFile = open('install.foxitipanel.sh', 'w')
            WriteToFile.write(content)
            WriteToFile.close()
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.centos or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                ...
                logging.FoxitiCPLogFileWriter.statusWriter(CSF.installLogPath,
            ...
            for items in data:
                ...
                #    writeToConf.writelines('UI_USER = "foxitipanel"\n')
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(CSF.installLogPath, 'CSF successfully Installed.[200]\n', 1)
            ...
            writeToFile.close()
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[installCSF]")
    ...
    def fetchCSFSettings():
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [fetchCSFSettings]")
    ...
    def changeStatus(controller, status):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[changeStatus]")
            print('0', str(msg))
    ...
    def modifyPorts(protocol, portsPath):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[modifyPorts]")
            print('0', str(msg))
    ...
    def allowIP(ipAddress):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[allowIP]")
    ...
    def blockIP(ipAddress):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[blockIP]")
    ...
    def checkIP(ipAddress):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[checkIP]")
