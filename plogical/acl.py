...
from .FoxitiCPLogFileWriter import FoxitiCPLogFileWriter as logging
...
class ACLManager:
    ...
    def FindIfChild():
        try:
            ...
            config = json.loads(open('/home/foxitipanel/cluster', 'r').read())
    ...
    def CheckStatusFilleLoc(statusFile, domain=None):
        ...
        try:
            ...
            if domain != None:
                value = statusFile.split('foxitipanel/')[1]
                #logging.writeToFile(f'value of log file {value}')
                if value == f'{domain}_rustic_backup_log':
                    return 1
            return 0

        if (statusFile[:18] != "/home/foxitipanel/." or statusFile[:16] == "/home/foxitipanel" or statusFile[:4] == '/tmp' or statusFile[
                                                                                                                 :18] == '/usr/local/FoxitiCP') \
                and statusFile != '/usr/local/FoxitiCP/FoxitiCP/settings.py' and statusFile.find(
            '..') == -1 and statusFile != '/home/foxitipanel/.my.cnf' and statusFile != '/home/foxitipanel/.bashrc' and statusFile != '/home/foxitipanel/.bash_logout' and statusFile != '/home/foxitipanel/.profile':
            return 1
        else:
            return 0
    ...
    def CreateSecureDir():
        ...
        UploadPath = '/usr/local/FoxitiCP/tmp/'

        if not os.path.exists(UploadPath):
            command = 'mkdir %s' % (UploadPath)
            ProcessUtilities.executioner(command)

        command = 'chown foxitipanel:foxitipanel %s' % (UploadPath)
        ProcessUtilities.executioner(command)
    ...
    def GetServiceStatus(dic):
        if os.path.exists('/home/foxitipanel/postfix'):
            dic['emailAsWhole'] = 1
        else:
            dic['emailAsWhole'] = 0
