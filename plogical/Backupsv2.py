...
class CPBackupsV2(multi.Thread):
    ...
    def __init__(self, data):
        ...
        self.StatusFile = f'/home/foxitipanel/{self.website.domain}_rustic_backup_log'
        self.StatusFile_Restore = f'/home/foxitipanel/{self.website.domain}_rustic_backup_log_Restore'
    ...
    def run(self):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + ' [CPBackupsV2.run]')

    def FetchSnapShots(self):
        try:
            command = f'rustic -r {self.repo} snapshots --password "" --json 2>/dev/null'
            # SLSkjoSCczb6wxTMCBPmBMGq/UDSpp28-u cyber5986 rustic -r rclone:None:foxitipanel.net snapshots --password "" --json 2>/dev/null
            result = json.loads(
            ...
            if type == CPBackupsV2.SFTP:
                ...
                try:
                    ...
                    if os.path.exists(ProcessUtilities.debugPath):
                        logging.FoxitiCPLogFileWriter.writeToFile(f'Successfully connected to {config["host"]} through user {config["user"]}')
                except BaseException as msg:
                    return 0, str(msg)
...
'''
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + ' [Configure.run]')
            return 0, str(msg)
    ...
    def BackupConfig(self):
        ...
        # command = f'chown foxitipanel:foxitipanel {self.FinalPathRuctic}'
        # ProcessUtilities.executioner(command)
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            logging.FoxitiCPLogFileWriter.writeToFile(f'Status {str(status)}')
        ...
        if status:
            ...
            command = f'chown foxitipanel:foxitipanel {self.FinalPathRuctic}/config.json'
            ProcessUtilities.executioner(command)
    ...
    def InitiateBackup(self):
        logging.FoxitiCPLogFileWriter.writeToFile("[Create Backup start]")
        ...
        while (1):
            ...
            if self.website.BackupLock == 0:
                ...
                # command = f'chown foxitipanel:foxitipanel {self.FinalPath}'
                # ProcessUtilities.executioner(command)
                ...
                try:
                    ...
                    if os.path.exists(ProcessUtilities.debugPath):
                        logging.FoxitiCPLogFileWriter.writeToFile(f'Snapshots to be merged {str(self.snapshots)}')
    ...
    def BackupRustic(self):
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            logging.FoxitiCPLogFileWriter.writeToFile(f'Status code {status}')
    ...
    def RestoreConfig(self):
        try:
            ...
            # try:
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.FoxitiCPLogFileWriter.writeToFile(f'ACL in config: {ConfigContent["acl"]}')
            #     acl = ACL.objects.get(name=ConfigContent['acl']['name'])
            # except:
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.FoxitiCPLogFileWriter.writeToFile('ACL Already existed.')
            ...
            #
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.FoxitiCPLogFileWriter.writeToFile(f'Passed content to Create ACL Func: {json.dumps(dataToPass)}')
            #
            #     resp = createACLFunc(requestNew)
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.FoxitiCPLogFileWriter.writeToFile(f'CreateACLFunc stats: {str(resp.content)}')
            ...
            try:
                ...
                for database in ConfigContent['databases']:
                    ...
                    if os.path.exists(ProcessUtilities.debugPath):
                        logging.FoxitiCPLogFileWriter.writeToFile(f'Databasename: {dbName}')
    ...
    def FetchCurrentSchedules(website):
        try:
            finalConfigPath = f'/home/foxitipanel/v2backups/{website}'
    ...
    def refresh_V2Gdive_token(refresh_token):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f'Error in tkupdate: {str(msg)}')
    ...
    def CreateScheduleV2(website, repo, frequency, websiteData, websiteDatabases, websiteEmails, retention):
        try:
            ...
            if not os.path.exists('/home/foxitipanel/v2backups/'):

                command = 'mkdir -p /home/foxitipanel/v2backups/'
...
if __name__ == "__main__":
    try:
        ...
        cpbuv2 = CPBackupsV2(
            {'function': 'InitiateRestore', 'domain': 'foxitipanel.net', 'BasePath': '/home/backup', 'SnapShotID': 1,
             'BackendName': 'usman'})
