    ...
    def __init__(self, function_run, data):
        ...
        if result.find('SECTION:Initial Settings') > -1:
            ...
            if tcpIN.find('8088') == -1:
                ...
                portsPath = '/home/foxitipanel/' + str(randint(1000, 9999))
                ...
                execPath = "sudo /usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/plogical/csf.py"
    ...
    def DeployWPContainer(self):
        ...
        try:
            ...
            if result.find("not found") > -1:
                ...
                execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/dockerManager/dockerInstall.py"
                ProcessUtilities.executioner(execPath)
...
services:
  '{self.data['ServiceName']}':
    user: root
    image: foxitipanel/openlitespeed:latest
    ports:
      - "{self.data['port']}:8088"
#      - "443:443"
    environment:
...
'''
            ...
            TempCompose = f'/home/foxitipanel/{self.data["finalURL"]}-docker-compose.yml'
            ...
            execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/DockerSites.py"
    ...
    def setup_docker_environment(self):
        try:
            ...
            command = "docker network ls | grep foxitipanel"
            network_exists = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
            if not network_exists:
                command = "docker network create foxitipanel"
                ProcessUtilities.outputExecutioner(command, None, None, None, 1)
    ...
    def log_deployment_metrics(self, metrics):
        ...
        if metrics:
            try:
                log_file = f"/var/log/foxitipanel/docker/{self.data['ServiceName']}_metrics.json"
...
def Main():
    try:
        ...
        parser = argparse.ArgumentParser(description='foxitiPanel Docker Sites')
        ...
        if args.function == "SetupProxy":
            ...
            # finalURL, blogTitle, adminUser, adminPassword, adminEmail, htaccessPath, externalApp
            data = {
                "JobID": '/home/foxitipanel/hey.txt',
                "ComposePath": "/home/docker.foxitipanel.net/docker-compose.yml",
                "MySQLPath": '/home/docker.foxitipanel.net/public_html/sqldocker',
                ...
                "MemoryMySQL": '512',
                "port": '8000',
                "SitePath": '/home/docker.foxitipanel.net/public_html/wpdocker',
                "CPUsSite": '2',
                "MemorySite": '512',
                "SiteName": 'wp docker test',
                "finalURL": 'docker.foxitipanel.net',
                "blogTitle": 'docker site',
                "adminUser": 'testdbwp',
                "adminPassword": 'testdbwp',
                "adminEmail": 'usman@cyberpersons.com',
                "htaccessPath": '/home/docker.foxitipanel.net/public_html/.htaccess',
                "externalApp": 'docke8463',
                "docRoot": "/home/docker.foxitipanel.net"
            }
