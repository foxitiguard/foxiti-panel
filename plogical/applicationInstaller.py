...
class ApplicationInstaller(multi.Thread):
    ...
    def UpgradeCP(self):
        command = f'/usr/local/foxitiPanel/bin/python /usr/local/FoxitiCP/plogical/upgrade.py "SoftUpgrade,{self.data["branchSelect"]}"'
    ...
    def installJoomla(self):
        ...
        try:
            ...
            try:
                os.remove('/usr/local/FoxitiCP/joomla.zip')
                os.remove('/usr/local/FoxitiCP/lscache_plugin.zip')
                os.remove('/usr/local/FoxitiCP/pkg_lscache.xml')
                os.remove('/usr/local/FoxitiCP/pkg_script.php')
            except:
                pass
            ...
            command = 'wget https://raw.githubusercontent.com/litespeedtech/lscache-joomla/master/package/lscache-1.3.1.zip -O /usr/local/FoxitiCP/joomla.zip'
            ProcessUtilities.executioner(command)

            command = 'unzip -o /usr/local/FoxitiCP/joomla.zip -d /usr/local/FoxitiCP/'
            ProcessUtilities.executioner(command)

            command = '%s extension:installfile %s --www %s /usr/local/FoxitiCP/lscache_plugin.zip' % (
            joomlaPath, dbUser, finalPath)
            ProcessUtilities.executioner(command)

            command = '%s extension:installfile %s --www %s /usr/local/FoxitiCP/com_lscache.zip' % (
            joomlaPath, dbUser, finalPath)
            ProcessUtilities.executioner(command)
    ...
    def DeployWordPress(self):
        try:
            ...
            self.extraArgs['tempStatusPath'] = "/home/foxitipanel/" + str(randint(1000, 9999))
            ...
            if cronData.find('WPAutoUpdates.py') == -1:
                ...
                writeToFile.write(
                    '0 12 * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/WPAutoUpdates.py\n')
                writeToFile.close()
    ...
    def _initializeStagingCreation(self, wpobj):
        ...
        tempPath = '%s/%s' % ('/usr/local/FoxitiCP/tmp', str(randint(1000, 9999)))
    ...
    def DeploytoProduction(self):
        ...
        try:
            ...
            ### Create secure folder
            ACLManager.CreateSecureDir()
            self.tempPath = '%s/%s' % ('/usr/local/FoxitiCP/tmp', str(randint(1000, 9999)))
    ...
    def WPCreateBackup(self):
        try:
            ...
            if Backuptype == "1":
                ...
                self.tempPath = '%s/%s' % ('/usr/local/FoxitiCP/tmp', RandomPath)
                ...
                # command = "sudo -u %s touch /home/foxitipanel/config.json" % (VHuser)
                # ProcessUtilities.executioner(command)
                ###### write into config

                json_object = json.dumps(config, indent=4)
                configPath = "/home/foxitipanel/" + str(randint(1000, 9999))
    ...
    def RestoreWPbackupNow(self):
        try:
            ...
            if BackupDestination == 'SFTP':
                ...
                # 1. Generate SSH keys on the remote server with the name 'foxitipanelbackup'
                ssh_keygen_command = "ssh-keygen -t rsa -b 2048 -f ~/.ssh/foxitipanelbackup -q -N ''"
                ...
                remote_private_key = "~/.ssh/foxitipanelbackup"
                remote_public_key = "~/.ssh/foxitipanelbackup.pub"
                ...
                loaclpath = "/home/foxitipanel/%s.tar.gz" % BackUpFileName
                ...
                #     ProcessUtilities.executioner(command)
                    ...
                    #         unzippathdb = "%s/ab/usr/local/FoxitiCP/tmp/%s/%s" % (
                    ...
                    #                     raise BaseException(stdout)
                    #
                    #                 unzippath = "%s/ab/usr/local/FoxitiCP/tmp/%s/public_html/" % (
                    ...
                    #         if oldhome == "0":
                    #             unzippath = "%s/ab/usr/local/FoxitiCP/tmp/%s/public_html/" % (self.tempPath, oldtemppath)
                    #         else:
                    #             unzippath = "%s/ab/usr/local/FoxitiCP/tmp/%s/public_html/public_html/" % (
                    #                 self.tempPath, oldtemppath)
                    #
                    #         command = "sudo -u %s cp -R %s* %s" % (VHuser, unzippath, newWPpath)
                    #         result, stdout = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
                    ...
                    #
                    #                 # dump Mysql file in unzippath path
                    #                 unzippath2 = "%s/ab/usr/local/FoxitiCP/tmp/%s/%s" % (
                    #                     self.tempPath, oldtemppath, DumpFileName)
            ...
            ###S#Backups
                ...
                FinalZipPath = "/home/foxitipanel/%s.tar.gz" % (uploadfilename)
                try:
                ...
                #
                #         # dump Mysql file in unzippath path
                #         unzippathdb = "%s/ab/usr/local/FoxitiCP/tmp/%s/%s" % (self.tempPath, oldtemppath, DumpFileName)
                #         # command = "mysql -u root %s < %s" % (Finaldbname, unzippathdb)
                #         command = 'sudo -u %s %s -d error_reporting=0 /usr/bin/wp --allow-root --skip-plugins --skip-themes --path=%s --quiet db import %s' % (
                #             VHuser, FinalPHPPath, newWPpath, unzippathdb)
                #         result, stdout = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
                ...
                #
                #         # dump Mysql file in unzippath path
                #         unzippath2 = "%s/ab/usr/local/FoxitiCP/tmp/%s/%s" % (self.tempPath, oldtemppath, DumpFileName)
                #         # command = "mysql -u root %s < %s" % (Finaldbname, unzippath2)
                #         command = 'sudo -u %s %s -d error_reporting=0 /usr/bin/wp --allow-root --skip-plugins --skip-themes --path=%s --quiet db import %s' % (
                #             VHuser, FinalPHPPath, newWPpath, unzippath2)
                #         result, stdout = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
            ...
            ####Check if backup type is Both web and DB
                ...
                ############## New Site
                    ...
                    execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/virtualHostUtilities.py"
    ...
    def StartOCRestore(self):
        try:
            ...
            # Ensure /home/foxitipanel directory exists with proper permissions
            if not os.path.exists('/home/foxitipanel'):
                command = 'mkdir -p /home/foxitipanel'
                ProcessUtilities.executioner(command)

            # Set proper permissions to allow application to write to the directory
            command = 'chmod 755 /home/foxitipanel'
            ProcessUtilities.executioner(command)

            sftp = ssh.open_sftp()

            logging.statusWriter(self.tempStatusPath, 'Downloading Backups...,15')
            loaclpath = f'/home/foxitipanel/{backupfile}'
            ...
            if errorRet:
                ...
                try:
                    sftp.get(f'cpbackups/{folder}/{backupfile}', f'/home/foxitipanel/{backupfile}',
                             callback=self.UpdateDownloadStatus)
            ...
            # sftp = ssh.open_sftp()
            #
            # sftp.get(f'cpbackups/{folder}/{backupfile}', f'/home/foxitipanel/{backupfile}', callback=self.UpdateDownloadStatus)
            ...
            command = f'mv /home/foxitipanel/{backupfile} /home/backup/{backupfile}'
            ProcessUtilities.executioner(command)
...
def main():
    parser = argparse.ArgumentParser(description='foxitiPanel Application Installer')
