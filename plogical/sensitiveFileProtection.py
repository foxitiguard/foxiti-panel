...
BEGIN_MARKER = '# BEGIN foxitiPanel sensitive-file denials'
END_MARKER = '# END foxitiPanel sensitive-file denials'

OLS_DENY_RULES = """# BEGIN foxitiPanel sensitive-file denials
RewriteRule (^|/)\\.env - [F,L]
RewriteRule (^|/)\\.git(/|$) - [F,L]
RewriteRule (^|/)\\.htpasswd$ - [F,L]
RewriteRule (^|/)\\.user\\.ini$ - [F,L]
RewriteRule (^|/)\\.htaccess$ - [F,L]
# END foxitiPanel sensitive-file denials"""
...
def protect_vhost_tree(vhost_root='/usr/local/lsws/conf/vhosts'):
    ...
    for name in sorted(os.listdir(vhost_root)):
        if name in ('Example', 'foxitipanel'):
            continue
