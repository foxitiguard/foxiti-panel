...
class cPanelImporter:
    ...
    def SetupSSL(self, path, domain):
        ...
        KeyPath = '/home/foxitipanel/%s' % (str(randint(1000, 9999)))
        ...
        CertPath = '/home/foxitipanel/%s' % (str(randint(1000, 9999)))
    ...
    def CreateMainWebsite(self):
        try:
            ...
            if os.path.exists(SSLPath):
                ...
                message = 'SSL not detected for %s, you can later issue SSL from Manage SSL in foxitiPanel.' % (DomainName)
                logging.statusWriter(self.logFile, message, 1)
                ...
                try:
                    ...
                    if os.path.exists(SSLPath):
                        ...
                        if os.path.exists(SSLPath):
                            ...
                            message = 'SSL not detected for %s, you can later issue SSL from Manage SSL in foxitiPanel.' % (
                                items)
                            logging.statusWriter(self.logFile, message, 1)
    ...
    def CreateDNSRecords(self):
        try:
            ...
            message = 'We are going to create DNS records now, please note we will not create DKIM records. Make sure to create them from foxitiPanel interface using our DKIM manager.'
            logging.statusWriter(self.logFile, message, 1)
...
""" % (password, password)
            ...
            for items in os.listdir(DatabasesPath):
                ...
                if items.endswith('.sql'):
                    ...
                    command =  f'mysql --defaults-file=/home/foxitipanel/.my.cnf -u {mysqluser} --host={mysqlhost} --port {mysqlport} ' + items.replace('.sql', '')
            ...
            for items in data:
                if items.find("--") > -1 or items.find("'foxitipanel'@") > -1:
                    continue
                try:
                    if os.path.exists(ProcessUtilities.debugPath):
                    ...
                    if items.find('.') > -1:
                        for it in os.listdir(FinalMailDomainPath):
                            try:
                                ...
                                mailUtilities.createEmailAccount(items, it, 'foxitipanel')
...
def main():
    LogFile = '/home/foxitipanel/%s' % (str(randint(1000, 9999)))
    message = 'Backup logs to be generated in %s' % (LogFile)
    print(message)

    parser = argparse.ArgumentParser(description='foxitiPanel cPanel Importer')
