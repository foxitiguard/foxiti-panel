...
class virtualHostUtilities:
    ...
    def emailServicesInstalled():
        ...
        """
        return (
            os.path.exists('/home/foxitipanel/postfix')
    ...
    def OnBoardingHostName(Domain, tempStatusPath, skipRDNSCheck):
        ...
        logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Setting up hostname,10')
        ...
        if not hasattr(admin, 'email') or not admin.email:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, message)
            logging.FoxitiCPLogFileWriter.writeToFile(message)
            return 0
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f'Postfix Hostname: {PostFixHostname}. Server IP {serverIP}. rDNS: {str(rDNS)}, rDNS check {skipRDNSCheck}')
        ...
        if (CurrentHostName == PostFixHostname and CurrentHostName in rDNS) and os.path.exists(filePath):
            ...
            try:
                ...
                SSLProvider = 'Denial'
                logging.FoxitiCPLogFileWriter.writeToFile(f"SSL certificate check error: {str(e)}")
            ...
            if SSLProvider == 'Denial':
                message = 'It seems that the hostname used with mail service and rDNS does not have a valid SSL certificate, foxitiPanel will try to issue valid SSL and restart related services,20'
                ...
                try:
                    ...
                    SSLProvider = 'Denial'
                    logging.FoxitiCPLogFileWriter.writeToFile(f"SSL re-check error: {str(e)}")
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Completed. [200]')
        else:
            ...
            try:
                ...
                try:
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile("Creating website result....%s" % coreResult1)
                    ...
                    while (time.time() - start_time) < timeout:
                        try:
                            ...
                            if os.path.exists(ProcessUtilities.debugPath):
                                logging.FoxitiCPLogFileWriter.writeToFile("Info web creating lastline ....... %s" % lastLine)
                            ...
                            logging.FoxitiCPLogFileWriter.writeToFile(f"Error reading status file: {str(e)}")
            ...
            try:
                ...
                SSLProvider = 'Denial'
                logging.FoxitiCPLogFileWriter.writeToFile(f"Hostname SSL check error: {str(e)}")
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Hostname SSL issued,50')
            ...
            if emailServicesAvailable:
                ...
                try:
                    ...
                    SSLProvider = 'Denial'
                    logging.FoxitiCPLogFileWriter.writeToFile(f"Mail server SSL check error: {str(e)}")
                ...
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Email services not installed, skipping mail server SSL setup.')
                config['hostname'] = Domain
                config['onboarding'] = 1
                config['skipRDNSCheck'] = skipRDNSCheck
                admin.config = json.dumps(config)
                admin.save()
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Hostname setup completed (without email configuration). [200]')
    ...
    def setupAutoDiscover(mailDomain, tempStatusPath, virtualHostName, admin):
        ...
        if not virtualHostUtilities.emailServicesInstalled():
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Email services not installed, skipping mail domain setup.')
            logging.FoxitiCPLogFileWriter.writeToFile('setupAutoDiscover: Email services not installed, skipping.')
            return

        if mailDomain:
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Creating mail child domain..,80')
            ...
            result = virtualHostUtilities.createDomain(virtualHostName, childDomain, 'PHP 7.3', childPath, 1, 0, 0,
                                                       admin.userName, 0,
                                                       "/home/foxitipanel/" + str(randint(1000, 9999)))
            ...
            if os.path.exists(dovecotPath):
                ...
                if not os.path.exists(postFixPath):
                    logging.FoxitiCPLogFileWriter.writeToFile(f"setupAutoDiscover: {postFixPath} not found, skipping postfix TLS SNI configuration")
                    return
    ...
    def createVirtualHost(virtualHostName, administratorEmail, phpVersion, virtualHostUser, ssl,
                          ...
                          tempStatusPath='/home/foxitipanel/fakePath', mailDomain=None, LimitsCheck=1):
        try:

            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Running some checks..,0')
            ...
            if LimitsCheck:
                ...
                if ACLManager.websitesLimitCheck(admin, 1) == 0:
                    logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath,
                ...
                if Websites.objects.filter(domain=virtualHostName).count() > 0:
                    logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'This website already exists. [404]')
                    ...
                    for wp_site in conflicting_wp_sites:
                        ...
                        try:
                            ...
                            if not Websites.objects.filter(id=owner_website.id).exists():
                                ...
                                wp_site.delete()
                                logging.FoxitiCPLogFileWriter.writeToFile(f"Cleaned up orphaned WP site: {wp_site.id} with URL: {wp_site.FinalURL}")
                        except:
                            # WP site owner is missing, delete it
                            wp_site.delete()
                            logging.FoxitiCPLogFileWriter.writeToFile(f"Cleaned up orphaned WP site: {wp_site.id} (missing owner)")
                    ...
                    for staging in orphaned_staging:
                        try:
                            ...
                            if not Websites.objects.filter(id=owner_website.id).exists():
                                ...
                                staging.delete()
                                logging.FoxitiCPLogFileWriter.writeToFile(f"Cleaned up orphaned staging site: {staging.id}")
                        except:
                            # Staging site has invalid references, delete it
                            staging.delete()
                            logging.FoxitiCPLogFileWriter.writeToFile(f"Cleaned up orphaned staging site: {staging.id} (invalid references)")
                except Exception as e:
                    logging.FoxitiCPLogFileWriter.writeToFile(f"Error during staging site cleanup: {str(e)}")

                ####### Limitations Check End

                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Creating DNS records..,10')
                ...
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Setting up directories..,25')
                ...
                if vhost.checkIfAliasExists(virtualHostName) == 1:
                    logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'This domain exists as Alias. [404]')
                    return 0, "This domain exists as Alias."

            postfixPath = '/home/foxitipanel/postfix'
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Creating configurations..,50')
            ...
            if ssl == 1:
                ...
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Setting up SSL..,70')
                if not os.path.exists(virtualHostUtilities.redisConf):
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'SSL set up done..,70')
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'vHost finalized..,70')
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'DKIM Setup..,70')
            ...
            if selectedPackage.enforceDiskLimits:
                try:
                    ...
                    if not success:
                        logging.FoxitiCPLogFileWriter.writeToFile(f"Warning: Failed to set resource limits for user {virtualHostUser}")

                except Exception as e:
                    logging.FoxitiCPLogFileWriter.writeToFile(f"Error applying resource limits for {virtualHostName}: {str(e)}")
            ...
            if selectedPackage.enforceDiskLimits:
                try:
                    ...
                    logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, message + ' [404]')
                    return 0, message

            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Website successfully created. [200]')
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "  [createVirtualHost]")
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, str(msg) + " [404]")
            return 0, str(msg)
    ...
    def issueSSL(virtualHost, path, adminEmail, forceIssue=False):
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile('SSL failed for %s: %s' % (virtualHost, error))
            print(RESULT_PREFIX + json.dumps(payload))
            return 0, str(error)
    ...
    def issueSSLv2(virtualHost, path, adminEmail, forceIssue=False):
        try:
            ...
            if retValues[0] == 0:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(str(retValues[1]))
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [issueSSL]")
            print("0," + str(msg))
            return 0, str(msg)
    ...
    def issueSSLForHostName(virtualHost, path, skipLSCPDRestart=0):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "  [issueSSLForHostName]")
            print("0," + str(msg))
            return 0, str(msg)
    ...
    def issueSSLForMailServer(virtualHost, path):
        try:
            # Check if email services are installed before proceeding
            if not virtualHostUtilities.emailServicesInstalled():
                logging.FoxitiCPLogFileWriter.writeToFile("Email services not installed, skipping mail server SSL setup")
            ...
            if not os.path.exists('/etc/postfix'):
                logging.FoxitiCPLogFileWriter.writeToFile("/etc/postfix directory not found, skipping mail server SSL")
            ...
            if not os.path.exists(filePath):
                logging.FoxitiCPLogFileWriter.writeToFile(f"{filePath} not found, skipping postfix hostname update")
    ...
    def createAlias(masterDomain, aliasDomain, ssl, sslPath, administratorEmail, owner=None):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "  [createAlias]")
            print("0," + str(msg))
    ...
    def issueAliasSSL(masterDomain, aliasDomain, sslPath, administratorEmail):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "  [issueAliasSSL]")
            print("0," + str(msg))
    ...
    def deleteAlias(masterDomain, aliasDomain):
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "  [deleteAlias]")
                print("0," + str(msg))
        else:
            try:
    ...
    def changeOpenBasedir(domainName, openBasedirValue):
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "  [changeOpenBasedir]")
                print("0," + str(msg))
        else:
            try:
                confPath = virtualHostUtilities.Server_root + "/conf/vhosts/" + domainName
    ...
    def createDomain(masterDomain, virtualHostName, phpVersion, path, ssl, dkimCheck, openBasedir, owner, apache,
                     tempStatusPath='/home/foxitipanel/fakePath', LimitsCheck=1, alias = 0):
        try:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'DKIM Setup..,30')
            ...
            if ssl == 1:
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Creating SSL..,50')
                if not os.path.exists(virtualHostUtilities.redisConf):
                    installUtilities.installUtilities.reStartLiteSpeed()
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Domain successfully created. [200]')
            return 1, "None"
    ...
    def switchServer(virtualHostName, phpVersion, server, tempStatusPath):
        try:
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Starting Conversion..,0')
            child = 0
            try:
            ...
            if server == virtualHostUtilities.apache:
                ...
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Creating apache configurations..,90')
                if child:
                ...
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Restarting servers and phps..,90')
                ...
                installUtilities.installUtilities.reStartLiteSpeed()
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Successfully converted.[200]')
            else:
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Starting Conversion..,0')
                ...
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Restarting server..,90')
                installUtilities.installUtilities.reStartLiteSpeed()
                logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Successfully converted. [200]')

        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, '%s[404]' % str(msg))
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "  [switchServer]")
    ...
    def EnableDisablePP(vhostName, username=None, password=None, path=None, wpid=None, externalApp=None):
        try:
            ...
            htpasstemp = f'/usr/local/FoxitiCP/{wpid}'
...
def main():
    ...
    if args.function == "createVirtualHost":
        ...
        virtualHostUtilities.setupAutoDiscover(1, '/home/foxitipanel/templogs', args.virtualHostName, admin)
