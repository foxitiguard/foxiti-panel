...
class ImunifyIntegrationTests(unittest.TestCase):
    ...
    def test_cagefs_prefers_foxitipanel_modules_over_system_packages(self):
        cagefs_source = (
            pathlib.Path(__file__).parents[1] / 'CLManager' / 'CageFS.py'
        ).read_text(encoding='utf-8')
        path_setup = "sys.path.insert(0, '/usr/local/FoxitiCP')"
    ...
    def test_generated_configs_include_required_panel_integrations(self):
        for config in (
            ...
            self.assertIn('panel_info = /usr/local/FoxitiCP/CLScript/panel_info.py', config)
            self.assertIn('users = /usr/local/FoxitiCP/CLScript/CloudLinuxUsers.py', config)
            self.assertIn('domains = /usr/local/FoxitiCP/CLScript/CloudLinuxDomains.py', config)
    ...
    def test_deploy_commands_pass_the_license_key_as_one_argument(self):
        ...
        self.assertEqual('/usr/local/FoxitiCP/CLManager/CageFS.py', worker_args[1])
