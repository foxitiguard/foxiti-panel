...
from foxitipanel_firewall_migration import CSF_UPGRADE_MESSAGE, requireCSFMigration
...
def update_all_config_files_with_password(new_password):
    """
    Update all configuration files that use the foxitipanel database password.
    This includes FTP, PowerDNS, Postfix, Dovecot configurations.
    """
    config_updates = [
        # Django settings
        {
            'path': '/usr/local/FoxitiCP/FoxitiCP/settings.py',
            'updates': [
                (r"('foxitipanel'[^}]+?'PASSWORD':\s*')[^']+'", r"\1%s'" % new_password)
            ]
        },
        # FTP configurations
        {
...
def update_dovecot_connect_string(connect_line, new_password):
    ...
    Format: connect = host=localhost dbname=foxitipanel user=foxitipanel password=oldpass
    """
...
def restart_affected_services():
    """
    Restart services that use the foxitipanel database password.
    """
    services_to_restart = [
...
# Try to import settings, but handle case where FoxitiCP directory is damaged
try:
    from FoxitiCP import settings
except ImportError:
    print("WARNING: Cannot import FoxitiCP settings. Attempting recovery...")
    
    def recover_database_credentials():
        """Attempt to recover or reset database credentials"""
        
        # First, ensure we have root MySQL password
        if not os.path.exists('/etc/foxitipanel/mysqlPassword'):
            print("FATAL: Cannot find MySQL root password file at /etc/foxitipanel/mysqlPassword")
            print("Manual intervention required.")
            sys.exit(1)
        
        root_password = open('/etc/foxitipanel/mysqlPassword', 'r').read().strip()
        foxitipanel_password = None
        
        # Try to read existing settings.py to get foxitipanel password
        settings_path = '/usr/local/FoxitiCP/FoxitiCP/settings.py'
        if os.path.exists(settings_path):
            try:
                ...
                import re
                # Extract foxitipanel database password
                db_pattern = r"'default':[^}]*'USER':\s*'foxitipanel'[^}]*'PASSWORD':\s*'([^']+)'"
                match = re.search(db_pattern, settings_content, re.DOTALL)
                
                if match:
                    foxitipanel_password = match.group(1)
                    print("Found existing foxitipanel password in settings.py")
                    
                    # Test if this password actually works
                    try:
                        test_conn = mysql.connect(host='localhost', user='foxitipanel', 
                                                passwd=foxitipanel_password, db='foxitipanel')
                        test_conn.close()
                        print("Verified foxitipanel database credentials are valid")
                    except:
        ...
        if foxitipanel_password is None:
            print("Resetting foxitipanel database user password...")
            
            # Check if we're on Ubuntu or CentOS
            # On Ubuntu, foxitipanel uses root password; on CentOS, it uses a separate password
            if os.path.exists('/etc/lsb-release'):
                # Ubuntu - use root password
                foxitipanel_password = root_password
                reset_to_root = True
            else:
                # CentOS/others - generate new password
                chars = string.ascii_letters + string.digits
                foxitipanel_password = ''.join(secrets.choice(chars) for _ in range(14))
                reset_to_root = False
            
            try:
                # Connect as root and reset foxitipanel user
                conn = mysql.connect(host='localhost', user='root', passwd=root_password)
                cursor = conn.cursor()
                
                # Check if foxitipanel database exists
                cursor.execute("SHOW DATABASES LIKE 'foxitipanel'")
                if not cursor.fetchone():
                    print("Creating foxitipanel database...")
                    cursor.execute("CREATE DATABASE IF NOT EXISTS foxitipanel")
                
                # Reset foxitipanel user - drop and recreate to ensure clean state
                cursor.execute("DROP USER IF EXISTS 'foxitipanel'@'localhost'")
                cursor.execute("CREATE USER 'foxitipanel'@'localhost' IDENTIFIED BY '%s'" % foxitipanel_password)
                cursor.execute("GRANT ALL PRIVILEGES ON foxitipanel.* TO 'foxitipanel'@'localhost'")
                cursor.execute("FLUSH PRIVILEGES")
                
                conn.close()
                
                if reset_to_root:
                    print("Reset foxitipanel user password to match root password (Ubuntu style)")
                else:
                    print("Reset foxitipanel user with new generated password (CentOS style)")
                
                # Update all configuration files with the new password
                print("Updating all service configuration files with new password...")
                update_all_config_files_with_password(foxitipanel_password)
                ...
                temp_pass_file = '/tmp/foxitipanel_recovered_password'
                with open(temp_pass_file, 'w') as f:
                    f.write(foxitipanel_password)
                os.chmod(temp_pass_file, 0o600)
                print("Saved recovered password to temporary file")
                
            except Exception as e:
                print("Failed to reset foxitipanel database user: %s" % str(e))
                print("Manual intervention required. Please run:")
                print("  mysql -u root -p")
                print("  CREATE DATABASE IF NOT EXISTS foxitipanel;")
                print("  GRANT ALL PRIVILEGES ON foxitipanel.* TO 'foxitipanel'@'localhost' IDENTIFIED BY 'your_password';")
                print("  FLUSH PRIVILEGES;")
                sys.exit(1)
        
        return foxitipanel_password, root_password
    
    # Perform recovery
    foxitipanel_password, root_password = recover_database_credentials()
...
class Upgrade:
    ...
    cdn = 'cdn.foxitipanel.sh'
    installedOutput = ''
    CentOSPath = '/etc/redhat-release'
    UbuntuPath = '/etc/lsb-release'
    openEulerPath = '/etc/openEuler-release'
    FromCloud = 0
    LogPathNew = '/home/foxitipanel/upgrade_logs'
    SoftUpgrade = 0
    ...
    @staticmethod
    def updateRepoURL():
        command = "sed -i 's|sgp.foxitipanel.sh|cdn.foxitipanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|lax.foxitipanel.sh|cdn.foxitipanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|fra.foxitipanel.sh|cdn.foxitipanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|mirror.foxitipanel.net|cdn.foxitipanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|sgp.foxitipanel.sh|cdn.foxitipanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|lax.foxitipanel.sh|cdn.foxitipanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|fra.foxitipanel.sh|cdn.foxitipanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|mirror.foxitipanel.net|cdn.foxitipanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)
    ...
    def dockerUsers():
        ...
        command = 'usermod -aG docker foxitipanel'
        Upgrade.executioner(command, 'adduser docker', 0)
    ...
    def fixSudoers():
        try:
            ...
            command = "chsh -s /bin/false foxitipanel"
    ...
    def installCustomOLSBinaries():
        ...
        try:
            ...
            BINARY_CONFIGS = {'rhel8': {'url': 'https://foxitipanel.net/openlitespeed-2.5.5-x86_64-rhel8',
                       'module_url': 'https://foxitipanel.net/foxitipanel_ols-2.7.7-x86_64-rhel8.so',
                       'modsec_url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-rhel8.so',
             ...
             'rhel9': {'url': 'https://foxitipanel.net/openlitespeed-2.5.5-x86_64-rhel9',
                       'module_url': 'https://foxitipanel.net/foxitipanel_ols-2.7.7-x86_64-rhel9.so',
                       'modsec_url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-rhel9.so',
             ...
             'rhel10': {'url': 'https://foxitipanel.net/openlitespeed-2.5.5-x86_64-rhel10',
                        'module_url': 'https://foxitipanel.net/foxitipanel_ols-2.7.7-x86_64-rhel10.so',
                        'modsec_url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-rhel10.so',
             ...
             'ubuntu': {'url': 'https://foxitipanel.net/openlitespeed-2.5.5-x86_64-ubuntu',
                        'module_url': 'https://foxitipanel.net/foxitipanel_ols-2.7.7-x86_64-ubuntu.so',
                        'modsec_url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-ubuntu.so',
             ...
             'ubuntu26': {'url': 'https://foxitipanel.net/openlitespeed-2.5.5-x86_64-ubuntu',
                          'module_url': 'https://foxitipanel.net/foxitipanel_ols-2.7.7-x86_64-ubuntu.so',
                          'modsec_url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-ubuntu26.so',
                          'sha256': {'binary': '736eeb47bd3dc7b2a7206b95106dd735a80b3386118d0b4b17f39f9fa5324f8c',
                                     'module': '41ae567f45931691e34facf1914bf78bb95236d9f8b1ee75d455cbbc19ca6d8b',
            ...
            MODULE_PATH = "/usr/local/lsws/modules/foxitipanel_ols.so"
            ...
            required = [
                ...
                ('module', config.get('module_url'), MODULE_PATH, 0o644, 'foxitipanel_ols.so.backup'),
            ]
            ...
            with tempfile.TemporaryDirectory(prefix='foxitipanel-ols-') as staging:
                downloads = {}
        ...
        try:
            ...
            if os.path.exists(backup_binary):
                ...
                for backup_name, target in (
                        ("foxitipanel_ols.so.backup", module_path or "/usr/local/lsws/modules/foxitipanel_ols.so"),
    ...
    def configureCustomModule():
        """Configure foxitiPanel module in OpenLiteSpeed config"""
        try:
            Upgrade.stdOut("Configuring foxitiPanel module...", 0)
...
"""
    ...
    def download_install_phpmyadmin():
        try:
            ...
            if not os.path.exists("/usr/local/FoxitiCP/public"):
                os.mkdir("/usr/local/FoxitiCP/public")

            try:
                shutil.rmtree("/usr/local/FoxitiCP/public/phpmyadmin")
            except:
                pass

            Upgrade.stdOut("Installing phpMyAdmin...", 0)
            
            command = 'wget -q -O /usr/local/FoxitiCP/public/phpmyadmin.zip https://github.com/usmannasir/foxitipanel/raw/stable/phpmyadmin.zip'
            Upgrade.executioner_silent(command, 'Download phpMyAdmin')

            command = 'unzip -q /usr/local/FoxitiCP/public/phpmyadmin.zip -d /usr/local/FoxitiCP/public/'
            Upgrade.executioner_silent(command, 'Extract phpMyAdmin')

            command = 'mv /usr/local/FoxitiCP/public/phpMyAdmin-*-all-languages /usr/local/FoxitiCP/public/phpmyadmin'
            subprocess.call(command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

            command = 'rm -f /usr/local/FoxitiCP/public/phpmyadmin.zip'
            ...
            data = open('/usr/local/FoxitiCP/public/phpmyadmin/config.sample.inc.php', 'r').readlines()

            writeToFile = open('/usr/local/FoxitiCP/public/phpmyadmin/config.inc.php', 'w')
...
"""
            ...
            writeToFile.writelines("$cfg['TempDir'] = '/usr/local/FoxitiCP/public/phpmyadmin/tmp';\n")

            writeToFile.close()

            os.mkdir('/usr/local/FoxitiCP/public/phpmyadmin/tmp')

            command = 'cp /usr/local/FoxitiCP/plogical/phpmyadminsignin.php /usr/local/FoxitiCP/public/phpmyadmin/phpmyadminsignin.php'
            Upgrade.executioner(command, 0)
            command = 'cp /usr/local/FoxitiCP/plogical/phpmyadminsession.php /usr/local/FoxitiCP/public/phpmyadmin/phpmyadminsession.php'
            Upgrade.executioner(command, 0)

            passFile = "/etc/foxitipanel/mysqlPassword"
            ...
            try:
                ...
                configure_phpmyadmin_signon(
                    '/usr/local/FoxitiCP/public/phpmyadmin/phpmyadminsignin.php',
                    mysqlhost,
                    mysqlport,
                )
    ...
    def setupComposer():
        ...
        command = "wget https://foxitipanel.sh/composer.sh"
        Upgrade.executioner(command, 0)
    ...
    def downloadLink():
        try:
            ...
            try:
                ...
                path = "/usr/local/FoxitiCP/version.txt"
                writeToFile = open(path, 'w')
    ...
    def setupCLI():
        try:
            ...
            command = "ln -s /usr/local/FoxitiCP/cli/cyberPanel.py /usr/bin/foxitipanel"
            Upgrade.executioner(command, 'CLI Symlink', 0)

            command = "chmod +x /usr/local/FoxitiCP/cli/cyberPanel.py"
    ...
    def staticContent():
        ...
        command = "rm -rf /usr/local/FoxitiCP/public/static"
        ...
        os.chdir('/usr/local/FoxitiCP')

        command = '/usr/local/foxitiPanel/bin/python manage.py collectstatic --noinput --clear'
        ...
        shutil.move("/usr/local/FoxitiCP/static", "/usr/local/FoxitiCP/public/")
        Upgrade.normalizeStaticPermissions('/usr/local/FoxitiCP/public/static')
    ...
    def ensurePostfixDomainLookup(path='/etc/postfix/mysql-virtual_domains.cf'):
        ...
        try:
            ...
            validation = subprocess.run(
                [
                    'postmap', '-q', '__foxitipanel_config_check__',
                    'mysql:%s' % path,
                ],
    ...
    def getMachineIP():
        try:
            with open('/etc/foxitipanel/machineIP', 'r') as ip_file:
                return ip_file.readline().strip()
        except Exception:
            return ''

    @staticmethod
    def localfoxitiPanelDatabaseAccountHosts(database_host, machine_ip):
    ...
    @staticmethod
    def repairLocalfoxitiPanelDatabaseAccess():
        ...
        try:
            ...
            account_hosts = Upgrade.localfoxitiPanelDatabaseAccountHosts(
                database_host, machine_ip,
            )

            if (database.get('NAME') != 'foxitipanel' or
                    database.get('USER') != 'foxitipanel' or
            ...
            for account_host in account_hosts:
                cursor.execute(
                    ...
                    ('foxitipanel', account_host),
                )

                if cursor.fetchone():
                    cursor.execute(
                        'ALTER USER %s@%s IDENTIFIED BY %s',
                        ('foxitipanel', account_host, database['PASSWORD']),
                    )
                else:
                    cursor.execute(
                ...
                cursor.execute(
                    'GRANT ALL PRIVILEGES ON `foxitipanel`.* TO %s@%s',
                    ('foxitipanel', account_host),
                )
            cursor.execute('FLUSH PRIVILEGES')
            connection.close()
            ...
            if connection:
                try:
                    ...
                    pass
            Upgrade.stdOut(
                'Unable to verify the local foxitiPanel database account during upgrade.',
                0,
            )
            return 0
    ...
    def applyLoginSystemMigrations():
        try:
            ...
            connection, cursor = Upgrade.setupConnection('foxitipanel')

            try:
                cursor.execute(
                    'CREATE TABLE `baseTemplate_foxitipanelcosmetic` (`id` integer AUTO_INCREMENT NOT NULL PRIMARY KEY, `MainDashboardCSS` longtext NOT NULL)')
            except:
                pass
...
""")
    ...
    def setupWebmail():
        ...
        try:
            ...
            already_configured = os.path.exists('/etc/foxitipanel/webmail.conf') and \
            ...
            if not already_configured:
                ...
                with open('/etc/dovecot/master-users', 'w') as f:
                    f.write('foxitipanel_master:' + password_hash + '\n')
                os.chmod('/etc/dovecot/master-users', 0o600)
                subprocess.call(['chown', 'dovecot:dovecot', '/etc/dovecot/master-users'])

                # Write /etc/foxitipanel/webmail.conf
                webmail_conf = {
                    'master_user': 'foxitipanel_master',
                    'master_password': master_password
                }
                with open('/etc/foxitipanel/webmail.conf', 'w') as f:
                    json.dump(webmail_conf, f)
                os.chmod('/etc/foxitipanel/webmail.conf', 0o600)
                subprocess.call(['chown', 'foxitipanel:foxitipanel', '/etc/foxitipanel/webmail.conf'])
...
"""
            ...
            Upgrade.executioner(
                'python /usr/local/FoxitiCP/manage.py makemigrations webmail',
                'Webmail makemigrations', shell=True
            )
            Upgrade.executioner(
                'python /usr/local/FoxitiCP/manage.py migrate',
                'Webmail migrate', shell=True
            )

            # Fix webmail.conf ownership for lscpd (may be wrong on existing installs)
            if os.path.exists('/etc/foxitipanel/webmail.conf'):
                subprocess.call(['chown', 'foxitipanel:foxitipanel', '/etc/foxitipanel/webmail.conf'])
                os.chmod('/etc/foxitipanel/webmail.conf', 0o600)
    ...
    def repairDovecot24SNIPaths():
        ...
        if normalized != content:
            ...
            temporary_conf = dovecot_conf + '.foxitipanel-sni.tmp'
            try:
  ...
  PRIMARY KEY (`id`)
    ...
    def GeneralMigrations():
        try:
            ...
            command = '/usr/local/foxitiPanel/bin/python manage.py makemigrations'
            Upgrade.executioner(command, 'python manage.py makemigrations', 0)

            command = '/usr/local/foxitiPanel/bin/python manage.py makemigrations'
            Upgrade.executioner(command, '/usr/local/foxitiPanel/bin/python manage.py migrate', 0)
  ...
  CONSTRAINT `websiteFunctions_gdr_owner_id_4cf7983e_fk_websiteFu` FOREIGN KEY (`owner_id`) REFERENCES `websiteFunctions_gdrive` (`id`)
    ...
    def enableServices():
        try:
            servicePath = '/home/foxitipanel/powerdns'
            writeToFile = open(servicePath, 'w+')
            writeToFile.close()

            servicePath = '/home/foxitipanel/postfix'
            writeToFile = open(servicePath, 'w+')
            writeToFile.close()

            servicePath = '/home/foxitipanel/pureftpd'
            writeToFile = open(servicePath, 'w+')
            writeToFile.close()
        except:
            pass
    ...
    def backupCriticalFiles():
        ...
        import tempfile
        backup_dir = tempfile.mkdtemp(prefix='foxitipanel_backup_')
        
        critical_files = [
            '/usr/local/FoxitiCP/FoxitiCP/settings.py',
            '/usr/local/FoxitiCP/.git/config',  # Git configuration
            '/usr/local/FoxitiCP/.env',
            '/usr/local/FoxitiCP/.env.backup',
            '/usr/local/FoxitiCP/secret_key',
        ]
        
        # Also backup any custom configurations
        custom_configs = [
            '/usr/local/FoxitiCP/baseTemplate/static/baseTemplate/custom/',
            '/usr/local/FoxitiCP/public/phpmyadmin/config.inc.php',
            '/usr/local/FoxitiCP/rainloop/data/_data_/',
        ]
        
        # Backup Imunify360 directories and configuration
        imunify_paths = [
            '/usr/local/FoxitiCP/public/imunify',
            '/usr/local/FoxitiCP/public/imunifyav',
    ...
    'rootdb': {
        ...
        'PORT': '%s',
            ...
            settingsFile = '/usr/local/FoxitiCP/FoxitiCP/settings.py'

            Upgrade.stdOut("Critical files backed up to: " + backup_dir)

            ## Always do a fresh clone for clean upgrade
            
            Upgrade.stdOut("Performing clean upgrade by removing and re-cloning foxitiPanel...")
            
            # Set git config first
            command = 'git config --global user.email "support@foxitipanel.net"'
            if not Upgrade.executioner(command, command, 1):
                return 0, 'Failed to execute %s' % (command)

            command = 'git config --global user.name "foxitiPanel"'
            if not Upgrade.executioner(command, command, 1):
                return 0, 'Failed to execute %s' % (command)
            ...
            # Remove old FoxitiCP directory
            if os.path.exists('FoxitiCP'):
                Upgrade.stdOut("Removing old FoxitiCP directory...")
                try:
                    shutil.rmtree('FoxitiCP')
                    Upgrade.stdOut("Old FoxitiCP directory removed successfully.")
                except Exception as e:
                    Upgrade.stdOut(f"Error removing FoxitiCP directory: {str(e)}")
                    # Try to restore backup if removal fails
                    Upgrade.restoreCriticalFiles(backup_dir, backed_up_files)
                    return 0, 'Failed to remove old FoxitiCP directory'

            # Clone the new repository directly to FoxitiCP
            Upgrade.stdOut("Cloning fresh foxitiPanel repository...")
            command = 'git clone https://github.com/usmannasir/foxitipanel FoxitiCP'
            if not Upgrade.executioner(command, command, 1):
                # Try to restore backup if clone fails
                Upgrade.stdOut("Clone failed, attempting to restore backup...")
                Upgrade.restoreCriticalFiles(backup_dir, backed_up_files)
                return 0, 'Failed to clone foxitiPanel repository'
    ...
    def installLSCPD(branch):
        try:
            ...
            if Upgrade.SoftUpgrade == 0:
                ...
                command = f'cp -f /usr/local/FoxitiCP/{lscpdSelection} /usr/local/lscp/bin/{lscpdSelection}'
                Upgrade.executioner(command, command, 0)
    ...
    ### disable dkim signing in rspamd in ref to https://github.com/usmannasir/foxitipanel/issues/1176
    @staticmethod
    def FixRSPAMDConfig():
...
'''
    ...
    def fixPermissions():
        try:
            ...
            ###### fix Core foxitiPanel permissions

            command = "find /usr/local/FoxitiCP -type d -exec chmod 0755 {} \;"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "find /usr/local/FoxitiCP -type f -exec chmod 0644 {} \;"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chmod -R 755 /usr/local/FoxitiCP/bin"
            Upgrade.executioner(command, 'chown core code', 0)

            ## change owner

            command = "chown -R root:root /usr/local/FoxitiCP"
            Upgrade.executioner(command, 'chown core code', 0)

            terminalSecretPath = '/usr/local/FoxitiCP/terminal_jwt_secret'
            if os.path.exists(terminalSecretPath):
                command = "chown foxitipanel:foxitipanel %s" % terminalSecretPath
            ...
            command = "chown -R lscpd:lscpd /usr/local/FoxitiCP/public/phpmyadmin/tmp"
            Upgrade.executioner(command, 'chown core code', 0)
            ...
            command = "chmod 700 /usr/local/FoxitiCP/cli/cyberPanel.py"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chmod 700 /usr/local/FoxitiCP/plogical/upgradeCritical.py"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chmod 755 /usr/local/FoxitiCP/postfixSenderPolicy/client.py"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chmod 640 /usr/local/FoxitiCP/FoxitiCP/settings.py"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chown root:foxitipanel /usr/local/FoxitiCP/FoxitiCP/settings.py"
            Upgrade.executioner(command, 'chown core code', 0)

            for path in ('/usr/local/FoxitiCP/.env', '/usr/local/FoxitiCP/secret_key'):
                if os.path.exists(path):
                    command = "chown root:foxitipanel %s" % path
            ...
            backup_env = '/usr/local/FoxitiCP/.env.backup'
            ...
            command = 'chmod +x /usr/local/FoxitiCP/CLManager/CLPackages.py'
            ...
            fileM = ['/usr/local/lsws/FileManager/', '/usr/local/FoxitiCP/install/FileManager',
                     '/usr/local/FoxitiCP/serverStatus/litespeed/FileManager',
            ...
            command = 'chmod +x /usr/local/FoxitiCP/plogical/renew.py'
            Upgrade.executioner(command, command, 0)

            command = 'chmod +x /usr/local/FoxitiCP/CLManager/CLPackages.py'
            Upgrade.executioner(command, command, 0)

            clScripts = ['/usr/local/FoxitiCP/CLScript/panel_info.py',
                         '/usr/local/FoxitiCP/CLScript/CloudLinuxPackages.py',
                         '/usr/local/FoxitiCP/CLScript/CloudLinuxUsers.py',
                         '/usr/local/FoxitiCP/CLScript/CloudLinuxDomains.py'
                , '/usr/local/FoxitiCP/CLScript/CloudLinuxResellers.py',
                         '/usr/local/FoxitiCP/CLScript/CloudLinuxAdmins.py',
                         '/usr/local/FoxitiCP/CLScript/CloudLinuxDB.py', '/usr/local/FoxitiCP/CLScript/UserInfo.py']

            for items in clScripts:
                command = 'chmod +x %s' % (items)
                Upgrade.executioner(command, 0)

            command = 'chmod 600 /usr/local/FoxitiCP/plogical/adminPass.py'
            Upgrade.executioner(command, 0)

            command = 'chmod 600 /etc/cagefs/exclude/foxitipanelexclude'
            Upgrade.executioner(command, 0)

            command = "find /usr/local/FoxitiCP/ -name '*.pyc' -delete"
            Upgrade.executioner(command, 0)
            ...
            command = 'chmod 640 /usr/local/lscp/foxitipanel/logs/access.log'
            Upgrade.executioner(command, 0)
            ...
            command = 'chmod 700 %s' % ('/home/foxitipanel')
            Upgrade.executioner(command, 0)
    ...
    def someDirectories():
        ...
        command = "mkdir -p /usr/local/lscp/foxitipanel/logs"
        Upgrade.executioner(command, 0)
    ...
    def upgradeDovecot():
        try:
            ...
            configbackups = '/home/foxitipanel/configbackups'
...
panel_info = /usr/local/FoxitiCP/CLScript/panel_info.py
packages = /usr/local/FoxitiCP/CLScript/CloudLinuxPackages.py
users = /usr/local/FoxitiCP/CLScript/CloudLinuxUsers.py
domains = /usr/local/FoxitiCP/CLScript/CloudLinuxDomains.py
resellers = /usr/local/FoxitiCP/CLScript/CloudLinuxResellers.py
admins = /usr/local/FoxitiCP/CLScript/CloudLinuxAdmins.py
db_info = /usr/local/FoxitiCP/CLScript/CloudLinuxDB.py

[lvemanager_config]
ui_user_info = /usr/local/FoxitiCP/CLScript/UserInfo.py
...
"""
                ...
                writeToFile = open('/etc/cagefs/exclude/foxitipanelexclude', 'w')
                writeToFile.write(content)
                writeToFile.close()
    ...
    def runSomeImportantBash():
        ...
        # Remove invalid crons from /etc/crontab Reference: https://github.com/usmannasir/foxitipanel/issues/216
        command = """sed -i '/FoxitiCP/d' /etc/crontab"""
        ...
        if os.path.exists(cronPath):
            ...
            if data.find('findBWUsage') == -1:
                content = """
0 * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/findBWUsage.py >/dev/null 2>&1
0 * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/postfixSenderPolicy/client.py hourlyCleanup >/dev/null 2>&1
0 0 1 * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/postfixSenderPolicy/client.py monthlyCleanup >/dev/null 2>&1
0 2 * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/upgradeCritical.py >/dev/null 2>&1
0 0 * * 4 /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/renew.py >/dev/null 2>&1
7 0 * * * "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" > /dev/null
*/3 * * * * if ! find /home/*/public_html/ -maxdepth 2 -type f -newer /usr/local/lsws/cgid -name '.htaccess' -exec false {} +; then /usr/local/lsws/bin/lswsctrl restart; fi
* * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/manage.py run_scheduled_scans >/usr/local/lscp/logs/scheduled_scans.log 2>&1
"""
            ...
            if data.find('IncScheduler.py') == -1:
                content = """
0 12 * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py Daily
0 0 * * 0 /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py Weekly
"""
            ...
            if data.find("IncScheduler.py '30 Minutes'") == -1:
                content = """
*/30 * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py '30 Minutes'
0 * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py '1 Hour'
0 */6 * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py '6 Hours'
0 */12 * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py '12 Hours'
0 1 * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py '1 Day'
0 0 */3 * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py '3 Days'
0 0 * * 0 /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py '1 Week'
"""
                writeToFile = open(cronPath, 'a')
                writeToFile.write(content)
                writeToFile.close()
...
"""
            ...
            if data.find('pdnsHealthCheck.py') == -1:
                content = """
*/5 * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/pdnsHealthCheck.py >/dev/null 2>&1
"""
                writeToFile = open(cronPath, 'a')
                writeToFile.write(content)
                writeToFile.close()
...
0 0 * * 4 /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/renew.py >/dev/null 2>&1
7 0 * * * "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" > /dev/null
0 0 * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py Daily
0 0 * * 0 /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/IncBackups/IncScheduler.py Weekly
* * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/manage.py run_scheduled_scans >/usr/local/lscp/logs/scheduled_scans.log 2>&1
*/5 * * * * /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/pdnsHealthCheck.py >/dev/null 2>&1
"""
...
'''
    ...
    def upgrade(branch):
        ...
        if not os.path.exists('/usr/local/lsws/bin/openlitespeed'):
            ...
            command = 'wget https://raw.githubusercontent.com/usmannasir/foxitipanel/stable/install/litespeed/httpd_config.xml'
            Upgrade.executioner(command, command, 0)
            ...
            if Upgrade.installCustomOLSBinaries():
                ...
                try:
                    ...
                    if 'autoSSL' not in content:
                        content = re.sub(
                            r'(adminEmails\s+\S+)',
                            r'\1\nautoSSL                   1\nacmeEmail                 admin@foxitipanel.net',
                            content,
                            count=1
                        )
        ...
        if os.path.exists('/usr/local/foxitiPanel.' + versionNumbring):
            os.remove('/usr/local/foxitiPanel.' + versionNumbring)

        ##

        # execPath = "sudo /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/csf.py"
        # execPath = execPath + " removeCSF"
        # Upgrade.executioner(execPath, 'fix csf if there', 0)

        Upgrade.repairLocalfoxitiPanelDatabaseAccess()
        ...
        if download_status != 1:
            raise RuntimeError(
                'Unable to prepare upgraded foxitiPanel source: %s' %
        ...
        if os.path.exists(integrationConfig):
            try:
                ...
                if product == 'av':
                    ...
                    imunifyAVPath = '/usr/local/FoxitiCP/public/imunifyav'

                    if os.path.exists(imunifyAVPath):
                        execPath = "/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/CLManager/CageFS.py"
                        command = execPath + " --function submitinstallImunifyAV"
                        Upgrade.executioner(command, command, 1)

                        # Set permissions on ImunifyAV execute file
                        imunifyAVExecute = '/usr/local/FoxitiCP/public/imunifyav/bin/execute.py'
                    ...
                    imunify360Path = '/usr/local/FoxitiCP/public/imunify'
                    ...
                    if os.path.exists(imunify360Path):
                        ...
                        imunify360Execute = '/usr/local/FoxitiCP/public/imunify/bin/execute.py'
    ...
    def restoreImunify360():
        ...
        try:
            ...
            imunifyPath = '/usr/local/FoxitiCP/public/imunify'
            imunifyAVPath = '/usr/local/FoxitiCP/public/imunifyav'
            ...
            if os.path.exists(imunifyAVPath):
                ...
                if os.path.exists(configPath):
                    ...
                    executePath = '/usr/local/FoxitiCP/public/imunifyav/bin/execute.py'
            ...
            if os.path.exists(imunifyPath):
                ...
                binDir = '/usr/local/FoxitiCP/public/imunify/bin'
                executeFile = '/usr/local/FoxitiCP/public/imunify/bin/execute.py'
    ...
    def finalImunifyPermissions():
        ...
        Upgrade.installDNS_foxitiPanelACMEFile()
        ...
        try:
            ...
            if os.path.exists(secret_path):
                shutil.chown(secret_path, user='foxitipanel', group='foxitipanel')
                os.chmod(secret_path, 0o600)

            service_source = '/usr/local/FoxitiCP/fastapi_ssh_server.service'
    ...
    @staticmethod
    def installDNS_foxitiPanelACMEFile():
        filePath = '/root/.acme.sh/dns_foxitipanel.sh'
        if os.path.exists(filePath):
            os.remove(filePath)
        shutil.copy('/usr/local/FoxitiCP/install/dns_foxitipanel.sh', filePath)
                ...
                if os.path.exists(ssl_conf) and os.path.exists(apache_conf):
                    ...
                    if 'Listen 8082' in backend_conf or 'Listen 8083' in backend_conf:
                        ...
                        for line in ssl_lines:
                            ...
                            if matched:
                                new_ssl_lines.append('%s# %s  # disabled by foxitiPanel: LiteSpeed owns 443'
