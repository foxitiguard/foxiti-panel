...
class phpUtilities:
    ...
    installLogPath = "/home/foxitipanel/phpExtensionRequestLog"
    ...
    def installPHPExtension(extension,extensions):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[installPHPExtension]")
    ...
    def unInstallPHPExtension(extension,extensions):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[unInstallPHPExtension]")
    ...
    def initiateInstall(extension):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [initiateInstall]")
    ...
    def savePHPConfigBasic(phpVers,allow_url_fopen,display_errors,file_uploads,allow_url_include,memory_limit,max_execution_time,upload_max_filesize,max_input_time,post_max_size, apache):
        try:
            ...
            if missing_directives:
                writeToFile.writelines("\n; Added by foxitiPanel PHP Config Manager\n")
    ...
    def GetPHPVersionFromFile(vhFile, domainName=None):
        ...
        if os.path.exists('/usr/local/FoxitiCP/debug'):
            logging.FoxitiCPLogFileWriter.writeToFile(f'Apache conf file in GetPHPVersionFromFile {finalConfPath}')
        ...
        if os.path.exists('/usr/local/FoxitiCP/debug'):
            logging.FoxitiCPLogFileWriter.writeToFile(f'VHFile in GetPHPVersion {vhFile}')
