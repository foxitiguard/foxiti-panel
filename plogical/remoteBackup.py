...
class remoteBackup:
    ...
    def getKey(ipAddress, password):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [getKey]")
            ...
            for backup in os.listdir(backupDir):
                ...
                if backup.endswith(ext):
                    ...
                    if data['restoreStatus'] == 1:
                        ...
                        logging.FoxitiCPLogFileWriter.writeToFile("Could not start restore process for: " + backup)
                        writeToFile = open(backupLogPath, "a")
                        writeToFile.writelines("\n")
                        writeToFile.writelines("\n")
                        writeToFile.writelines("[" + time.strftime(
    ...
    def remoteRestore(backupDir, dir):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [remoteRestore]")
            return [0, msg]
    ...
    def postRemoteTransfer(ipAddress, ownIP ,password, sshkey):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [postRemoteTransfer]")
            return [0, msg]
    ...
    def createBackup(virtualHost, ipAddress,writeToFile, dir):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [214:startBackup]")
    ...
    def sendBackup(completedPathToSend, IPAddress, folderNumber, writeToFile):
        try:
            ...
            if ACLManager.commandInjectionCheck(IPAddress) == 1:
                logging.FoxitiCPLogFileWriter.writeToFile("Invalid IP address - command injection attempt detected [sendBackup]")
                return
            ...
            if not re.match(ip_pattern, IPAddress):
                logging.FoxitiCPLogFileWriter.writeToFile("Invalid IP address format [sendBackup]")
                return

            # SECURITY: Validate folderNumber is alphanumeric
            if ACLManager.commandInjectionCheck(str(folderNumber)) == 1:
                logging.FoxitiCPLogFileWriter.writeToFile("Invalid folder number - command injection attempt detected [sendBackup]")
                return

            if not re.match(r'^[a-zA-Z0-9_-]+$', str(folderNumber)):
                logging.FoxitiCPLogFileWriter.writeToFile("Invalid folder number format [sendBackup]")
                return
            ...
            if '..' in completedPathToSend or not completedPathToSend.startswith('/home/backup/'):
                logging.FoxitiCPLogFileWriter.writeToFile("Invalid backup path - path traversal attempt detected [sendBackup]")
                return

            # SECURITY: Use shlex.quote for all user-controllable parameters
            command = 'sudo rsync -avz -e "ssh -i /root/.ssh/foxitipanel -o StrictHostKeyChecking=no" ' + shlex.quote(completedPathToSend) + ' root@' + shlex.quote(IPAddress) + ':/home/backup/transfer-' + shlex.quote(str(folderNumber))
            subprocess.call(shlex.split(command), stdout=writeToFile)
            os.remove(completedPathToSend)

        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [sendBackup]")
    ...
    def backupProcess(ipAddress, dir, backupLogPath,folderNumber, accountsToTransfer):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [backupProcess]")
    ...
    def remoteTransfer(ipAddress, dir,accountsToTransfer):
        try:
            ...
            if backupUtil.backupUtilities.checkIfHostIsUp(ipAddress) == 1:
                ...
                if checkConn[0] == 0:
                    writeToFile.writelines("[" + time.strftime(
                        "%m.%d.%Y_%H-%M-%S") + "]" + " Connection to:" + ipAddress + " Failed, please resetup this destination from foxitiPanel, aborting." + "\n")
                    writeToFile.close()
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [remoteTransfer]")
            return [0, msg]
