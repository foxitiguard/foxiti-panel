...
class remoteTransferUtilities:
    ...
    def writeAuthKey(pathToKey):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile("For remote transfer, I am not able to write key to auth file, Error Message: "+str(msg))
    ...
    def remoteTransfer(ipAddress, dir, accountsToTransfer, sshPort='22'):
        try:
            ...
            if backupUtil.backupUtilities.checkIfHostIsUp(ipAddress) == 1:
                ...
                if checkConn[0] == 0:
                    writeToFile.writelines("[" + time.strftime(
                        "%m.%d.%Y_%H-%M-%S") + "]" + " Connection to:" + ipAddress + " Failed, please resetup this destination from foxitiPanel, aborting. [5010]" + "\n")
                    writeToFile.close()
                    return
                else:
                    pass
            ...
            try:
                ...
                for virtualHost in accountsToTransfer:
                    try:
                        ...
                        allBackupsSent = False
                        logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [remoteTransferUtilities.backupProcess:173]")
    ...
    def sendBackup(completedPathToSend, IPAddress, folderNumber, writeToFile,
        ...
        try:
            ...
            command = "sudo scp -o StrictHostKeyChecking=no -i /root/.ssh/foxitipanel -P "+ sshPort + " " + completedPathToSend + " root@" + IPAddress + ":/home/backup/transfer-" + folderNumber + "/"
            ...
            if return_Code == 0:
                logging.FoxitiCPLogFileWriter.writeToFile("Remote backup file sent: %s" % completedPathToSend)
                writeToFile.writelines("[" + time.strftime(
