...
from foxitipanel_version import (
    BUILD,
    VERSION,
...
class backupUtilities:
    ...
    completeKeyPath = "/home/foxitipanel/.ssh"
    destinationsPath = "/home/foxitipanel/destinations"
    licenseKey = '/usr/local/lsws/conf/license.key'
    NiceDefault = '10'
FPUDefault = '1000'
    CloudBackupConfigPath = '/home/foxitipanel/CloudBackup.json'
    time = 10
    ...
    def prepareBackupMeta(backupDomain, backupName, tempStoragePath, backupPath, FromInner=1):
        try:
            ...
            if FromInner:
                ...
                #logging.FoxitiCPLogFileWriter.statusWriter(status, 'Setting up meta data..')
                command = f"echo 'Setting up meta data..' > {status}"
                ProcessUtilities.executioner(command, website.externalApp)
            else:
                status = '/home/foxitipanel/dummy'

            if os.path.exists(ProcessUtilities.debugPath):
                logging.FoxitiCPLogFileWriter.writeToFile(f'Creating meta for {backupDomain}.')
            ...
            ### try to take care of - https://github.com/usmannasir/foxitipanel/issues/1196
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile('%s. [167:prepMeta]' % (str(msg)))
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile('%s. [158:prepMeta]' % (str(msg)))
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile('%s. [179:prepMeta]' % (str(msg)))
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.FoxitiCPLogFileWriter.writeToFile(f'Path to meta file {metaPath}')
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f"{str(msg)} [207][5009]")
            if FromInner:
                #logging.FoxitiCPLogFileWriter.statusWriter(status, "%s [207][5009]" % (str(msg)), status)
    ...
    def startBackup(tempStoragePath, backupName, backupPath, metaPath=None):
        try:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(backupFileNamePath, backupName)

            #####

            status = os.path.join(backupPath, 'status')

            logging.FoxitiCPLogFileWriter.statusWriter(status, "Making archive of home directory.\n")
            ...
            ### doing backup of whole dir and keeping it in public_html folder will restore from here - ref https://github.com/usmannasir/foxitipanel/issues/1196
            command = (
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Backing up databases..")
            print('1,None')
            ...
            status = os.path.join(backupPath, 'status')
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Aborted, " + str(msg) + ".[365] [5009]")
    ...
    def BackupRoot(tempStoragePath, backupName, backupPath, metaPath=None, externalApp = None, CPHomeStorage=None):
        ...
        ### CPHomeStorage /home/foxitipanel/<random_number>
        ...
        if os.path.exists(sslStoragePath):
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(f'{str(msg)}. [283:startBackup]')
        ...
        try:
            for childDomain in childDomains:
                ...
                TempConfPath = f'/home/foxitipanel/{actualChildDomain}.vhost.conf'
        ...
        if os.path.islink(status) or os.path.islink(tempStoragePath or os.path.islink(backupPath)) or os.path.islink(
                metaPath):
            logging.FoxitiCPLogFileWriter.writeToFile('symlinked.')
            #logging.FoxitiCPLogFileWriter.statusWriter(status, 'Symlink attack. [365][5009]')
            return 0

        ## backup email accounts

        if externalApp == None:
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Backing up email accounts..\n")
        else:
        ...
        try:
            ...
            if externalApp == None:
                logging.FoxitiCPLogFileWriter.statusWriter(status, "Preparing final compressed package..\n")
            else:
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile('%s. [backupRoot:499]' % str(msg))
                for items in backupObs:
                    items.status = 1
                    items.size = totalSize
                    items.save()
            ...
            if externalApp == None:
                logging.FoxitiCPLogFileWriter.statusWriter(status, "Completed\n")
            else:
            ...
            if externalApp == None:
                logging.FoxitiCPLogFileWriter.statusWriter(status, '%s. [511:BackupRoot][[5009]]\n' % str(msg))
            else:
                command = 'echo %s > %s' % (
    ...
    def initiateBackup(tempStoragePath, backupName, backupPath):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [initiateBackup]")
    ...
    def createWebsiteFromBackup(backupFileOrig, dir):
        try:
            ...
            for database in databases:
                ...
                result = mysqlUtilities.mysqlUtilities.createDatabaseAndRegister(dbName, dbUser, 'foxitipanel', website)
                if result[0] != 1:
    ...
    def startRestore(backupName, dir):
        try:
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(pid, str(os.getpid()))

            status = os.path.join(completPath, 'status')
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Extracting Main Archive!")
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Creating Accounts,Databases and DNS records!")
            ...
            if result[0] == 1:
                ...
                if os.path.exists(sslStoragePath):
                    ...
                    try:
                        ...
                        logging.FoxitiCPLogFileWriter.writeToFile('%s. [555:startRestore]' % (str(msg)))

            else:
                logging.FoxitiCPLogFileWriter.statusWriter(status, "Error Message: " + result[
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Creating Child Domains!")
            ...
            try:
                for childDomain in childDomains:
                    ...
                    if retValues[0] == 1:
                        ...
                        try:
                            ...
                            if os.path.exists(completPath + '/' + domain + '.vhost.conf'):
                                ...
                                if CurrentServer == ProcessUtilities.decideServer():
                                    ...
                                    url = "https://cp.foxitiguard.com/CyberpanelAdOns/Adonpermission"
                                    data = {
                                        "name": "all",
                                    ...
                                    if (Status == 1):
                                        ...
                                        tempStatusPath = '/home/foxitipanel/fakePath'

                                        if os.path.exists(ProcessUtilities.debugPath):
                                            logging.FoxitiCPLogFileWriter.writeToFile(f'Conf path of apache for child domain {domain} in backup is {childConfPathinBKUPApache}')
                                        ...
                                        if childData.find('proxyApacheBackendSSL') > -1 and os.path.exists(childConfPathinBKUPApache):
                                            ...
                                            if os.path.exists(finalConfPathApache):
                                                ...
                                                if os.path.exists(ProcessUtilities.debugPath):
                                                    logging.FoxitiCPLogFileWriter.writeToFile(
                                                        f'foxitiPanel was able to successfully convert {domain} to apache conf as {finalConfPathApache} exists..')
                        ...
                        logging.FoxitiCPLogFileWriter.writeToFile('Error domain %s' % (domain))
                        logging.FoxitiCPLogFileWriter.statusWriter(status, "Error Message: " + retValues[
                ...
                status.close()
                logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [startRestore]")
                return 0

            ## Restore Aliases

            logging.FoxitiCPLogFileWriter.statusWriter(status, "Restoring Domain Aliases!")
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Restoring email accounts!")
            ...
            try:
                ...
                logging.FoxitiCPLogFileWriter.statusWriter(status, "Error Message: " + str(
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Restoring Databases!")
            ...
            for database in databases:
                ...
                if backup_uses_database_users_schema(backup_version, backup_build):
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile('Multi-user database backup metadata detected..')
                    ...
                    for databaseUser in databaseUsers:
                        ...
                        if os.path.exists(ProcessUtilities.debugPath):
                            ...
                            logging.FoxitiCPLogFileWriter.writeToFile('Database user: %s' % (dbUser))
                            logging.FoxitiCPLogFileWriter.writeToFile('Database host: %s' % (dbHost))
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Extracting web home data!")
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Extracting email accounts!")
            ...
            if os.path.exists(completPath + '/vhost.conf'):
                if CurrentServer == ProcessUtilities.decideServer():
                    ...
                    if (Status == 1):
                        ...
                        if os.path.exists(ProcessUtilities.debugPath):
                            logging.FoxitiCPLogFileWriter.writeToFile(f'Conf path of apache for main site {masterDomain} in backup is {confPathApache}')
                        ...
                        if childData.find('proxyApacheBackendSSL') > -1 and os.path.exists(confPathApache):
                            ...
                            if os.path.exists(ProcessUtilities.debugPath):
                                logging.FoxitiCPLogFileWriter.writeToFile(
                                    f'Apache conf path of main domain exists which means foxitiPanel successfully converted site to Apache for {masterDomain}')
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(status, "Done")

        except BaseException as msg:
            status = os.path.join(completPath, 'status')
            logging.FoxitiCPLogFileWriter.statusWriter(status, str(msg) + " [736][5009]")
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [startRestore]")
    ...
    def initiateRestore(backupName, dir):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [initiateRestore]")
    ...
    #         ## Temp changes
    #
    #         command = 'chmod 600 %s' % ('/root/.ssh/foxitipanel.pub')
    #         ProcessUtilities.executioner(command)
    #
    #         command = "scp -o StrictHostKeyChecking=no -P " + port + " /root/.ssh/foxitipanel.pub " + user + "@" + IPAddress + ":~/.ssh/authorized_keys"
    #         setupKeys = pexpect.spawn(command, timeout=3)
    #
    #         if os.path.exists(ProcessUtilities.debugPath):
    #             logging.FoxitiCPLogFileWriter.writeToFile(command)
    #
    ...
    #         ## Temp changes
    #
    #         command = 'chmod 644 %s' % ('/root/.ssh/foxitipanel.pub')
    #         ProcessUtilities.executioner(command)
    #
    #         return [1, "None"]
    #
    ...
    #         ProcessUtilities.executioner(command)
    #
    #         logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [sendKey]")
    ...
    #             pass
    #         else:
    #             logging.FoxitiCPLogFileWriter.writeToFile("Host is Down.")
    ...
    def sendKey(IPAddress, password, port='22', user='root'):
        try:
            ...
            if os.path.exists('/root/.ssh/foxitipanel.pub'):
                pass
            else:
            ...
            try:
                ...
                sftp.put('/root/.ssh/foxitipanel.pub', '.ssh/authorized_keys')
                sftp.close()
    ...
    def setupSSHKeys(IPAddress, password, port='22', user='root'):
        try:
            ...
            if password != 'NOT-NEEDED':
                ...
                private_key_path = '/root/.ssh/foxitipanel'
    ...
    def checkIfHostIsUp(IPAddress):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[checkIfHostIsUp]")
    ...
    def checkConnection(IPAddress, port='22', user='root'):
        try:
            ...
            command = "sudo ssh -i /root/.ssh/foxitipanel -o StrictHostKeyChecking=no -p " + port + ' ' + user + "@" + IPAddress
            ...
            logging.FoxitiCPLogFileWriter.writeToFile("Timeout " + IPAddress + " [checkConnection]")
            return [0, "371 Timeout while making connection to this server [checkConnection]"]
        except pexpect.EOF as msg:
            logging.FoxitiCPLogFileWriter.writeToFile("EOF " + IPAddress + "[checkConnection]")
            return [0, "374 Remote Server is not able to authenticate for transfer to initiate. [checkConnection]"]
        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " " + IPAddress + " [checkConnection]")
    ...
    def verifyHostKey(IPAddress, port='22', user='root'):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile("Timeout [verifyHostKey]")
            return [0, "Timeout [verifyHostKey]"]
        except pexpect.EOF as msg:
            logging.FoxitiCPLogFileWriter.writeToFile("EOF [verifyHostKey]")
            return [0, "EOF [verifyHostKey]"]
        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [verifyHostKey]")
    ...
    def createBackupDir(IPAddress, port='22', user='root'):
        ...
        try:
            ...
            command = "sudo ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/foxitipanel " + user + "@" + IPAddress + " mkdir ~/backup"
            ...
            command = "sudo ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/foxitipanel " + user + "@" + IPAddress + ' "cat ~/.ssh/authorized_keys ~/.ssh/temp > ~/.ssh/authorized_temp"'
            ...
            command = "sudo ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/foxitipanel " + user + "@" + IPAddress + ' "cat ~/.ssh/authorized_temp > ~/.ssh/authorized_keys"'
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [createBackupDir]")
    ...
    def host_key_verification(IPAddress):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [host_key_verification]")
            return 0
    ...
    def getAliases(masterDomain):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "  [getAliases]")
            print(0)
    ...
    def CheckIfSleepNeeded(self):
        import psutil
        while (1):
            logging.FoxitiCPLogFileWriter.writeToFile('Current CPU percent %s.' % (int(psutil.cpu_percent(interval=None))))
            if int(psutil.cpu_percent(interval=None)) > int(self.cpu):
                logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
        ...
        try:
            ...
            for items in self.website.databases_set.all():
                ...
                if isinstance(dumpResult, tuple):
                    ...
                    dumpOk = dumpResult
                if dumpOk == 0:
                    errorMessage = ('Failed to back up database %s. The mysqldump command failed — check the foxitiPanel '
                                    'log for the exact error. Common causes: a stale /home/foxitipanel/.my.cnf (delete it '
    ...
    def CloudBackups(self):
        ...
        command = 'nice -n %s tar czf %s.tar.gz -C %s %s' % (self.nice, self.BackupPath, '/home/foxitipanel/backups/%s' % (self.extraArgs['domain']), self.BackupPath.split('/')[-1])
        ProcessUtilities.executioner(command)
        ...
        command = 'chown foxitipanel:foxitipanel %s' % (finalPath)
        ProcessUtilities.executioner(command)
        ...
        if self.extraArgs['port'] != 0:
            ...
            command = "scp -o StrictHostKeyChecking=no -P %s -i /root/.ssh/foxitipanel %s root@%s:/home/foxitipanel/backups/%s/" % (self.extraArgs['port'], finalPath, self.extraArgs['ip'], self.extraArgs['destinationDomain'])
            ProcessUtilities.outputExecutioner(command)

        logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Completed [200].')
    ...
    def SubmitCloudBackupRestore(self):
        try:
            ...
            self.BackupPath = '/home/foxitipanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'])
            self.website = Websites.objects.get(domain=self.extraArgs['domain'])
            ...
            command = 'tar -xf %s -C %s' % (self.BackupPath, '/home/foxitipanel/backups/%s/' % (self.extraArgs['domain']))
            ProcessUtilities.executioner(command)

            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
                                                      'Main Archive extracted,20')

            self.extractedPath = '/home/foxitipanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'].rstrip('.tar.gz'))
                    ...
                    for child in childDomains:
                        try:
                            ...
                            virtualHostUtilities.createDomain(self.website.domain, child['domain'], child['php'], child['path'], 1, 0, 0,
                                                              self.website.admin.userName, 0, "/home/foxitipanel/" + str(randint(1000, 9999)))

                except BaseException as msg:
                    logging.FoxitiCPLogFileWriter.writeToFile('%s [SubmitCloudBackupRestore:1533]' % str(msg))
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], '%s [404].' % str(msg))

    ### Cloud Backup functions ends

    def fetchAWSKeys(self):
        path = '/home/foxitipanel/.aws'
    ...
    def SubmitS3BackupRestore(self):
        ...
        try:
            ...
            self.BackupPath = '/home/foxitipanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'].split('/')[-1])
            ...
            self.extractedPath = '/home/foxitipanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'].split('/')[-1].rstrip('.tar.gz'))
...
def submitBackupCreation(tempStoragePath, backupName, backupPath, backupDomain):
    try:
        ...
        ## /home/example.com/backup - backupPath
        ## /home/foxitipanel/1047.xml - metaPath
        ## /home/backup/<random_number> - CPHomeStorage
    ...
    FPHomeStorage = f'/home/backup/{generate_pass(5)}'
        ...
        schedulerPath = f'/home/foxitipanel/{backupDomain}-backup.txt'
        ...
        logging.FoxitiCPLogFileWriter.writeToFile(backupPath)
        logging.FoxitiCPLogFileWriter.writeToFile(tempStoragePath)

        execPath = "sudo nice -n 10 /usr/local/FoxitiCP/bin/python " + virtualHostUtilities.cyberPanel + "/plogical/backupUtilities.py"
        ...
        if output.find('[5009]') > -1:
            logging.FoxitiCPLogFileWriter.writeToFile(output)
            writeToFile = open(schedulerPath, 'w')
        ...
        command = f'chown foxitipanel:foxitipanel {result[2]}'
        ProcessUtilities.executioner(command)
        ...
        for database in databases:
            ...
            dbName = database.find('dbName').text
            res = mysqlUtilities.mysqlUtilities.createDatabaseBackup(dbName, '/home/foxitipanel')
            if res == 0:
                ## This login can be further improved later.
                logging.FoxitiCPLogFileWriter.writeToFile('Failed to create database backup for %s. This could be false positive, moving on.' % (dbName))

            # Move database backup (check for both .sql.gz and .sql)
            if os.path.exists(f'/home/foxitipanel/{dbName}.sql.gz'):
                command = f'mv /home/foxitipanel/{dbName}.sql.gz {CPHomeStorage}/{dbName}.sql.gz'
                ProcessUtilities.executioner(command)
                # Also move metadata file if it exists
                if os.path.exists(f'/home/foxitipanel/{dbName}.backup.json'):
                    command = f'mv /home/foxitipanel/{dbName}.backup.json {CPHomeStorage}/{dbName}.backup.json'
                    ProcessUtilities.executioner(command)
            elif os.path.exists(f'/home/foxitipanel/{dbName}.sql'):
                command = f'mv /home/foxitipanel/{dbName}.sql {CPHomeStorage}/{dbName}.sql'
                ProcessUtilities.executioner(command)
        ...
        ProcessUtilities.executioner(command, 'foxitipanel')
...
def cancelBackupCreation(backupCancellationDomain, fileName):
    try:
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f"{str(msg)} [cancelBackupCreation]")
...
def main():
    parser = argparse.ArgumentParser(description='foxitiPanel Backup Generator')
