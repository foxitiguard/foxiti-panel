...
class modSec:
    ...
    installLogPath = "/home/foxitipanel/modSecInstallLog"
    tempRulesFile = "/home/foxitipanel/tempModSecRules"
    mirrorPath = "foxitipanel.net"
    ...
    MODSEC_COMPATIBLE = {'rhel8': {'url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-rhel8.so',
               'sha256': 'cfdf61bb3e0115fbcd172a5dd55fe107a8e17888711a31eec25d34b94df3bb6c'},
     'rhel9': {'url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-rhel9.so',
               'sha256': 'eb67cce467b29b73f70f798db8e5097b13e8c264a83b146bac601bbd62399b0f'},
     'rhel10': {'url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-rhel10.so',
                'sha256': 'a7d8131bf7fa9b14286a088a1a9eb8f0bca15de991c79d6173ac0f274dcd9bcf'},
     'ubuntu': {'url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-ubuntu.so',
                'sha256': '0714c9e43781d51ffab5ee4faf2b4506b68cc0f9483285bfa8a8d334d0f96172'},
     'ubuntu26': {'url': 'https://foxitipanel.net/mod_security-2.5.4-x86_64-ubuntu26.so',
    ...
    def downloadCompatibleModSec(platform):
        ...
        try:
            ...
            with open('/usr/local/lsws/bin/openlitespeed', 'rb') as core:
                if b'foxitipanel_lsi_api_abi_v1' not in core.read():
                    logging.FoxitiCPLogFileWriter.writeToFile(
            ...
            with tempfile.TemporaryDirectory(prefix='.modsec-', dir=os.path.dirname(modsec_path)) as staging:
                ...
                if digest.hexdigest() != config['sha256']:
                    logging.FoxitiCPLogFileWriter.writeToFile('ModSecurity checksum mismatch')
                    return False
                ...
                if dependencies.returncode != 0 or 'not found' in dependencies.stdout + dependencies.stderr:
                    logging.FoxitiCPLogFileWriter.writeToFile('ModSecurity runtime dependencies are unavailable')
                    return False
                if os.path.exists(modsec_path):
                    shutil.copy2(modsec_path, modsec_path + '.stock')
                os.chmod(tmp_path, 0o644)
                os.replace(tmp_path, modsec_path)
            logging.FoxitiCPLogFileWriter.writeToFile('Installed compatible ModSecurity binary')
            return True
        except Exception as error:
            logging.FoxitiCPLogFileWriter.writeToFile(str(error) + ' [downloadCompatibleModSec]')
            return False
    ...
    def isCustomOLSBinaryInstalled():
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f"WARNING: Could not detect OLS binary type: {msg}")
            return False
    ...
    def detectBinarySuffix():
        ...
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f"Error detecting OS: {msg}, defaulting to Ubuntu binaries")
            return 'ubuntu'
    ...
    def installCompatibleModSecurity():
        ...
        try:
            ...
            BASE_URL = "https://foxitipanel.net/binaries"
            ...
            if result != 0 or not os.path.exists(tmp_modsec):
                ...
                logging.FoxitiCPLogFileWriter.writeToFile("[Could not download compatible ModSecurity]")
                return 0
            ...
            if actual_sha256 != EXPECTED_SHA256:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile("[ModSecurity checksum verification failed]")
                return 0
            ...
            logging.FoxitiCPLogFileWriter.writeToFile("[Compatible ModSecurity installed successfully]")
            return 1
            ...
            with open(modSec.installLogPath, 'a') as f:
                ...
                f.write("Can not be installed.[404]\n")
            logging.FoxitiCPLogFileWriter.writeToFile("[Timeout during ModSecurity installation]")
            return 0
        except Exception as msg:
            with open(modSec.installLogPath, 'a') as f:
                f.write(f"ERROR: {str(msg)}\n")
                f.write("Can not be installed.[404]\n")
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[installCompatibleModSecurity]")
            return 0
    ...
    def installModSec():
        try:
            ...
            if res == 1:
                ...
                writeToFile.close()
                logging.FoxitiCPLogFileWriter.writeToFile("[Could not Install]")
                return 0
            else:
                writeToFile = open(modSec.installLogPath, 'a')
            ...
            custom_ols_marker = "/usr/local/lsws/modules/foxitipanel_ols.so"
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + "[installModSec]")
        ...
        try:
            ...
            if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
                ...
                try:
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + ' [disableComodo]')
