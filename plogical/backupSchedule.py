...
class backupSchedule:
    ...
    ERROR = 1
    backupLog = ''
    runningPath = '/home/foxitipanel/remoteBackupPID'
    ...
    def createLocalBackup(virtualHost, backupLogPath):
        try:
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.FoxitiCPLogFileWriter.writeToFile(r.text)
            ...
            time.sleep(5)
            schedulerPath = '/home/foxitipanel/%s-backup.txt' % (virtualHost)
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [119:startBackup]")
            backupSchedule.remoteBackupLogging(backupLogPath,
                                               "Local backup creating failed for %s, Error message: %s" % (
    ...
    def createBackup(virtualHost, ipAddress, backupLogPath , port='22', user='root'):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [backupSchedule.createBackup]")
    ...
    def sendBackup(backupPath, IPAddress, backupLogPath , port='22', user='root'):
        try:
            ...
            command = "scp -o StrictHostKeyChecking=no -P "+port+" -i /root/.ssh/foxitipanel " + backupPath + " " + user + "@" + IPAddress+":" + remote_dir
            ...
            if result != 0:
                ...
                try:
                    ...
                    try:
                        private_key = paramiko.RSAKey.from_private_key_file('/root/.ssh/foxitipanel')
                    ...
                    logging.FoxitiCPLogFileWriter.writeToFile(f"Successfully transferred backup via SFTP to {IPAddress}")
                except BaseException as msg:
                    writeToFile.write(f"SFTP transfer failed: {str(msg)}\n")
                    logging.FoxitiCPLogFileWriter.writeToFile(f"SFTP transfer failed: {str(msg)}")
                    raise
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [189:startBackup]")
    ...
    def prepare():
        try:
            ...
            if os.path.exists(backupSchedule.runningPath):
                ...
                if output.find('/usr/local/FoxitiCP/plogical/backupSchedule.py') > -1 and output.find(pid) > -1:
                    print(
            ...
            if checkConn[0] == 0:
                backupSchedule.remoteBackupLogging(backupLogPath,
                                                   "Connection to: " + ipAddress + " Failed, please resetup this destination from foxitiPanel, aborting.")
                return 0
            else:
                ## Create backup dir on remote server in ~/backup

                command = "ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/foxitipanel " + user + "@" + ipAddress + " mkdir -p ~/backup/" + ipAddressLocal + "/" + time.strftime(
                    "%a-%b")
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [prepare]")
...
def handler(signum, frame):
    ...
    logging.FoxitiCPLogFileWriter.writeToFile('Signal: %s, time spent: %s' % (str(signum), str(diff.total_seconds())))
