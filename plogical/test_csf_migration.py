...
class CSFMigrationTests(unittest.TestCase):
    def setUp(self):
        ...
        if (ROOT / 'foxitipanel_firewall_migration.py').exists():
            definitions('foxitipanel_firewall_migration.py',
    ...
    def test_command_line_refuses_before_database_imports(self):
        ...
        def guarded_import(name, *args, **kwargs):
            imports.append(name)
            if name in ('MySQLdb', 'FoxitiCP', 'foxitipanel_version'):
...
class CSFCloudUpgradeTests(unittest.TestCase):
    def test_cloud_refusal_does_not_backup_or_restore_installation(self):
        ...
        namespace = definitions('plogical/foxitiPanelUpgrade.py',
                                {'UpgradefoxitiPanel'}, {'os': os_proxy})
        cloud = namespace['UpgradefoxitiPanel'].__new__(namespace['UpgradefoxitiPanel'])
        cloud.branch = 'stable'
    ...
    def test_cloud_cli_propagates_refusal_as_nonzero(self):
        ...
        namespace = definitions('plogical/foxitiPanelUpgrade.py', {'main'},
                                {'argparse': argparse, 'UpgradefoxitiPanel': cloud, 'sys': sys})
        namespace['__name__'] = '__main__'
        guard = source_tree('plogical/foxitiPanelUpgrade.py').body[-1]
        with patch.object(sys, 'argv', ['foxitiPanelUpgrade.py', '--branch', 'stable',
                                      '--mail', '1', '--dns', '1', '--ftp', '1']):
            with self.assertRaises(SystemExit) as stopped:
                exec(compile(ast.Module(body=[guard], type_ignores=[]),
                             'plogical/foxitiPanelUpgrade.py', 'exec'), namespace)
        self.assertEqual(1, stopped.exception.code)
...
class CSFUpgradeRequestTests(unittest.TestCase):
    ...
    def test_panel_nonadmin_keeps_acl_denial(self):
        ...
        worker.assert_not_called()
        self.assertNotIn('foxitipanel_firewall_migration', imports)
    ...
    def cloud_request(self, present):
        ...
        method = next(n for n in node.body
                      if isinstance(n, ast.FunctionDef) and n.name == 'SubmitfoxitiPanelUpgrade')
        ...
        instance = types.SimpleNamespace(data={'foxitiPanelBranch': 'stable',
                                              'mail': 1, 'dns': 0, 'ftp': 1})
        instance.ajaxPre = types.MethodType(namespace['ajaxPre'], instance)
        return namespace['SubmitfoxitiPanelUpgrade'](instance), process
    ...
    def test_cloud_request_without_csf_keeps_existing_launch(self):
        ...
        process.executioner.assert_called_once_with(
            '/usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/foxitiPanelUpgrade.py '
            '--branch stable --mail 1 --dns 0 --ftp 1')


class CSFShellUpgradeTests(unittest.TestCase):
    def run_prefix(self, kind):
        script = (ROOT / 'foxitipanel_upgrade.sh').read_text()
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            paths = ['/etc/foxitipanel', '/etc/csf', '/etc/cxs', '/var/log',
                     '/usr/local/FoxitiCP', '/home/foxitipanel/plugins']
            for path in paths:
                (root / path.lstrip('/')).parent.mkdir(parents=True, exist_ok=True)
            (root / 'etc/foxitipanel').mkdir()
            (root / 'var/log').mkdir()
            marker = root / 'etc/csf'
            if kind == 'directory':
                marker.mkdir()
            ...
            fixtures = [
                '/usr/local/FoxitiCP/configservercsf/plugin.py',
                '/usr/local/FoxitiCP/public/static/configservercsf/plugin.js',
                '/home/foxitipanel/plugins/configservercsf',
                '/usr/local/FoxitiCP/FoxitiCP/settings.py',
                '/usr/local/FoxitiCP/FoxitiCP/urls.py',
                '/usr/local/FoxitiCP/baseTemplate/templates/baseTemplate/index.html',
            ]
            before = {}
    ...
    def test_standalone_upgrader_stages_guard_dependency(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = subprocess.run(['bash', '-c', harness], cwd=temporary,
                ...
                capture_output=True, text=True, timeout=10)
            self.assertEqual(0, result.returncode, result.stderr)
            self.assertEqual((ROOT / 'foxitipanel_firewall_migration.py').read_bytes(),
                             (root / 'foxitipanel_firewall_migration.py').read_bytes())
            probe = (
                'import os,runpy\n'
    ...
    def test_older_upgrader_without_guard_import_does_not_fetch_new_helper(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (source / 'foxitipanel_version.py').write_text('VERSION = "3.0"\n')
            harness = ('Download_Upgrade_Source() { cp "$SOURCE_ROOT/$1" "$2"; }\n'
                       + script[start:end])
            result = subprocess.run(['bash', '-c', harness], cwd=temporary,
            ...
            self.assertFalse((root / 'foxitipanel_firewall_migration.py').exists())
