...
class sslUtilities:
    ...
    def FindIfDomainInCloudflare(virtualHostName):
        try:
            ...
            if RetStatus:
                ...
                logging.FoxitiCPLogFileWriter.writeToFile(f'top level domain in cf: {topLevelDomain}')
                import CloudFlare
                ...
                for zone in sorted(zones, key=lambda v: v['name']):
                    logging.FoxitiCPLogFileWriter.writeToFile(f'zone: {zone["name"]}')
                    if zone['name'] == topLevelDomain:
                        if zone['status'] == 'active':
                            return 1, None
                        else:
                            logging.FoxitiCPLogFileWriter.writeToFile(f'zone is not active in cf: {zone["name"]}')
    ...
    def FindIfDomainInPowerDNS(virtualHostName):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(f'PDNS Result: {str(result)}.')
    ...
    def obtainSSLForADomain(virtualHostName, adminEmail, sslpath, aliasDomain=None, forceIssue=False):
        ...
        if CF_Check:
            ...
            foxitiPanel_Check, message = sslUtilities.FindIfDomainInPowerDNS(virtualHostName)

            if foxitiPanel_Check:
                DNS_TO_USE = 'dns_foxitipanel'
            else:
        ...
        try:
            ...
            #     acmePath = '/home/foxitipanel/.acme.sh/acme.sh'
            ...
            if aliasDomain is None:
                ...
                try:
                    ...
                    # ResultText = open(logging.FoxitiCPLogFileWriter.fileName, 'r').read()
                    # CurrentMessage = "Trying to obtain SSL for: " + virtualHostName + " and: www." + virtualHostName
                    # logging.FoxitiCPLogFileWriter.writeToFile(CurrentMessage, 0)

                    logging.FoxitiCPLogFileWriter.writeToFile(command, 0)

                    output = subprocess.check_output(shlex.split(command)).decode("utf-8")
                    logging.FoxitiCPLogFileWriter.writeToFile(
                        "Successfully obtained SSL for: " + virtualHostName + " and: www." + virtualHostName, 0)

                    logging.FoxitiCPLogFileWriter.SendEmail(sender_email, adminEmail, output,
                    ...
                    try:
                        ...
                        finalText = '%s\nTrying to obtain SSL for: %s' % (finalText, virtualHostName)
                        logging.FoxitiCPLogFileWriter.writeToFile("Trying to obtain SSL for: " + virtualHostName, 0)
                        logging.FoxitiCPLogFileWriter.writeToFile(command)
                        output = subprocess.check_output(shlex.split(command)).decode("utf-8")
                        logging.FoxitiCPLogFileWriter.writeToFile(
                            "Successfully obtained SSL for: " + virtualHostName, 0)
                        finalText = '%s\nSuccessfully obtained SSL for: %s.' % (finalText, virtualHostName)
                        logging.FoxitiCPLogFileWriter.SendEmail(sender_email, adminEmail, finalText,
                                                               'SSL Notification for %s.' % (virtualHostName))

                    except subprocess.CalledProcessError:
                        logging.FoxitiCPLogFileWriter.writeToFile(
                            'Failed to obtain SSL, issuing self-signed SSL for: ' + virtualHostName, 0)
                        logging.FoxitiCPLogFileWriter.SendEmail(sender_email, adminEmail,
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + " [Failed to obtain SSL. [obtainSSLForADomain]]")
            return 0, str(msg)
