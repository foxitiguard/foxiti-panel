...
PowerDNS 4.7+ (including 4.8/4.9/5.0.x) require but older foxitiPanel installs
never received.
...
transitive dependency of unrelated package upgrades) but foxitiPanel has
...
allows VARCHAR(64000). foxitiPanel's Django ORM, however, may have created
...
Invocation
----------
    From foxitiPanel upgrade  : Upgrade.pdnsSchemaMigrations() in upgrade.py
    From fresh install       : right before startPowerDNS()
    Directly (customer SOS)  :
        /usr/local/FoxitiCP/bin/python /usr/local/FoxitiCP/plogical/pdnsSchemaMigration.py
"""
...
PDNS_DB_NAME_DEFAULT = 'foxitipanel'
MYSQL_PASSWORD_FILE = '/etc/foxitipanel/mysqlPassword'
HEALTH_FILE = '/etc/foxitipanel/health.json'
ERROR_LOG_FILE = '/home/foxitipanel/error-logs.txt'
...
def _connect(db_name):
    ...
    try:
        ...
        from FoxitiCP import settings as django_settings
