...
def _temporary(path, data, metadata):
    descriptor, name = tempfile.mkstemp(prefix='.foxitipanel-sshd-', dir=str(path.parent))
    temporary = Path(name)
    try:
...
def update_ssh_config(path, port, root_login, prepare, restart, is_active,
    ...
    lock_path = path.parent / '.foxitipanel-sshd.lock'
    ...
    try:
        ...
        backup_root = Path(backup_directory or '/var/lib/foxitipanel/ssh-config-backups')
