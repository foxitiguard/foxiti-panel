...
app.controller('readFoxitiCPLogFile', function ($scope, $http) {
...
app.controller('listOSPackages', function ($scope, $http, $timeout) {
    ...
    $scope.fetchPackages = function (type = 'installed') {
        if (type === 'upgrade') {
            $scope.currentTab = 'upgrade';
        } else if (type === 'foxitiPanel') {
            $scope.currentTab = 'foxitipanel';
        } else {
    ...
    $scope.changeCPPort = function () {
        ...
        function ListInitialData(response) {
            ...
            if (response.data.status === 1) {
                new PNotify({
                    title: 'Success!',
                    text: 'Port changed, open foxitiPanel on new port.',
                    type: 'success'
                });
            } else {
                new PNotify({
