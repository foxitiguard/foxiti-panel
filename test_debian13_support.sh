...
# Debian 13 Support Test Script for foxitiPanel
# This script tests the compatibility of foxitiPanel with Debian 13
...
# Function to test foxitiPanel OS detection logic
test_foxitipanel_os_detection() {
    print_test_header "foxitiPanel OS Detection Logic Test"
    
    # Test the OS detection logic from foxitipanel.sh
    if grep -q -E "Debian GNU/Linux 11|Debian GNU/Linux 12|Debian GNU/Linux 13" /etc/os-release; then
        print_success "foxitiPanel OS detection logic recognizes Debian 11/12/13"
    else
        print_error "foxitiPanel OS detection logic does not recognize current Debian version"
        return 1
    fi
}
...
test_web_server_compatibility() {
    ...
    if command -v apache2 >/dev/null 2>&1; then
        ...
        print_warning "Apache2 not found (will be installed by foxitiPanel)"
    fi
...
run_all_tests() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  foxitiPanel Debian 13 Compatibility Test${NC}"
    echo -e "${BLUE}========================================${NC}\n"
    ...
    if [[ $TESTS_FAILED -eq 0 ]]; then
        echo -e "\n${GREEN}✅ All tests passed! Debian 13 appears to be compatible with foxitiPanel.${NC}"
        return 0
...
show_usage() {
    ...
    echo ""
    echo "This script tests foxitiPanel compatibility with Debian 13."
