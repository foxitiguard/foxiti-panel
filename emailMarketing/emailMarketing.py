...
class emailMarketing(multi.Thread):
    ...
    def run(self):
        try:
            ...
            logging.FoxitiCPLogFileWriter.writeToFile(str(msg) + ' [emailMarketing.run]')
    ...
    def createEmailList(self):
        try:
                                ...
                                if re.match('^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$', value) != None:
                                    try:
                                        ...
                                        try:
                                            ...
                                            pass
                                    logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], str(counter) + ' emails read.')
                                    counter = counter + 1
                        except BaseException as msg:
                            logging.FoxitiCPLogFileWriter.writeToFile('%s. [createEmailList]' % (str(msg)))
                            continue
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], str(counter) + 'Successfully read all emails. [200]')
        except BaseException as msg:
            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], str(msg) +'. [404]')
            return 0
    ...
    def verificationJob(self):
        try:
            ...
            if not os.path.exists('/home/foxitipanel/' + domain):
                os.mkdir('/home/foxitipanel/' + domain)

            tempStatusPath = '/home/foxitipanel/' + domain + "/" + self.extraArgs['listName']
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, 'Starting verification job..')
            ...
            configureVerifyPath = '/home/foxitipanel/configureVerify'
            ...
            for items in allEmailsInList:
                if items.verificationStatus != 'Verified':
                    try:
                        ...
                        logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, str(counter) + ' emails verified so far..')
                        counter = counter + 1
                    except BaseException as msg:
                        items.verificationStatus = 'Verification Failed'
                        items.save()
            ...
            logging.FoxitiCPLogFileWriter.statusWriter(tempStatusPath, str(counter) + ' emails successfully verified. [200]')
        except BaseException as msg:
            verificationList = EmailLists.objects.get(listName=self.extraArgs['listName'])
            domain = verificationList.owner.domain
            tempStatusPath = '/home/foxitipanel/' + domain + "/" + self.extraArgs['listName']
    ...
    def startEmailJob(self):
        try:
            ...
            if self.setupSMTPConnection() == 0:
                logging.FoxitiCPLogFileWriter.writeToFile('SMTP Connection failed. [301]')
                return 0
                        ...
                        try:
                            ...
                            logging.FoxitiCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
                                                                      'Successfully sent: ' + str(
                                                                          sent) + ', Failed: ' + str(failed))
                            if self.setupSMTPConnection() == 0:
                                logging.FoxitiCPLogFileWriter.writeToFile('SMTP Connection failed. Error: %s. [399]' % (str(msg)))
                                return 0
