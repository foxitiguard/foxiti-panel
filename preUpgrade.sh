...
VERSION_DATA=$(curl -s https://foxitipanel.net/version.txt)
...
case "$BRANCH_NAME" in
  ...
  *)
    echo ""
    echo "Failed to determine the foxitiPanel version from https://foxitipanel.net/version.txt (got: '$BRANCH_NAME')."
    echo "Please check your network connection and retry later."
    exit 1
    ;;
esac

UPGRADE_URL="https://raw.githubusercontent.com/usmannasir/foxitipanel/$BRANCH_NAME/foxitipanel_upgrade.sh"

rm -f /usr/local/foxitipanel_upgrade.sh

ATTEMPT=1
while [ "$ATTEMPT" -le 3 ]; do
    HTTP_CODE=$(curl -s -o /usr/local/foxitipanel_upgrade.sh -w "%{http_code}" "$UPGRADE_URL" 2>/dev/null)
    if [ "$HTTP_CODE" = "200" ] && head -n 1 /usr/local/foxitipanel_upgrade.sh | grep -q '^#!'; then
        chmod 700 /usr/local/foxitipanel_upgrade.sh
        exec /usr/local/foxitipanel_upgrade.sh
    fi
