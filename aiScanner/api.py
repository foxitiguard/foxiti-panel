...
def validate_access_token(token, scan_id):
    ...
    try:
        ...
        # OPTION 2: Try foxitiPanel's own API Key (for post-scan file operations from platform)
        # The platform sends back the same API key that foxitiPanel used to submit the scan
        try:
            ...
            # First, check if this is a valid foxitiPanel API key (any admin's key)
...
def scan_callback(request):
    ...
    try:
        ...
        # Update scan status in foxitiPanel database
        try:
