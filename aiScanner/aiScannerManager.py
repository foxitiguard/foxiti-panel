...
class AIScannerManager:
    AI_SCANNER_API_BASE = 'https://cp.foxitiguard.com/ai-scanner'
    ...
    def setupPayment(self, request, userID):
        ...
        try:
            ...
            foxitipanel_host = request.get_host()  # Keep full host including port
            foxitipanel_domain = foxitipanel_host.split(':')[0]  # Domain only for email fallback
            admin_email = admin.email if hasattr(admin, 'email') and admin.email else f'{admin.userName}@{foxitipanel_domain}'
            
            self.logger.writeToFile(f'[AIScannerManager.setupPayment] Admin: {admin.userName}, Email: {admin_email}, Host: {foxitipanel_host}')
            
            # Setup payment with AI Scanner API
            self.logger.writeToFile(f'[AIScannerManager.setupPayment] Attempting payment setup for {admin_email} on {foxitipanel_host}')
            setup_data = self.setup_ai_scanner_payment(admin_email, foxitipanel_host)
    ...
    def addPaymentMethod(self, request, userID):
        ...
        try:
            ...
            setup_data = self.setup_add_payment_method(api_key_to_use, admin_email, foxitipanel_host)
    ...
    def setup_ai_scanner_payment(self, user_email, foxitipanel_host):
        """Setup payment method with AI Scanner API"""
        try:
            payload = {
                'email': user_email,
                'domain': foxitipanel_host.split(':')[0],  # Send domain without port
                'return_url': f'https://{foxitipanel_host}/aiscanner/setup-complete/'  # Include port in URL
            }
            
            self.logger.writeToFile(f'[AIScannerManager.setup_ai_scanner_payment] Sending request to: {self.AI_SCANNER_API_BASE}/foxitipanel/setup-payment/')
            self.logger.writeToFile(f'[AIScannerManager.setup_ai_scanner_payment] Payload: {payload}')
            
            response = requests.post(
                f'{self.AI_SCANNER_API_BASE}/foxitipanel/setup-payment/',
                json=payload,
                timeout=10
            )
    ...
    def submit_wordpress_scan(self, api_key, domain, scan_type, callback_url, file_access_token, file_access_base_url, scan_id, server_ip):
        ...
        try:
            payload = {
                ...
                'site_url': domain,
                'scan_type': scan_type,
                'foxitipanel_callback': callback_url,
    ...
    def check_vps_free_scans(self, server_ip):
        ...
        try:
            ...
            response = requests.post(
                'https://cp.foxitiguard.com/ai-scanner/api/vps/check-free-scans/',
                json={'ip': server_ip},
                timeout=10
            )
    ...
    def setup_add_payment_method(self, api_key, user_email, foxitipanel_host):
        """Setup additional payment method with AI Scanner API"""
        try:
            payload = {
                'domain': foxitipanel_host.split(':')[0],  # Send domain without port
                'return_url': f'https://{foxitipanel_host}/aiscanner/payment-method-complete/',  # Include port in URL
                'action': 'add_payment_method'  # Indicate this is adding a payment method, not initial setup
            }
            
            self.logger.writeToFile(f'[AIScannerManager.setup_add_payment_method] Sending request to: {self.AI_SCANNER_API_BASE}/foxitipanel/add-payment-method/')
            self.logger.writeToFile(f'[AIScannerManager.setup_add_payment_method] Payload: {payload}')
            
            response = requests.post(
                f'{self.AI_SCANNER_API_BASE}/foxitipanel/add-payment-method/',
                headers={'X-API-Key': api_key},
                json=payload,
                timeout=10
            )
