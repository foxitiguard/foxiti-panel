...
class TwoFactorAPIIntegrationTests(SimpleTestCase):
    def setUp(self):
        required = {
            'base_url': os.environ.get('FOXITIPANEL_2FA_TEST_URL', '').rstrip('/'),
            'username': os.environ.get('FOXITIPANEL_2FA_TEST_USERNAME', ''),
            'token': os.environ.get('FOXITIPANEL_2FA_TEST_TOKEN', ''),
            'secret': os.environ.get('FOXITIPANEL_2FA_TEST_SECRET', ''),
        }
    ...
    def test_token_requires_two_factor_for_api_and_session_handoff(self):
        ...
        self.assertNotIn('foxitipanel_sessionid', session.cookies)

        allowed = session.get(
            access_url,
            params=params,
            headers={'X-foxitiPanel-OTP': pyotp.TOTP(self.config['secret']).now()},
            allow_redirects=False,
            timeout=20,
        )
        self.assertEqual(allowed.status_code, 302)
        self.assertIn('foxitipanel_sessionid', session.cookies)
