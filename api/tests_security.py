...
class SecurityUtilsTests(SimpleTestCase):
    ...
    def test_terminal_secret_prefers_secret_file(self):
        ...
        old_path = os.environ.get("FOXITIPANEL_TERMINAL_JWT_SECRET_FILE")
        old_secret = os.environ.get("FOXITIPANEL_TERMINAL_JWT_SECRET")
        try:
            os.environ["FOXITIPANEL_TERMINAL_JWT_SECRET_FILE"] = secret_path
            os.environ.pop("FOXITIPANEL_TERMINAL_JWT_SECRET", None)
            self.assertEqual(get_terminal_jwt_secret(), expected_secret)
        finally:
            if old_path is None:
                os.environ.pop("FOXITIPANEL_TERMINAL_JWT_SECRET_FILE", None)
            else:
                os.environ["FOXITIPANEL_TERMINAL_JWT_SECRET_FILE"] = old_path

            if old_secret is not None:
                os.environ["FOXITIPANEL_TERMINAL_JWT_SECRET"] = old_secret
    ...
    def test_terminal_secret_fails_when_not_configured(self):
        old_path = os.environ.get("FOXITIPANEL_TERMINAL_JWT_SECRET_FILE")
        old_secret = os.environ.get("FOXITIPANEL_TERMINAL_JWT_SECRET")
        os.environ["FOXITIPANEL_TERMINAL_JWT_SECRET_FILE"] = "/path/that/does/not/exist"
        os.environ.pop("FOXITIPANEL_TERMINAL_JWT_SECRET", None)
