...
class TestAPI(TestCase):
    ...
    def MakeRequest(self, endPoint, data):
        ...
        path = 'https://foxitipanel.xyz:8090/api/%s' % (endPoint)
    ...
    def test_submitWebsiteCreation(self):
        ...
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'domainName': 'api.foxitipanel.xyz', 'ownerEmail': 'usman@cyberpersons.com' , 'phpSelection': 'PHP 7.1',
        ...
        if self.MakeRequestRaw('http://api.foxitipanel.xyz').find('foxitiPanel') > -1:
            exists = 1
        ...
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'domainName': 'api.foxitipanel.xyz'}
        ...
        if self.MakeRequestRaw('http://api.foxitipanel.xyz').find('404') > -1:
            exists = 1
    ...
    def test_changePackageAPI(self):
        ...
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'websiteName': 'foxitipanel.xyz', 'packageName': 'Default'}
    ...
    def test_submitWebsiteStatus(self):
        ...
        ## Suspend  check
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'websiteName': 'foxitipanel.xyz', 'state': 'Suspend'}
        ...
        if self.MakeRequestRaw('http://foxitipanel.xyz').find('404') > -1 or self.MakeRequestRaw(
                'http://foxitipanel.xyz').find('Access to this resource on the server is denied!') > -1:
            exists = 1
        ...
        suspend = 0
        import os
        if os.path.exists('/usr/local/lsws/conf/vhosts/foxitipanel.xyz-suspended'):
            suspend = 1
        ...
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'websiteName': 'foxitipanel.xyz', 'state': 'Unsuspend'}
        ...
        if self.MakeRequestRaw('http://foxitipanel.xyz').find('foxitiPanel') > -1:
            exists = 1
        ...
        suspend = 0
        import os
        if os.path.exists('/usr/local/lsws/conf/vhosts/foxitipanel.xyz'):
            suspend = 1
