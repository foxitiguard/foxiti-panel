...
# foxitiPanel phpMyAdmin Access Control Rollback Script
# This script reverts the phpMyAdmin access control changes

echo "=== foxitiPanel phpMyAdmin Access Control Rollback ==="
...
LATEST_BACKUP=$(ls -t /usr/local/FoxitiCP/public/phpmyadmin/index.php.backup.* 2>/dev/null | head -n1)
...
cp "$LATEST_BACKUP" /usr/local/FoxitiCP/public/phpmyadmin/index.php

# Remove the .htaccess file if it exists
if [ -f "/usr/local/FoxitiCP/public/phpmyadmin/.htaccess" ]; then
    echo "Removing .htaccess file..."
    rm /usr/local/FoxitiCP/public/phpmyadmin/.htaccess
fi
